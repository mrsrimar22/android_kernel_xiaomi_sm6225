...
then
	...
	grep -Ev --binary-files=text '\(null\)|rtc: 000000000* ' |
...
then
	...
	n_warn=`grep -v 'Warning: unable to open an initial console' $file | grep -Ec 'WARNING:|Warn'`
	if test "$n_warn" -ne 0
	then
		summary="$summary  Warnings: $n_warn"
	fi
	n_bugs=`grep -Ec 'BUG|Oops:' $file`
	if test "$n_bugs" -ne 0
	then
	...
	n_stalls=`grep -Ec 'detected stalls on CPUs/tasks:|self-detected stall on CPU|Stall ended before state dump start|\?\?\? Writer stall state' $file`
