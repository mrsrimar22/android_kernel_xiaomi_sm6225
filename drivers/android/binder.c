					...
					bool inherit_rt)
{
	...
	if (!inherit_rt && is_rt_policy(desired_prio.sched_policy)) {
		/*
		 * Some app processes are boosted to FIFO, but a binder
		 * out thread inheriting from FIFO would end up with a low
		 * priority, so give it a higher one instead.
		 */
			       ...
			       binder_size_t extra_buffers_size)
{
	...
	if (reply) {
		...
		t->timesRecord = trace_clock_local();
		binder_inner_proc_unlock(proc);
			      ...
			      struct binder_proc *proc, u64 timesRecord)
{
	...
	/* one-way (async) transactions queued on nodes */
	for (n = rb_first(&proc->nodes); n != NULL; n = rb_next(n)) {
