...
#define _BQ_RATE(a, b) _BQ_RAW(a, b)

#if defined(__COUNTER__)
#define _BQ_FALLBACK() _BQ_RATE(bq_rl_, __COUNTER__)
#else
#define _BQ_FALLBACK() _BQ_RATE(bq_rl_, __LINE__)
#endif

#define _BQ_LOG_WRAPPER(name, fmt, ...)			\
do {							\
...
#define bq_log(fmt, ...)	_BQ_LOG_WRAPPER(_BQ_FALLBACK(), fmt, ##__VA_ARGS__)
...
static int bq2589x_set_watchdog_timer(struct bq2589x *bq, u8 timeout)
{
...
static int bq2589x_psy_register(struct bq2589x *bq)
{
	...
	if (IS_ERR(bq->wall_psy)) {
		...
		bq_err("failed to register wall psy: %d\n", ret);
	}
...
static void bq2589x_start_charging_workfunc(struct work_struct *work)
{
	...
	while (bq->bms_psy && bq->batt_psy && times <= 50) {
		...
		bq_info("times: %d, status: %d\n", times, status);
		if (status != last_status) {
			last_status = status;
			power_supply_changed(bq->batt_psy);
		}
		if (status == POWER_SUPPLY_STATUS_CHARGING) {
			//bq_info("power_supply_changed: bms_psy\n");
			power_supply_changed(bq->bms_psy);
			break;
		}
		times++;
...
//modify by HTH-234945/HTH-234948 at 2022/06/08 begin
	if (bq->part_no == SC89890H) {
		...
		if (!(vbus_status & BQ2589X_VBUS_GD_MASK) && (bq->status & BQ2589X_STATUS_PLUGIN)) {
			bq2589x_usb_switch(bq, false);
			msleep(5); // rest for power output
			bq2589x_usb_switch(bq, true);
			bq2589x_adc_stop(bq);
			bq->status &= ~BQ2589X_STATUS_PLUGIN;
...
static irqreturn_t bq2589x_charger_interrupt(int irq, void *data)
{
	...
	queue_delayed_work(bq->irq_wq, &bq->irq_work, msecs_to_jiffies(5));
				...
				unsigned long event, void *data)
{
	...
	case TCP_NOTIFY_SOURCE_VBUS:
		bq_info("TCP_NOTIFY_SOURCE_VBUS\n");
			...
			const struct i2c_device_id *id)
{
	...
	/* Initialize descriptor pointers to NULL */
	bq->irq_gpiod = NULL;
	...
	/* Require irq descriptor from DT */
	if (!bq->irq_gpiod) {
		pr_err("irq descriptor unavailable\n");
		ret = -EINVAL;
		goto err_gpio;
	}
	...
	if (!bq->irq_wq) {
		pr_err("failed to create irq workqueue\n");
		ret = -ENOMEM;
		goto err_psy;
	}

	ret = devm_add_action_or_reset(&client->dev, bq2589x_wq_destroy, bq->irq_wq);
	if (ret)
		goto err_psy;
	...
	if (ret) {
		pr_err("request_threaded_irq failed for IRQ %d: %d\n", irqn, ret);
		goto err_irq;
	}
	...
	if (bq->usb_switch1_gpiod) {
		...
		/* Keep conservative default value. */
		bq->usb_switch_flag = 0;
	}
	pr_info("initial usb_switch_flag: %d\n", bq->usb_switch_flag);
