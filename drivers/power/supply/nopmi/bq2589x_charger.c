...
static int bq2589x_set_fast_charge_mode(struct bq2589x *bq, int pd_active)
{
	...
	if (bq->bms_psy) {
		...
		if (rc < 0)
			pr_err("get battery capacity fail\n");
		else
...
static int __bq2589x_read_byte(struct bq2589x *bq, u8 reg, u8 *data)
{
	...
	do {
		...
		pr_err("failed to read reg 0x%02x, ret=%d\n", reg, ret);
		udelay(200);
	} while (--retry);
...
static int __bq2589x_write_byte(struct bq2589x *bq, u8 reg, u8 data)
{
	...
	do {
		...
		pr_err("failed to write 0x%02x to reg 0x%02x, ret=%d\n", data, reg, ret);
		udelay(200);
	} while (--retry);
...
static int bq2589x_update_bits(struct bq2589x *bq, u8 reg, u8 mask, u8 data)
{
	...
	if (ret)
		pr_err("failed to write reg 0x%02x, ret=%d\n", reg, ret);
...
int bq2589x_adc_start(struct bq2589x *bq, bool oneshot)
{
	...
	if (ret < 0) {
		pr_err("failed to read register 0x02:%d\n", ret);
		return ret;
	}
...
int bq2589x_get_charge_current(struct bq2589x *bq)
{
	...
	if (ret < 0) {
		pr_err("failed to read register 0x00:%d\n", ret);
		return ret;
	}
...
u8 bq2589x_get_charging_status(struct bq2589x *bq)
{
	...
	if (val == BQ2589X_CHRG_STAT_IDLE) {
		...
		if (bq->bms_psy) {
			...
			if (ret < 0) {
				pr_err("get battery cap fail\n");
			}
			cap = propval.intval;
			bq_dbg(PR_OEM, "battery cap: %d\n", cap);
		}
...
//modify by HTH-209427/HTH-209841 at 2022/05/12 end
	...
	if (ret < 0) {
		bq_dbg(PR_OEM, "failed to set prechg current: %d\n", ret);
		return ret;
	}
	...
	if (bq->part_no == SYV690) {
		bq_dbg(PR_OEM, "init syv690 HV_TYPE 9/12V\n");
...
static int bq2589x_charge_status(struct bq2589x *bq)
{
	...
	bq2589x_read_byte(bq, BQ2589X_REG_03, &val1);
	bq_dbg(PR_OEM, "get charge status: REG_0B=0x%x, REG_03=0x%x\n", val, val1);
...
static int bq2589x_psy_register(struct bq2589x *bq)
{
	...
	if (IS_ERR(bq->wall_psy)) {
		...
		pr_err("failed to register wall psy: %d\n", ret);
	}
...
{
	bq_dbg(PR_OEM, "start unregister\n");
}
...
static int bq2589x_parse_dt(struct device *dev, struct bq2589x *bq)
{
	...
	if (ret < 0) {
		bq_dbg(PR_OEM, "no intr_gpio info\n");
		return ret;
	} else {
		bq_dbg(PR_OEM, "intr_gpio info: %d\n", bq->irq_gpio);
	}

	bq->usb_switch1 = of_get_named_gpio(np, "usb-switch1", 0);
	if (ret < 0) {
		bq_dbg(PR_OEM, "no usb-switch1 info\n");
		return ret;
	}
...
static void bq2589x_ico_workfunc(struct work_struct *work)
{
	...
	if (bq->part_no == SYV690) {
		pr_info("SYV690 IC detected, skip ico\n");
		return;
	}
...
static void bq2589x_monitor_workfunc(struct work_struct *work)
{
	...
	if (bq->bms_psy) {
		...
		batt_temp = propval.intval;
		bq_dbg(PR_OEM, "get battery temp: %d\n", batt_temp);
	}
...
static void bq2589x_start_charging_workfunc(struct work_struct *work)
{
	...
	while (bq->bms_psy && bq->batt_psy && times <= 50) {
		...
		bq_dbg(PR_OEM, "times: %d, status: %d\n", times, status);
		if (status != last_status) {
			last_status = status;
			power_supply_changed(bq->batt_psy);
		}
		if (status == POWER_SUPPLY_STATUS_CHARGING) {
			//bq_dbg(PR_OEM, "power_supply_changed: bms_psy\n");
			power_supply_changed(bq->bms_psy);
			break;
		}
		times++;
...
/*static int bq2589x_set_charger_type(struct bq2589x *bq, enum power_supply_type chg_type)
{
	...
	if (ret < 0)
		pr_err("set prop REAL_TYPE fail: %d\n", ret);
...
static int bq2589x_set_charger_type(struct bq2589x *bq, enum power_supply_type chg_type)
{
	...
	if (!bq->pd_active || propval.intval) {
		ret = power_supply_set_property(bq->usb_psy, POWER_SUPPLY_PROP_ONLINE, &propval);
		if (ret < 0)
			pr_err("inform power supply usb_online failed, ret=%d\n", ret);
	}
...
static void bq2589x_charger_irq_workfunc(struct work_struct *work)
{
	...
	/*if (prev_chg_type == bq->vbus_type) {
		pr_err("prev_chg_type == new_chg_type\n");
		return;
	}*/
...
//modify by HTH-234945/HTH-234948 at 2022/06/08 begin
	if (bq->part_no == SC89890H) {
		...
		if (!(vbus_status & BQ2589X_VBUS_GD_MASK) && (bq->status & BQ2589X_STATUS_PLUGIN)) {
			...
			bq_dbg(PR_OEM, "adapter plugged in\n");
...
static void set_pd_active(struct bq2589x *bq, int pd_active)
{
	...
	if (bq->usb_psy) {
		...
		if (rc < 0)
			bq_dbg(PR_OEM, "Couldn't set USB Present status, rc=%d\n", rc);
...
static int bq2589x_set_cc_orientation(struct bq2589x *bq, int cc_orientation)
{
	...
	if (ret < 0)
		pr_err("set prop CC_ORIENTATION fail: %d\n", ret);
...
static int bq2589x_set_typec_mode(struct bq2589x *bq, enum power_supply_typec_mode typec_mode)
{
	...
	if (ret < 0)
		pr_err("set prop TYPEC_MODE fail: %d\n", ret);
				...
				unsigned long event, void *data)
{
	...
	case TCP_NOTIFY_SINK_VBUS:
		bq_dbg(PR_OEM, "TCP_NOTIFY_SINK_VBUS\n");
		break;
			   ...
			   const struct i2c_device_id *id)
{
	...
	if (ret) {
		bq_dbg(PR_OEM, "%d gpio request failed\n", bq->irq_gpio);
		goto err_gpio;
	}

	irqn = gpio_to_irq(bq->irq_gpio);
	if (irqn < 0) {
		bq_dbg(PR_OEM, "%d gpio_to_irq failed\n", irqn);
		ret = irqn;
		goto err_gpio;
	}
	client->irq = irqn;
...
//modify by HTH-209427/HTH-209841 at 2022/05/12 end
	...
	if (ret) {
		bq_dbg(PR_OEM, "request IRQ %d failed: %d\n", client->irq, ret);
		goto err_irq;
	} else {
		bq_dbg(PR_OEM, "IRQ = %d\n", client->irq);
	}
