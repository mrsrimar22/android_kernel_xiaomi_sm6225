...
static void sc8551_irq_work(struct work_struct *work)
{
	...
	struct sc8551 *sc = container_of(dwork, struct sc8551, irq_work);
...
static irqreturn_t sc8551_charger_interrupt(int irq, void *dev_id)
{
	...
	queue_delayed_work(sc->irq_wq, &sc->irq_work, msecs_to_jiffies(5));
		...
		const struct i2c_device_id *id)
{
	...
	ret = sysfs_create_group(&client->dev.kobj, &sc8551_attr_group);
	if (ret) {
	...
	sc->irq = irq;
	ret = devm_request_threaded_irq(&client->dev, sc->irq,
				NULL, sc8551_charger_interrupt,
				IRQF_TRIGGER_FALLING | IRQF_ONESHOT,
				"sc8551 charger irq", sc);
	if (ret < 0) {
...
err_remove_sysfs:
	sysfs_remove_group(&client->dev.kobj, &sc8551_attr_group);
err_unreg_psy:
