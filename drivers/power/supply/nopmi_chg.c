...
static int nopmi_update_batt_temp(struct nopmi_chg *nopmi_chg)
{
	...
	if (ret < 0) {
		...
		WRITE_ONCE(nopmi_chg->batt_temp_c, 0);
		return ret;
	}

	temp_c = pval.intval / 10;
	WRITE_ONCE(nopmi_chg->batt_temp_c, temp_c);
	return 0;
}
...
static int nopmi_update_batt_volt(struct nopmi_chg *nopmi_chg)
{
	...
	if (ret < 0) {
		...
		WRITE_ONCE(nopmi_chg->batt_volt_m, 0);
		return ret;
	}

	volt_m = pval.intval / 1000;
	WRITE_ONCE(nopmi_chg->batt_volt_m, volt_m);
	return 0;
}
...
static void nopmi_update_prop_system_temp_level(struct nopmi_chg *nopmi_chg)
{
	...
	temp_c = READ_ONCE(nopmi_chg->batt_temp_c);
		...
		union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		batt_volt_m = READ_ONCE(nopmi_chg->batt_volt_m);
		rc = power_supply_get_property(nopmi_chg->main_psy,
...
static void nopmi_cv_step_monitor(struct nopmi_chg *nopmi_chg)
{
	...
	pr_info("cccv_step check = batt_volt: %d, batt_curr: %d\n", batt_volt, batt_curr);
...
static void nopmi_chg_workfunc(struct work_struct *work)
{
	...
	if (READ_ONCE(nopmi_chg->is_awake)) {
		nopmi_update_prop_system_temp_level(nopmi_chg);
