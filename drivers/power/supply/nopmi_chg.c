		...
		const union power_supply_propval *val)
{
	...
	if (val->intval == 0) {
		rc = vote(usb_icl_votable, MAIN_CHG_SUSPEND_VOTER, true, icl);
	} else {
		rc = vote(usb_icl_votable, MAIN_CHG_SUSPEND_VOTER, false, 0);
	}
...
static int nopmi_update_system_temp_level(struct nopmi_chg *nopmi_chg, int level)
{
	...
	if (!nopmi_chg->batt_psy) {
		...
		if (!nopmi_chg->batt_psy) {
			pr_err("get batt_psy fail!\n");
			return -ENODEV;
		}
	}

	rc = power_supply_get_property(nopmi_chg->batt_psy, POWER_SUPPLY_PROP_TEMP, &pval);
	if (rc < 0) {
		pr_err("read batt temp property fail, rc=%d\n", rc);
		return rc;
	}
	...
	if (pval.intval <= 30)
		tmp = 0;
	else if (pval.intval <= 35)
		tmp = 1 + (pval.intval - 31) / 2;
	else if (pval.intval <= 42)
		tmp = 4 + (pval.intval - 36);
	else
		tmp = level;

	if (tmp > nopmi_chg->thermal_levels)
		tmp = nopmi_chg->thermal_levels;

	nopmi_chg->system_temp_level = tmp;
		...
		const union power_supply_propval *val)
{
	...
	if (level < 0 || nopmi_chg->thermal_levels < 0 || level > nopmi_chg->thermal_levels)
		return -EINVAL;

	if (level == nopmi_chg->system_temp_level)
		return 0;

	rc = nopmi_update_system_temp_level(nopmi_chg, level);
	if (rc < 0) {
		pr_err("Failed to update system_temp_level, rc=%d", rc);
		return rc;
	}
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if (nopmi_chg->bms_psy) {
			...
			if (rc < 0) {
				...
				pr_err("get bms voltage_now fail\n");
			}
			if (g_nopmi_chg->usb_online && value.intval < 3300000)
				g_nopmi_chg->usb_online = 0;
		}

		if (NOPMI_CHARGER_IC_NONE != nopmi_get_charger_ic_type() && NOPMI_CHARGER_IC_MAX != nopmi_get_charger_ic_type()) {
			if (g_nopmi_chg->usb_online && !g_nopmi_chg->is_awake) {
				pm_stay_awake(g_nopmi_chg->dev);
				start_nopmi_chg_workfunc();
				g_nopmi_chg->is_awake = true;
				pr_info("wakelock held for %d\n", psp);
			} else if (!g_nopmi_chg->usb_online && g_nopmi_chg->is_awake) {
				stop_nopmi_chg_workfunc();
				pm_relax(g_nopmi_chg->dev);
				g_nopmi_chg->is_awake = false;
				pr_info("wakelock released from %d\n", psp);
			}
		}
		ret = 0;
		break;
...
#if 0
	if (!of_property_read_s32(np, "normal-charge-voltage", &chg->jeita_ctl.dt.normal_charge_voltage)) {
		chg->jeita_ctl.dt.normal_charge_voltage = JEITA_TEMP_NORMAL_VOLTAGE;
	}
#endif
...
static int nopmi_parse_dt_thermal(struct nopmi_chg *chg, struct device_node *np)
{
	...
	if (of_find_property(np, "nopmi,thermal-mitigation", &byte_len)) {
		...
		if (rc < 0) {
			pr_err("Couldn't read thremal limits, rc=%d\n", rc);
			return rc;
		}
	}
...
static void nopmi_cv_step_monitor_work(struct work_struct *work)
{
	...
	for (i = 0; i < step_table_max; i++) {
		if (batt_volt >= pcc_cv_step_config[i].volt_lim - CV_BATT_VOLT_HYSTERESIS && batt_curr > pcc_cv_step_config[i].curr_lim) {
			if (++step_count[i] >= 2) {
				stepdown = 1;
				step_count[i] = 0;
				pr_info("fg_cc_cv_step_check = stepdown at step %u\n", i);
			}
			break;
		} else {
			step_count[i] = 0;
	...
	pr_info("fg_cc_cv_step_check = step table: %d, cccv_step vote: %dmA, stepdown: %d, finalFCC: %dmA\n",
...
static int nopmi_chg_probe(struct platform_device *pdev)
{
	...
	if (probe_cnt > PROBE_CNT_MAX) {
		pr_err("stop, probe_cnt: %d, PROBE_CNT_MAX: %d\n", probe_cnt, PROBE_CNT_MAX);
		return 0;
	}
