...
static int usbpd_request_vdm_cmd(enum uvdm_state cmd, unsigned char *data)
{
	...
	if (in_interrupt()) {
		int_data = kzalloc(40, GFP_ATOMIC);
		msg_maxim("kzalloc atomic ok.");
	} else {
		int_data = kzalloc(40, GFP_KERNEL);
		msg_maxim("kzalloc kernel ok.");
	}
...
static void max77729_request_response(struct max77729_usbc_platform_data *usbc_data)
{
	...
	pr_err("OPCODE(0x%02x) W_LENGTH(%d) R_LENGTH(%d)",
...
static int pd_set_pd_verify_process(struct device *dev, int verify_in_process)
{
	...
	msg_maxim("pd verify in process: %d", verify_in_process);
/*
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (kstrtou32(buf, 0, &start_fw_update)) {
		msg_maxim("Failed converting from str to u32.");
	}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (in_interrupt()) {
		data = kzalloc(40, GFP_ATOMIC);
		msg_maxim("kzalloc atomic ok.");
	} else {
		data = kzalloc(40, GFP_KERNEL);
		msg_maxim("kzalloc kernel ok.");
	}
	memset(data, 0, 40);
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	/* allocate bounded, zeroed buffer and check */
	data = kzalloc(alloc_size, gfp);
	if (!data) {
		msg_maxim("allocation failed");
		return -ENOMEM;
	}
	msg_maxim("%s kzalloc %s ok.", __func__, in_interrupt() ? "atomic" : "kernel");

	/* use bounded %s to avoid overflow of buffer */
	sscanf_ret = sscanf(buf, "%d,%63s", &cmd, buffer);
	if (sscanf_ret <= 0) {
		/* try parse only cmd */
		sscanf_ret = sscanf(buf, "%d", &cmd);
		if (sscanf_ret <= 0) {
			msg_maxim("%s: invalid input", __func__);
			kfree(data);
			return -EINVAL;
		}
		/* no hex token */
	...
	msg_maxim("cmd: %d, buffer: %s", cmd, buffer[0] ? buffer : "<empty>");

	/* convert hex into data (StringToHex unchanged) */
	StringToHex(buffer, data, &count);

	/* defensive cap: ensure StringToHex didn't produce more than we allocated */
	if (count > alloc_size)
		count = alloc_size;

	msg_maxim("count = %u", count);

	/* record and dispatch */
	vdm_count = count;
	rc = usbpd_request_vdm_cmd(cmd, (unsigned char *)data);
	if (rc < 0)
		msg_maxim("%s: usbpd_request_vdm_cmd failed: %d", __func__, rc);
		...
		struct device_attribute *attr, char *buf)
{
	...
	msg_maxim("request_vdm_cmd_show: uvdm_state: %d", usbpd_data->uvdm_state);
	switch (usbpd_data->uvdm_state) {
	case USBPD_UVDM_CHARGER_VERSION:
		return snprintf(buf, PAGE_SIZE, "%d,%x\n", usbpd_data->uvdm_state, usbpd_data->vdm_data.ta_version);
		break;
	case USBPD_UVDM_CHARGER_TEMP:
		return snprintf(buf, PAGE_SIZE, "%d,%d\n", usbpd_data->uvdm_state, usbpd_data->vdm_data.ta_temp);
		break;
	case USBPD_UVDM_CHARGER_VOLTAGE:
		return snprintf(buf, PAGE_SIZE, "%d,%d\n", usbpd_data->uvdm_state, usbpd_data->vdm_data.ta_voltage);
		break;
	case USBPD_UVDM_SESSION_SEED:
	...
	/* case USBPD_UVDM_REVERSE_AUTHEN: */
		/* return snprintf(buf, PAGE_SIZE, "%d,%d\n", usbpd_data->uvdm_state, pd->vdm_data.reauth); */
		/* break; */
	case USBPD_UVDM_AUTHENTICATION:
		for (i = 0; i < 4; i++) {
			memset(data, 0, sizeof(data));
			snprintf(data, sizeof(data), "%08lx\n", usbpd_data->vdm_data.digest[i]);
			strlcat(str_buf, data, sizeof(str_buf));
		}

		msg_maxim("str_buf: %s", str_buf);
		return snprintf(buf, PAGE_SIZE, "%d,%s\n", usbpd_data->uvdm_state, str_buf);
		break;
	default:
		...
		struct device_attribute *attr, char *buf)
{
	...
	msg_maxim("current_state is %d", usbpd_data->pd_state);

	if (usbpd_data->sink_Ready) {
		msg_maxim("%s", "SNK_Ready");
		return snprintf(buf, PAGE_SIZE, "%s\n", "SNK_Ready");
	} else if (usbpd_data->source_Ready) {
		msg_maxim("%s", "SRC_Ready");
		return snprintf(buf, PAGE_SIZE, "%s\n", "SRC_Ready");
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	msg_maxim("adapter_id is %08x", usbpd_data->adapter_id);
		...
		struct device_attribute *attr, char *buf)
{
	...
	max77729_pd_get_svid(usbpd_data);
	msg_maxim("adapter_svid is %04x", usbpd_data->adapter_svid);
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	msg_maxim("batterysecret verify process: %d", usbpd_data->verify_process);
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	msg_maxim("batt_verify set usbpd verifyed: %d", val);
		...
		struct device_attribute *attr, char *buf)
{
	...
	msg_maxim("current_pr is %d", usbpd_data->typec_power_role);
...
static int max77729_usbc_probe(struct platform_device *pdev)
{
	...
	INIT_WORK(&usbc_data->fw_update_work, max77729_firmware_update_sysfs_work);
