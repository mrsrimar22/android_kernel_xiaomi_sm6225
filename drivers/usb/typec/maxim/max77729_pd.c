...
#undef pr_fmt
#define pr_fmt(fmt) "[max77729_pd]: %s: " fmt "\n", __func__
...
static void set_pd_active(struct max77729_usbc_platform_data *usbc_data, int pd_active)
{
	...
	pr_info("set_pd_active %d", pd_active);
	...
	if (usbc_data->usb_psy) {
		...
		if (rc < 0)
			pr_err("Couldn't read USB Present status, rc=%d", rc);
...
static void max77729_process_pd(struct max77729_usbc_platform_data *usbc_data)
{
	...
	pr_info("current_pdo_num(%d), available_pdo_num(%d), has_apdo(%d)",
...
void max77729_select_pdo(int num)
{
	...
	pr_info("OPCODE(0x%02x) W_LENGTH(%d) R_LENGTH(%d) NUM(%d)",
		...
		unsigned char *data)
{
	...
	pr_info("%s (0x%02X)", result ? "Error," : "Sent,", result);

	switch (result) {
	case 0x00:
		pr_info("Sent PDO Request Message to Port Partner(0x%02X)", result);
		break;
	case 0xFE:
		pr_info("Error, SinkTxNg(0x%02X)", result);
		break;
	case 0xFF:
		pr_info("Error, Not in SNK Ready State(0x%02X)", result);
		break;
	default:
		break;
	}
...
void max77729_set_enable_pps(bool enable, int ppsVol, int ppsCur)
{
	...
	if (enable) {
		...
		value.read_length = 1;
		pr_info("PPS_On (Vol:%dmV, Cur:%dmA)", ppsVol, ppsCur);
	} else {
		...
		unsigned char *data)
{
	...
	pr_info("PPS_%s (0x%02X)", usbc_data->pd_data->bPPS_on ? "On" : "Off", result);
}
		...
		unsigned char *data)
{
	...
	switch (result) {
	case 0x00:
		pr_info("Sent APDO Request Message to Port Partner(0x%02X)", result);
		break;
	case 0x01:
		pr_info("Error, Invalid APDO position(0x%02X)", result);
		break;
	case 0x02:
		pr_info("Error, Invalid Output Voltage(0x%02X)", result);
		break;
	case 0x03:
		pr_info("Error, Invalid Operating Current(0x%02X)", result);
		break;
	case 0x04:
		pr_info("Error, PPS Function Off(0x%02X)", result);
		break;
	case 0x05:
		pr_info("Error, Not in SNK Ready State(0x%02X)", result);
		break;
	case 0x06:
		pr_info("Error, PD2.0 Contract(0x%02X)", result);
		break;
	case 0x07:
		pr_info("Error, SinkTxNg(0x%02X)", result);
		break;
	...
	if (vbvolt != 0x01)
		pr_info("Error, VBUS isn't above 5V(0x%02X)", vbvolt);
...
int max77729_select_pps(int num, int ppsVol, int ppsCur)
{
	...
	if (ppsVol > pd_data->pd_noti.sink_status.power_list[num].max_voltage) {
		...
		pr_info("ppsVol is under(%d, min:%d)",
	...
	pr_info("PPS PDO(%d), voltage(%d), current(%d), is selected to change",
...
void max77729_pd_retry_work(struct work_struct *work)
{
	...
	pr_info("latest selected_pdo_num(%d)", num);
	g_usbc_data->pn_flag = false;
...
void max77729_usbc_icurr(u8 curr)
{
	...
	pr_info("OPCODE(0x%02x) W_LENGTH(%d) R_LENGTH(%d) USBC_ILIM(0x%x)",
...
static void max77729_set_snkcap(struct max77729_usbc_platform_data *usbc_data)
{
	...
	if (!np) {
		pr_info("max77729: device node not found");
		return;
	}

	if (!of_get_property(np, "max77729,snkcap_data", &len_bytes) || len_bytes <= 0) {
		pr_info("max77729: max77729,snkcap_data is empty or missing");
		of_node_put(np);
		return;
	}

	elems = len_bytes / sizeof(u8);
	if (elems <= 0) {
		pr_warn("max77729: snkcap_data length invalid: %d", len_bytes);
		of_node_put(np);
		return;
	}
	if (elems > MAX_SNKCAP_ELEMS) {
		pr_err("max77729: snkcap_data too large (%d > %d)", elems, MAX_SNKCAP_ELEMS);
		of_node_put(np);
		return;
	}

	snkcap_data = kcalloc(elems, sizeof(*snkcap_data), GFP_KERNEL);
	if (!snkcap_data) {
		pr_err("max77729: Failed to allocate memory for snkcap_data");
		of_node_put(np);
		return;
	}
	...
	ret = of_property_read_u8_array(np, "max77729,snkcap_data",
					snkcap_data, elems);
	if (ret) {
		pr_info("max77729: failed to read snkcap_data (%d)", ret);
		goto out_free;
	}
	...
	if (elems)
		memcpy(value.write_data, snkcap_data, elems);

	str = kcalloc(LOG_BUF_SIZE, sizeof(char), GFP_KERNEL);
	if (str) {
		p = str;
		remaining = LOG_BUF_SIZE;
		for (i = 0; i < elems; i++) {
			int written = scnprintf(p, remaining, "0x%02x ", value.write_data[i]);
			if (written >= remaining)
				break;
			p += written;
			remaining -= written;
		}
		pr_info("SNK_CAP: %s", str);
	} else {
		pr_warn("max77729: failed to allocate log buffer");
	}

	value.opcode = OPCODE_SET_SNKCAP;
	value.write_length = elems;
	value.read_length = 0;
...
void max77729_vbus_turn_on_ctrl(struct max77729_usbc_platform_data *usbc_data, bool enable, bool swaped)
{
	...
	while (count) {
		...
		if (psy_otg) {
			...
			if (ret == -ENODEV) {
				pr_err("fail to set power_suppy ONLINE property %d) retry (%d)", ret, count);
				count--;
			} else {
				if (ret) {
					pr_err("fail to set power_suppy ONLINE property(%d)", ret);
				} else {
					pr_info("otg accessory power: %d", on);
				}
				break;
			}
		} else {
			pr_err("fail to get psy otg");
			count--;
			msleep(200);
		}
	}
...
void max77729_pdo_list(struct max77729_usbc_platform_data *usbc_data, unsigned char *data)
{
	...
	if (temp > MAX_PDO_NUM) {
		pr_info("update available_pdo_num[%d -> %d]",
			temp, MAX_PDO_NUM);
		temp = MAX_PDO_NUM;
	}

	pd_data->pd_noti.sink_status.available_pdo_num = temp;
	pr_info("Temp[0x%02x] Data[0x%02x] available_pdo_num[%d]",
	...
	for (i = 0; i < temp; i++) {
		...
		pr_info("PDO_Num[%d] MAX_CURR(%d) MAX_VOLT(%d), AVAILABLE_PDO_Num(%d)",
	...
	if (usbc_data->pd_data->pdo_list && do_power_nego) {
		pr_info("PDO list is changed, so power negotiation is need");
	...
	if (pd_data->pd_noti.sink_status.current_pdo_num != pd_data->pd_noti.sink_status.selected_pdo_num) {
		if (pd_data->pd_noti.sink_status.selected_pdo_num == 0)
			pr_info("PDO is not selected yet by default");
	}
...
void max77729_current_pdo(struct max77729_usbc_platform_data *usbc_data, unsigned char *data)
{
	...
	pr_info("current_pdo_num(%d), available_pdo_num(%d/%d), comm(%d), suspend(%d)",
	...
	if (do_power_nego || pd_data->pd_noti.sink_status.selected_pdo_num == 0) {
		for (i = 0; i < num_of_pdo; ++i) {
			pdo_obj.data = (data[2 + (i * 4)]
				...
				| (data[4 + (i * 4)] << 16)
				| (data[5 + (i * 4)] << 24));
			pr_info("PDO[%d] = 0x%08X", i + 1, pdo_obj.data);
		...
		for (i = 0; i < pd_data->pd_noti.sink_status.available_pdo_num; ++i) {
			...
			pr_info("PDO[%d,%s,%s] max_vol(%dmV),min_vol(%dmV),max_cur(%dmA)",
				i + 1,
...
static void max77729_notify_prswap(struct max77729_usbc_platform_data *usbc_data, u8 pd_msg)
{
	...
	pr_info("PR SWAP pd_msg [%x]", pd_msg);
...
void max77729_notify_rp_current_level(struct max77729_usbc_platform_data *usbc_data)
{
	...
	if (usbc_data->plug_attach_done && !usbc_data->pd_data->psrdy_received &&
		...
		pr_debug("rp_currentlvl(%d)", pd_data->rp_currentlvl);
	}
...
static void max77729_pd_check_pdmsg(struct max77729_usbc_platform_data *usbc_data, u8 pd_msg)
{
	...
	case PRSWAP_SNKTOSWAP:
		...
		if (psy_charger) {
			...
			pr_err("Fail to get psy charger");
		}
#endif
		break;
	...
	case UFP:
		if (pd_data->current_dr != UFP) {
			...
			if (pd_data->previous_dr != 0xFF)
				msg_maxim("detach previous usb connection");
...
{
	//pr_err("IRQ(%d)_IN", irq);
	max77729_datarole_irq_handler(data, irq);
	//pr_err("IRQ(%d)_OUT", irq);
	return IRQ_HANDLED;
}
...
static void max77729_check_cc_sbu_short(void *data)
{
	...
	msg_maxim("cc_status1: %x, cc_sbu_short: %d", cc_status1, pd_data->cc_sbu_short);
}
...
int max77729_pd_init(struct max77729_usbc_platform_data *usbc_data)
{
	...
	if (pd_data->irq_pdmsg) {
		...
		if (ret) {
			pr_err("Failed to Request IRQ (%d)", ret);
			goto err_irq;
		}
	}
