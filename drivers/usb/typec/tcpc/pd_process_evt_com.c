...
static bool pd_evaluate_reject_dr_swap(struct pd_port *pd_port)
{
...
static bool pd_evaluate_reject_pr_swap(struct pd_port *pd_port)
{
...
static bool pd_evaluate_accept_vconn_swap(struct pd_port *pd_port)
{
...
static bool pd_process_ctrl_msg_dr_swap(struct pd_port *pd_port,
...
static bool pd_process_ctrl_msg_pr_swap(struct pd_port *pd_port,
					struct pd_event *pd_event)
{
#ifdef CONFIG_USB_PD_PR_SWAP
...
static bool pd_process_ctrl_msg_vconn_swap(struct pd_port *pd_port,
					   struct pd_event *pd_event)
{
#ifdef CONFIG_USB_PD_VCONN_SWAP
	if (!pd_check_pe_state_ready(pd_port))
...
static bool pd_process_data_msg_bist(struct pd_port *pd_port,
				     struct pd_event *pd_event)
{
...
static bool pd_process_ctrl_msg_wait(struct pd_port *pd_port)
{
				...
				struct pd_event *pd_event)
{
	...
	case PD_CTRL_GOOD_CRC:
		if (pd_port->pe_data.pe_state_flags & PE_STATE_FLAG_ENABLE_SENDER_RESPONSE_TIMER)
			pd_enable_timer(pd_port, PD_TIMER_SENDER_RESPONSE);

		if (pd_port->pe_data.pe_state_flags2 & PE_STATE_FLAG_BACK_READY_IF_RECV_GOOD_CRC) {
			pe_transit_ready_state(pd_port);
			return true;
		}
		break;
	...
	case PD_CTRL_REJECT:
		...
		if (pd_port->pe_data.pe_state_flags & PE_STATE_FLAG_BACK_READY_IF_RECV_REJECT)
			return pd_process_ctrl_msg_wait_reject(pd_port);
	...
	case PD_CTRL_WAIT:
		...
		if (pd_port->pe_data.pe_state_flags & PE_STATE_FLAG_BACK_READY_IF_RECV_WAIT)
			return pd_process_ctrl_msg_wait(pd_port);

		break;

	case PD_CTRL_SOFT_RESET:
		if (!pd_port->pe_data.during_swap && !pd_check_pe_during_hard_reset(pd_port)) {
	...
	case PD_CTRL_NOT_SUPPORTED:
		...
		if (pd_port->pe_data.pe_state_flags & PE_STATE_FLAG_BACK_READY_IF_SR_TIMER_TOUT) {
			pe_transit_ready_state(pd_port);
			return true;
...
{
	bool ret = false;
	u8 ready_state = pe_get_curr_ready_state(pd_port);
...
#ifdef CONFIG_USB_PD_REV30
static bool pd_process_ext_msg(struct pd_port *pd_port,
			       struct pd_event *pd_event)
{
	bool ret = false;
	u8 ready_state = pe_get_curr_ready_state(pd_port);
			       ...
			       struct pd_event *pd_event)
{
	...
	switch (pd_event->msg) {
	case PD_DPM_ACK:
		if (pd_port->pe_data.pe_state_flags2 & PE_STATE_FLAG_BACK_READY_IF_DPM_ACK) {
			pe_transit_ready_state(pd_port);
			return true;
		}
		break;
...
static bool pd_process_recv_hard_reset(struct pd_port *pd_port,
				       struct pd_event *pd_event)
{
...
static bool pd_process_hw_msg_tx_failed(struct pd_port *pd_port,
					struct pd_event *pd_event)
{
#ifdef CONFIG_USB_PD_RENEGOTIATION_COUNTER
	struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;
	...
	if (pd_port->pe_data.pe_state_flags & PE_STATE_FLAG_BACK_READY_IF_TX_FAILED) {
		...
		pe_transit_ready_state(pd_port);
		return true;
	} else if (pd_port->pe_data.pe_state_flags & PE_STATE_FLAG_HRESET_IF_TX_FAILED) {
		pe_transit_hard_reset_state(pd_port);
		return true;
	}
...
static bool pd_check_rx_pending(struct pd_port *pd_port)
{
	u32 alert;
	struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;
...
bool pd_process_event_com(struct pd_port *pd_port,
			  struct pd_event *pd_event)
{
