...
{
	u8 vconn_state = pd_port->vconn_role ? PE_VCS_WAIT_FOR_VCONN : PE_VCS_TURN_ON_VCONN;
	...
	case PD_CTRL_PS_RDY:
		if (PE_MAKE_STATE_TRANSIT_SINGLE(PE_VCS_WAIT_FOR_VCONN, PE_VCS_TURN_OFF_VCONN))
			return true;
		break;
	}
			       ...
			       struct pd_event *pd_event)
{
	...
	case PD_DPM_NAK:
		if (PE_MAKE_STATE_TRANSIT_SINGLE(PE_VCS_EVALUATE_SWAP, PE_VCS_REJECT_VCONN_SWAP))
			return true;
		break;
	}
...
bool pd_process_event_vcs(struct pd_port *pd_port,
			  struct pd_event *pd_event)
{
