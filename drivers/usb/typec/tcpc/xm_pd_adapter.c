...
static int pd_get_svid(struct adapter_device *dev)
{
	...
	if (info->adapter_svid_list == NULL) {
		if (in_interrupt()) {
			info->adapter_svid_list = kzalloc(sizeof(struct tcpm_svid_list), GFP_ATOMIC);
		} else {
			info->adapter_svid_list = kzalloc(sizeof(struct tcpm_svid_list), GFP_KERNEL);
		}
		if (info->adapter_svid_list == NULL)
	...
	unsigned int data_len)
{
	...
	if (in_interrupt()) {
		...
		vdm_data = kzalloc(sizeof(*vdm_data), GFP_ATOMIC);
		adapter_info("%s: kzalloc atomic ok.\n", __func__);
	} else {
		int_data = kzalloc(40, GFP_KERNEL);
		vdm_data = kzalloc(sizeof(*vdm_data), GFP_KERNEL);
		adapter_info("%s: kzalloc kernel ok.\n", __func__);
	}
