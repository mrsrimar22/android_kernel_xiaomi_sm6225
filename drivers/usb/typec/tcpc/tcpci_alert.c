...
static int tcpci_alert_vsafe0v(struct tcpc_device *tcpc)
{
...
static void tcpci_vbus_level_init_v10(struct tcpc_device *tcpc,
				      u16 power_status)
{
...
{
	tcpc->vbus_level = tcpc->vbus_present ?
			   TCPC_VBUS_VALID : TCPC_VBUS_INVALID;
	if (tcpc->vbus_safe0v) {
		if (tcpc->vbus_level == TCPC_VBUS_INVALID)
			tcpc->vbus_level = TCPC_VBUS_SAFE0V;
...
static void tcpci_vbus_level_refresh(struct tcpc_device *tcpc)
{
	mutex_lock(&tcpc->access_lock);
...
{
	u8 tx_state = PD_TX_STATE_GOOD_CRC;
	struct pd_event evt = {
...
static int tcpci_alert_fault(struct tcpc_device *tcpc)
{
	...
	if (tcpci_get_fault_status(tcpc, &fault_status) < 0)
		return 0;
...
#ifdef CONFIG_USB_POWER_DELIVERY
static bool tcpci_check_hard_reset_complete(struct tcpc_device *tcpc, u32 alert_status)
{
...
{
	int rv = 0, i = 0;
	u32 alert_status = 0, alert_mask = 0;
	const u8 typec_role = tcpc->typec_role;
...
int tcpci_alert_init(struct tcpc_device *tcpc)
{
	tcpc->alert_wake_lock = wakeup_source_register(&tcpc->dev, "tcpc_alert_wake_lock");
	if (IS_ERR_OR_NULL(tcpc->alert_wake_lock)) {
		dev_err(&tcpc->dev, "%s: register alert wake lock fail\n", __func__);
		return IS_ERR(tcpc->alert_wake_lock) ?
		       PTR_ERR(tcpc->alert_wake_lock) : -ENOMEM;
	}

	return 0;
}

void tcpci_alert_deinit(struct tcpc_device *tcpc)
{
	if (!IS_ERR_OR_NULL(tcpc->alert_wake_lock))
		wakeup_source_unregister(tcpc->alert_wake_lock);
}

static int tcpci_set_wake_lock_pd(struct tcpc_device *tcpc, int bit_flag, bool pd_lock)
{
	unsigned long flags_old, flags_new;

	mutex_lock(&tcpc->access_lock);

	flags_old = tcpc->wake_lock_flags;

	if (pd_lock)
		set_bit(bit_flag, &tcpc->wake_lock_flags);
	else
		clear_bit(bit_flag, &tcpc->wake_lock_flags);

	flags_new = tcpc->wake_lock_flags;

	if (flags_old == 0 && flags_new > 0) {
		TCPC_DBG("wake_lock=1\n");
		__pm_wakeup_event(tcpc->alert_wake_lock,
				  CONFIG_TCPC_ATTACH_WAKE_LOCK_TOUT);
		if (tcpc->typec_watchdog)
			tcpci_set_intrst(tcpc, true);
	} else if (flags_old > 0 && flags_new == 0) {
		TCPC_DBG("wake_lock=0\n");
		if (tcpc->typec_watchdog)
			tcpci_set_intrst(tcpc, false);
		__pm_wakeup_event(tcpc->alert_wake_lock,
				  CONFIG_TCPC_DETACH_WAKE_LOCK_TOUT);
	}
...
static int tcpci_report_usb_port_attached(struct tcpc_device *tcpc)
{
	...
	tcpci_set_wake_lock_pd(tcpc, TCPC_WAKE_LOCK_ATTACH, true);
...
static int tcpci_report_usb_port_detached(struct tcpc_device *tcpc)
{
	...
	tcpci_set_wake_lock_pd(tcpc, TCPC_WAKE_LOCK_ATTACH, false);
...
static int tcpci_report_power_control_on(struct tcpc_device *tcpc)
{
	tcpci_set_wake_lock_pd(tcpc, TCPC_WAKE_LOCK_POWER, true);
...
static int tcpci_report_power_control_off(struct tcpc_device *tcpc)
{
	mutex_lock(&tcpc->access_lock);
	...
	tcpci_set_wake_lock_pd(tcpc, TCPC_WAKE_LOCK_POWER, false);
