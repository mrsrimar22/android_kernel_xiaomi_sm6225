...
static void typec_wait_ps_change(struct tcpc_device *tcpc,
				 enum TYPEC_WAIT_PS_STATE state)
{
#if TYPEC_INFO2_ENABLE
	u8 old_state = tcpc->typec_wait_ps_change;
	...
	tcpc->typec_wait_ps_change = (u8)state;
}

/* #define TYPEC_EXIT_ATTACHED_SRC_NO_DEBOUNCE */
#define TYPEC_EXIT_ATTACHED_SNK_VIA_VBUS

static int typec_enable_low_power_mode(struct tcpc_device *tcpc, u8 pull);
...
static int typec_enable_vconn(struct tcpc_device *tcpc)
{
...
static void typec_transfer_state(struct tcpc_device *tcpc,
...
static int typec_alert_attach_state_change(struct tcpc_device *tcpc)
{
	...
	TYPEC_INFO("Attached-> %s\n",
		   (tcpc->typec_attach_new < ARRAY_SIZE(typec_attach_name)) ?
		   typec_attach_name[tcpc->typec_attach_new] : "??");
...
static int __maybe_unused typec_set_drp_toggling(struct tcpc_device *tcpc)
{
	int ret;
...
static int typec_norp_src_attached_entry(struct tcpc_device *tcpc)
{
#ifdef CONFIG_WATER_DETECTION
...
static int typec_try_low_power_mode(struct tcpc_device *tcpc)
{
...
static int typec_enter_low_power_mode(struct tcpc_device *tcpc)
{
	int ret = 0;
...
static int typec_disable_low_power_mode(struct tcpc_device *tcpc)
{
	int ret = 0;
...
static void typec_unattached_src_and_drp_entry(struct tcpc_device *tcpc)
{
...
static void typec_unattached_snk_and_drp_entry(struct tcpc_device *tcpc)
{
...
static void typec_unattached_cc_entry(struct tcpc_device *tcpc)
{
#ifdef CONFIG_TYPEC_CAP_ROLE_SWAP
...
static void typec_cc_open_entry(struct tcpc_device *tcpc, u8 state)
{
	typec_attach_new_unattached(tcpc);
...
static void typec_error_recovery_entry(struct tcpc_device *tcpc)
{
...
static void typec_disable_entry(struct tcpc_device *tcpc)
{
...
static int typec_set_polarity(struct tcpc_device *tcpc,
			      bool polarity)
{
...
static int typec_set_plug_orient(struct tcpc_device *tcpc,
				 u8 pull, bool polarity)
{
...
static void typec_source_attached_entry(struct tcpc_device *tcpc)
{
...
static void typec_sink_attached_entry(struct tcpc_device *tcpc)
{
...
static void typec_custom_src_attached_entry(struct tcpc_device *tcpc)
{
#ifdef CONFIG_TYPEC_CAP_DBGACC_SNK
...
static u8 typec_get_sink_dbg_acc_rp_level(int cc1, int cc2)
{
...
static void typec_sink_dbg_acc_attached_entry(struct tcpc_device *tcpc)
{
	bool polarity;
	u8 rp_level;

	u8 cc1 = typec_get_cc1();
	u8 cc2 = typec_get_cc2();
...
static bool typec_role_is_try_src(struct tcpc_device *tcpc)
{
...
static void typec_try_src_entry(struct tcpc_device *tcpc)
{
...
static void typec_trywait_snk_entry(struct tcpc_device *tcpc)
{
...
static void typec_trywait_snk_pe_entry(struct tcpc_device *tcpc)
{
	typec_attach_new_unattached(tcpc);
...
static bool typec_role_is_try_sink(struct tcpc_device *tcpc)
{
...
static void typec_try_snk_entry(struct tcpc_device *tcpc)
{
...
static void typec_trywait_src_entry(struct tcpc_device *tcpc)
{
...
static void typec_cc_snk_detect_vsafe5v_entry(struct tcpc_device *tcpc)
{
	typec_wait_ps_change(tcpc, TYPEC_WAIT_PS_DISABLE);
...
static void typec_cc_snk_detect_entry(struct tcpc_device *tcpc)
{
...
static void typec_cc_src_detect_vsafe0v_entry(struct tcpc_device *tcpc)
{
	typec_wait_ps_change(tcpc, TYPEC_WAIT_PS_DISABLE);
...
static void typec_cc_src_detect_entry(struct tcpc_device *tcpc)
{
...
static void typec_cc_src_remove_entry(struct tcpc_device *tcpc)
{
	typec_wait_ps_change(tcpc, TYPEC_WAIT_PS_DISABLE);
...
static void typec_cc_snk_remove_entry(struct tcpc_device *tcpc)
{
	typec_wait_ps_change(tcpc, TYPEC_WAIT_PS_DISABLE);
...
static void typec_legacy_reset_cable_suspect(struct tcpc_device *tcpc)
{
...
static void typec_legacy_reset_retry_wk(struct tcpc_device *tcpc)
{
...
static bool typec_legacy_charge(struct tcpc_device *tcpc)
{
	int i, vbus_level = 0;

	TYPEC_INFO("LC->Charge\n");
	tcpci_source_vbus(tcpc, TCP_VBUS_CTRL_TYPEC,
			  TCPC_VBUS_SOURCE_5V, 100);
...
static bool typec_legacy_discharge(struct tcpc_device *tcpc)
{
	int i, vbus_level = 0;
...
static bool typec_legacy_stable1(struct tcpc_device *tcpc)
{
...
{
	bool run_legacy2;
	u8 retry_max = TCPC_LEGACY_CABLE_RETRY_SOLUTION;
...
static bool typec_legacy_stable2(struct tcpc_device *tcpc)
{
...
static bool typec_legacy_confirm(struct tcpc_device *tcpc)
{
...
static bool typec_legacy_check_cable(struct tcpc_device *tcpc)
{
...
static void typec_legacy_reset_timer(struct tcpc_device *tcpc)
{
#ifdef CONFIG_TYPEC_CHECK_LEGACY_CABLE2
	if (tcpc->typec_legacy_cable == 2)
...
static void typec_legacy_handle_ps_change(struct tcpc_device *tcpc, int vbus_level)
{
#ifdef CONFIG_TYPEC_CHECK_LEGACY_CABLE2
...
static void typec_legacy_handle_detach(struct tcpc_device *tcpc)
{
#if TCPC_LEGACY_CABLE_SUSPECT_THD
...
static int typec_legacy_handle_cc_present(struct tcpc_device *tcpc)
{
#ifdef CONFIG_TYPEC_CHECK_LEGACY_CABLE2
...
static int typec_legacy_handle_cc_change(struct tcpc_device *tcpc)
{
	int ret = 0;
...
static void typec_debug_acc_attached_entry(struct tcpc_device *tcpc)
{
#ifdef CONFIG_TYPEC_CAP_DBGACC
...
static bool typec_audio_acc_sink_vbus(struct tcpc_device *tcpc, bool vbus_valid)
{
	if (vbus_valid) {
		tcpci_report_power_control(tcpc, true);
		tcpci_sink_vbus(tcpc, TCP_VBUS_CTRL_TYPEC,
...
static bool typec_audio_acc_attached_entry(struct tcpc_device *tcpc)
{
...
static bool typec_cc_change_source_entry(struct tcpc_device *tcpc)
{
...
static bool typec_attached_snk_cc_change(struct tcpc_device *tcpc)
{
	u8 cc_res = typec_get_cc_res();
...
static bool typec_cc_change_sink_entry(struct tcpc_device *tcpc)
{
...
static bool typec_handle_cc_changed_entry(struct tcpc_device *tcpc)
{
...
static void typec_attach_wait_entry(struct tcpc_device *tcpc)
{
	bool as_sink;
#ifdef CONFIG_USB_POWER_DELIVERY
	...
	case typec_unattachwait_pe:
		...
		WRITE_ONCE(tcpc->pd_wait_pe_idle, false);
...
static int typec_attached_snk_cc_detach(struct tcpc_device *tcpc)
{
	tcpc_reset_typec_debounce_timer(tcpc);
#ifdef CONFIG_USB_POWER_DELIVERY
	/*
...
static void typec_detach_wait_entry(struct tcpc_device *tcpc)
{
#ifdef CONFIG_TYPEC_CHECK_LEGACY_CABLE
...
static bool typec_is_cc_attach(struct tcpc_device *tcpc)
{
...
static bool typec_check_false_ra_detach(struct tcpc_device *tcpc)
{
...
EXPORT_SYMBOL(tcpc_typec_enter_lpm_again);

#ifdef CONFIG_TYPEC_CAP_TRY_SINK
static int typec_handle_try_sink_cc_change(struct tcpc_device *tcpc)
{
	/*
...
static int typec_get_rp_present_flag(struct tcpc_device *tcpc)
{
	u8 rp_flag = 0;
...
static bool typec_is_ignore_cc_change(struct tcpc_device *tcpc, u8 rp_present)
{
...
#ifdef CONFIG_TYPEC_CAP_TRY_STATE
static int typec_handle_drp_try_timeout(struct tcpc_device *tcpc)
{
...
static int typec_handle_debounce_timeout(struct tcpc_device *tcpc)
{
#ifdef CONFIG_TYPEC_CAP_NORP_SRC
...
static int typec_handle_error_recovery_timeout(struct tcpc_device *tcpc)
{
#ifdef CONFIG_USB_POWER_DELIVERY
	WRITE_ONCE(tcpc->pd_wait_pe_idle, false);
#endif	/* CONFIG_USB_POWER_DELIVERY */
...
#ifdef CONFIG_USB_POWER_DELIVERY
static int typec_handle_pe_idle(struct tcpc_device *tcpc)
{
	switch (tcpc->typec_state) {
#ifdef CONFIG_TYPEC_CAP_TRY_SOURCE
...
static void typec_handle_pd_wait_bc12(struct tcpc_device *tcpc)
{
	int ret = 0;
	u8 type = TYPEC_UNATTACHED;
	...
	TYPEC_INFO("type=%d, ret=%d, chg_type=%d, count=%d\n",
		   type, ret, val.intval, tcpc->pd_wait_bc12_count);

	if (type != TYPEC_ATTACHED_SNK &&
	    type != TYPEC_ATTACHED_DBGACC_SNK)
		goto out;
...
static int typec_handle_src_reach_vsafe0v(struct tcpc_device *tcpc)
{
	if (typec_is_drp_toggling()) {
		TYPEC_DBG("[Warning] DRP Toggling\n");
		return 0;
...
static int typec_handle_src_toggle_timeout(struct tcpc_device *tcpc)
{
#ifdef CONFIG_TYPEC_CAP_ROLE_SWAP
	if (tcpc->typec_during_role_swap)
		return 0;
...
#ifdef CONFIG_TYPEC_CAP_ROLE_SWAP
static int typec_handle_role_swap_start(struct tcpc_device *tcpc)
{
	u8 role_swap = tcpc->typec_during_role_swap;
...
static int typec_handle_role_swap_stop(struct tcpc_device *tcpc)
{
	if (tcpc->typec_during_role_swap) {
...
int tcpc_typec_handle_timeout(struct tcpc_device *tcpc, u32 timer_id)
{
	int ret = 0;
	...
	case TYPEC_RT_TIMER_LEGACY_STABLE:
		if (tcpc->typec_legacy_cable && tcpc->typec_legacy_retry_wk > 0)
			tcpc->typec_legacy_retry_wk--;
		break;
...
static int typec_handle_vbus_present(struct tcpc_device *tcpc)
{
...
static int typec_attached_snk_vbus_absent(struct tcpc_device *tcpc)
{
#ifdef TYPEC_EXIT_ATTACHED_SNK_VIA_VBUS
#ifdef CONFIG_USB_POWER_DELIVERY
...
static int typec_handle_vbus_absent(struct tcpc_device *tcpc)
{
#ifdef CONFIG_USB_POWER_DELIVERY
	if (tcpc->pd_wait_pr_swap_complete) {
...
int tcpc_typec_handle_ps_change(struct tcpc_device *tcpc, int vbus_level)
{
	...
	if (tcpc->typec_state == typec_audioaccessory) {
		return typec_audio_acc_sink_vbus(tcpc,
			vbus_level >= TCPC_VBUS_VALID);
	}
#endif	/* CONFIG_TYPEC_CAP_AUDIO_ACC_SINK_VBUS */
...
int tcpc_typec_set_rp_level(struct tcpc_device *tcpc, u8 rp_lvl)
{
	switch (rp_lvl) {
	case TYPEC_RP_DFT:
	case TYPEC_RP_1_5:
...
int tcpc_typec_change_role(struct tcpc_device *tcpc,
			   u8 typec_role, bool postpone)
{
