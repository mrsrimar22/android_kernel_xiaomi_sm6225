...
static u64 tcpc_get_timer_tick(struct tcpc_device *tcpc)
{
	u64 tick;
	unsigned long flags;
...
static u64 tcpc_get_and_clear_all_timer_tick(struct tcpc_device *tcpc)
{
	u64 tick;
	unsigned long flags;
...
#define DECL_TCPC_TIMEOUT_RANGE(name, min, max)	\
		{ #name, TIMEOUT_RANGE(min, max) }
#define DECL_TCPC_TIMEOUT(name, ms)	\
		{ #name, TIMEOUT_VAL(ms) }
...
#endif	/* CONFIG_USB_POWER_DELIVERY */
DECL_TCPC_TIMEOUT_RANGE(TYPEC_TIMER_ERROR_RECOVERY, 25, 50),
...
DECL_TCPC_TIMEOUT_RANGE(TYPEC_TIMER_PDDEBOUNCE, 10, 15),
...
#ifdef CONFIG_USB_POWER_DELIVERY
static void on_pe_timer_timeout(struct tcpc_device *tcpc, u32 timer_id)
{
...
static void wake_up_work_func(struct kthread_work *work)
{
	struct tcpc_device *tcpc = container_of(work,
			struct tcpc_device, wake_up_work);
...
{
	struct tcpc_device *tcpc = container_of(alarm,
			struct tcpc_device, wake_up_timer);

	pm_wakeup_ws_event(tcpc->wakeup_wake_lock, 1000, true);
	kthread_queue_work(tcpc->wake_up_worker, &tcpc->wake_up_work);
	return ALARMTIMER_NORESTART;
}

static enum hrtimer_restart tcpc_timer_call(struct hrtimer *timer)
{
	struct tcpc_timer *tcpc_timer = container_of(timer,
			struct tcpc_timer, timer);
...
void tcpc_disable_timer(struct tcpc_device *tcpc, u32 timer_id)
{
	if (timer_id >= PD_TIMER_NR) {
		PD_WARN_ON(1);
		return;
...
{
	int i = 0;
	u64 tick = tcpc_get_and_clear_all_timer_tick(tcpc);
...
static int tcpc_timer_thread_fn(void *data)
{
	...
	while (true) {
		ret = wait_event_timeout(tcpc->timer_wait_que,
...
int tcpci_timer_init(struct tcpc_device *tcpc)
{
	...
	dev_info(&tcpc->dev, "%s: Timer Num = %d\n", __func__, PD_TIMER_NR);
	...
	if (IS_ERR_OR_NULL(tcpc->wakeup_wake_lock)) {
		dev_err(&tcpc->dev, "%s: register wakeup wake lock fail\n", __func__);
		ret = IS_ERR(tcpc->wakeup_wake_lock) ?
		      PTR_ERR(tcpc->wakeup_wake_lock) : -ENOMEM;
		goto err_unregister_wake_lock;
	}

	tcpc->wake_up_worker = kthread_create_worker(0, "tcpc_wake_%s", tcpc->desc.name);
	if (IS_ERR(tcpc->wake_up_worker)) {
		ret = PTR_ERR(tcpc->wake_up_worker);
		dev_err(&tcpc->dev, "%s: create wake worker fail(%d)\n", __func__, ret);
		goto err_unregister_wake_lock;
	}
	sched_set_fifo(tcpc->wake_up_worker->task);
	kthread_init_work(&tcpc->wake_up_work, wake_up_work_func);

	alarm_init(&tcpc->wake_up_timer, ALARM_BOOTTIME, tcpc_timer_wakeup);

	tcpc->timer_task = kthread_run(tcpc_timer_thread_fn, tcpc,
				       "tcpc_timer_%s", tcpc->desc.name);
	if (IS_ERR(tcpc->timer_task)) {
		ret = PTR_ERR(tcpc->timer_task);
		dev_err(&tcpc->dev, "%s: create timer thread fail(%d)\n", __func__, ret);
		goto err_destroy_wake_worker;
	}

	dev_info(&tcpc->dev, "%s: OK\n", __func__);
	return 0;

err_destroy_wake_worker:
	if (tcpc->wake_up_worker)
		kthread_destroy_worker(tcpc->wake_up_worker);
