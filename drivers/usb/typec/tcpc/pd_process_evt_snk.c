...
static bool pd_process_ctrl_msg_get_source_cap(struct pd_port *pd_port, u8 next)
{
...
static bool pd_process_ctrl_msg(struct pd_port *pd_port, struct pd_event *pd_event)
{
	...
	case PD_CTRL_NOT_SUPPORTED:
		if (PE_MAKE_STATE_TRANSIT_SINGLE(PE_SNK_READY, PE_SNK_NOT_SUPPORTED_RECEIVED))
			return true;
		break;

#ifdef CONFIG_USB_PD_REV30_SRC_CAP_EXT_LOCAL
	case PD_CTRL_GET_SOURCE_CAP_EXT:
		if (pd_process_ctrl_msg_get_source_cap(pd_port, PE_DR_SNK_GIVE_SOURCE_CAP_EXT))
			return true;
		break;
...
static bool pd_process_data_msg(struct pd_port *pd_port, struct pd_event *pd_event)
{
	switch (pd_event->msg) {
	...
	case PD_DATA_ALERT:
		if (PE_MAKE_STATE_TRANSIT_SINGLE(PE_SNK_READY, PE_SNK_SOURCE_ALERT_RECEIVED))
			return true;
		break;
 ...
 */
#ifdef CONFIG_USB_PD_REV30
static bool pd_process_ext_msg(struct pd_port *pd_port, struct pd_event *pd_event)
{
	switch (pd_event->msg) {
...
static bool pd_process_dpm_msg(struct pd_port *pd_port, struct pd_event *pd_event)
{
	switch (pd_event->msg) {
	case PD_DPM_ACK:
...
static bool pd_process_hw_msg_sink_tx_change(struct pd_port *pd_port,
...
static bool pd_process_vbus_absent(struct pd_port *pd_port)
{
...
static void pd_report_typec_only_charger(struct pd_port *pd_port)
{
	u8 state;
