...
{
	int i;
	const struct pe_state_transition *state_transition = state_reaction->state_transition;
...
{
	bool power_change = false, ret = false;
#if PE_INFO_ENABLE
	uint8_t __maybe_unused event_type = pd_event->event_type;
	uint8_t __maybe_unused msg_id = pd_get_msg_hdr_id(pd_port);
	uint8_t __maybe_unused msg_type = pd_event->msg;
#endif
	struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;
