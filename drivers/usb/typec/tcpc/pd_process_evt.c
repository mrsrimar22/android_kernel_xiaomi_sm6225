...
static void print_ctrl_msg_event(struct tcpc_device *tcpc, u8 msg)
{
...
static void print_data_msg_event(struct tcpc_device *tcpc, u8 msg)
{
...
static void print_ext_msg_event(struct tcpc_device *tcpc, u8 msg)
{
...
static void print_hw_msg_event(struct tcpc_device *tcpc, u8 msg)
{
	if (msg < PD_HW_MSG_NR)
		PE_EVT_INFO("%s\n", pd_hw_msg_name[msg]);
}
...
static void print_pe_msg_event(struct tcpc_device *tcpc, u8 msg)
{
	if (msg < PD_PE_MSG_NR)
		PE_EVT_INFO("%s\n", pd_pe_msg_name[msg]);
}
...
static void print_dpm_msg_event(struct tcpc_device *tcpc, u8 msg)
{
...
static void print_tcp_event(struct tcpc_device *tcpc, u8 msg)
{
...
static void print_event(struct pd_port *pd_port, struct pd_event *pd_event)
{
#if PE_EVENT_DBG_ENABLE
...
static bool pd_process_ready_protocol_error(struct pd_port *pd_port)
{
#ifdef CONFIG_USB_PD_REV30
			       ...
			       struct pd_event *pd_event)
{
	...
	u8 msg_type = pd_event->msg;
	u8 msg_id = pd_get_msg_hdr_id(pd_port);
#endif
	struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;
...
bool pd_process_tx_failed(struct pd_port *pd_port)
{
	...
	if (pd_check_pe_state_ready(pd_port) || pd_check_pe_during_hard_reset(pd_port)) {
...
static bool pd_process_cable_ctrl_msg_accept(struct pd_port *pd_port,
					     struct pd_event *pd_event)
{
	switch (pd_port->pe_state_curr) {
...
static bool pd_process_event_cable(struct pd_port *pd_port,
				   struct pd_event *pd_event)
{
	bool ret = false;
	struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;
...
static void pd_copy_msg_data(struct pd_port *pd_port,
			     u8 *payload, u16 count, u8 unit_sz)
{
...
#ifdef CONFIG_USB_PD_REV30
static void pd_copy_msg_data_from_ext_evt(struct pd_port *pd_port,
					  struct pd_msg *pd_msg)
{
	u32 *payload = pd_msg->payload;

	u16 *ext_hdr = (u16 *)payload;
	u8 *ext_data = (u8 *)(ext_hdr + 1);

	u16 size = PD_EXT_HEADER_DATA_SIZE(*ext_hdr);

	pd_copy_msg_data(pd_port, ext_data, size, 1);
}
#endif	/* CONFIG_USB_PD_REV30 */

static void pd_copy_msg_data_from_evt(struct pd_port *pd_port,
				      struct pd_event *pd_event)
{
	...
	case PD_EVT_DATA_MSG:
		PD_WARN_ON(!pd_msg);
		pd_copy_msg_data(pd_port, (u8 *)pd_msg->payload,
				 pd_get_msg_hdr_cnt(pd_port), sizeof(u32));
		break;
...
static bool pe_is_valid_pd_msg_id(struct pd_port *pd_port,
				  struct pd_event *pd_event,
				  struct pd_msg *pd_msg)
{
	...
	if (pd_event->event_type == PD_EVT_CTRL_MSG) {
		switch (pd_event->msg) {
		/* SoftReset always has a MessageID value of zero */
...
static bool pe_is_valid_pd_msg_role(struct pd_port *pd_port,
				    struct pd_event *pd_event,
				    struct pd_msg *pd_msg)
{
	bool ret = true;
...
static void pe_translate_pd_msg_event(struct pd_port *pd_port,
				      struct pd_event *pd_event,
				      struct pd_msg *pd_msg)
{
	u16 msg_hdr;
...
static u8 pe_get_startup_state(struct pd_port *pd_port,
			       struct pd_event *pd_event)
{
	bool act_as_sink = true;
	u8 startup_state = 0xff;
...
static bool pe_transit_startup_state(struct pd_port *pd_port,
				     struct pd_event *pd_event)
{
	u8 startup_state = pe_get_startup_state(pd_port, pd_event);
...
static u8 pe_check_trap_in_idle_state(struct pd_port *pd_port,
				      struct pd_event *pd_event)
{
	struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;
...
static void pe_init_curr_state(struct pd_port *pd_port)
{
	if (pd_port->power_role == PD_ROLE_SINK) {
		      ...
		      struct pd_event *pd_event)
{
	bool ret = false;
	struct pd_msg *pd_msg = pd_event->pd_msg;
	u8 tii = pe_check_trap_in_idle_state(pd_port, pd_event);
	...
	if (pd_event->event_type < PD_EVT_PD_MSG_END && pd_msg) {
	...
	if (pd_event->event_type == PD_EVT_CTRL_MSG &&
	    ...
	    pd_msg && pd_msg->frame_type != TCPC_TX_SOP)
