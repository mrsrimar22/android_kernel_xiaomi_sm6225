...
void crcbits(u32 data, u32 *crc, u32 *ppolynomial)
{
	u32 i, newbit, newword, rl_crc;
...
static u32 dc_get_random_code(void)
{
	u32 num;
...
static u32 dc_get_authorization_code(u32 data)
{
	u32 dwpolynomial = 0x04C11DB6, dwcrc = 0xFFFFFFFF;
...
static bool dc_dfp_send_en_unlock(struct pd_port *pd_port,
				  u32 cmd, u32 data0, u32 data1)
{
	pd_port->uvdm_cnt = 3;
	...
	if (pd_port->pe_data.dc_pps_mode) {
		pd_port->uvdm_data[0] = VDO_S(USB_VID_DIRECTCHARGE,
					      SVDM_REV20, CMDT_INIT,
...
void dc_dfp_set_state(struct pd_port *pd_port, u8 state)
{
...
bool dc_dfp_start_en_unlock1(struct pd_port *pd_port)
{
	...
	pd_port->dc_pass_code =
		dc_get_authorization_code((rn_code[0] & 0xffff) | (rn_code[1] & 0xffff0000));
...
{
	u32 rn_code = dc_get_random_code();
	struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;
...
static bool dc_dfp_notify_en_unlock1(struct pd_port *pd_port,
				     struct svdm_svid_data *svid_data, bool ack)
{
...
static bool dc_dfp_notify_en_unlock2(struct pd_port *pd_port,
				     struct svdm_svid_data *svid_data, bool ack)
{
			...
			struct svdm_svid_data *svid_data)
{
#ifdef RTDC_TA_EMULATE
	u32 reply_cmd[3];
	u32 recv_code[2], rn_code, pass_code;

	reply_cmd[0] = PD_UVDM_HDR(0x29cf, RTDC_UVDM_EN_UNLOCK);
	u32 cmd = PD_UVDM_HDR_CMD(pd_port->uvdm_data[0]);
	...
	case DC_UFP_T0:
		...
		pass_code =
			dc_get_authorization_code((recv_code[0] & 0xffff) | (recv_code[1] & 0xffff0000));
