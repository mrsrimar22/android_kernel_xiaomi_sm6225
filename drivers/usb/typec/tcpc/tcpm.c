...
{
	struct pd_port *pd_port = &tcpc->pd_port;
	u32 ret = PD_CABLE_CURR_UNKNOWN;

	mutex_lock(&pd_port->pd_lock);
	if (pd_port->pe_data.power_cable_present)
		ret = pd_get_cable_curr_lvl(pd_port);
...
static int __tcpm_inquire_select_source_cap(struct pd_port *pd_port,
				 ...
				 struct pd_country_codes *ccdb)
{
	...
	return tcpm_put_tcp_dpm_event_cbk2(tcpc, &tcp_event,
			cb_data, TCPM_BK_PD_CMD_TOUT,
			(u8 *)ccdb, PD_CCDB_MAX_SIZE);
}
EXPORT_SYMBOL(tcpm_dpm_pd_get_country_code);

int tcpm_dpm_pd_get_country_info(struct tcpc_device *tcpc, u32 ccdo,
				 const struct tcp_dpm_event_cb_data *cb_data,
			       ...
			       u32 *bsdo)
{
	...
	return tcpm_put_tcp_dpm_event_cbk2(tcpc, &tcp_event,
			cb_data, TCPM_BK_PD_CMD_TOUT,
			(u8 *) bsdo, sizeof(u32) * PD_BSDO_SIZE);
}
...
#ifdef CONFIG_USB_PD_TCPM_CB_2ND
static void tcpm_replace_curr_tcp_event(struct pd_port *pd_port,
					struct tcp_dpm_event *event)
{
...
static u16 tcpm_calc_battery_cap(struct pd_battery_info *battery_info, u16 soc)
{
	u16 wh;
...
static int tcpm_update_bat_status_wh_no_mutex(struct tcpc_device *tcpc,
					      enum pd_battery_reference ref, u8 status, u16 wh)
{
...
static int tcpm_update_bat_status_soc_no_mutex(struct tcpc_device *tcpc,
					       enum pd_battery_reference ref, u8 status, u16 soc)
{
...
static void tcpm_alert_status_changed(struct tcpc_device *tcpc, u8 ado_type)
{
#ifdef CONFIG_USB_PD_REV30_ALERT_LOCAL
				    ...
				    u8 bat_input, u8 bat_mask)
{
	u8 input;
	u8 ado_type = 0;
	const u8 flag = PD_STATUS_INPUT_INT_POWER_BAT;
	struct pd_port *pd_port = &tcpc->pd_port;
...
#ifdef CONFIG_USB_PD_TCPM_CB_2ND
static void tcpm_dpm_bk_copy_data(struct pd_port *pd_port)
{
	u8 size = pd_port->tcpm_bk_cb_data_max;

	if (size >= pd_get_msg_data_size(pd_port))
		size = pd_get_msg_data_size(pd_port);

	if (pd_port->tcpm_bk_cb_data)
		memcpy(pd_port->tcpm_bk_cb_data,
		       pd_get_msg_data_payload(pd_port), size);
}
#endif	/* CONFIG_USB_PD_TCPM_CB_2ND */

int tcpm_dpm_bk_event_cb(struct tcpc_device *tcpc,
...
static int __tcpm_dpm_wait_bk_event(struct pd_port *pd_port, u32 tout_ms)
{
...
static int tcpm_dpm_wait_bk_event(struct pd_port *pd_port, u32 tout_ms)
{
...
{
	/* Barrier: no waiter left before the tcpc core destroys the lock. */
	mutex_lock(&pd_port->tcpm_bk_lock);
	mutex_unlock(&pd_port->tcpm_bk_lock);
}
