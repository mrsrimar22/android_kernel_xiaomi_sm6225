...
static void dpm_extract_apdo_info(u32 pdo, struct dpm_pdo_info_t *info)
{
...
void dpm_extract_pdo_info(u32 pdo, struct dpm_pdo_info_t *info)
{
	...
	case PDO_TYPE_BATTERY:
		...
		info->ma = info->vmin ? (info->uw / info->vmin) : 0;
		break;
...
static int dpm_calc_src_cap_power_uw(struct dpm_pdo_info_t *source,
					...
					struct dpm_pdo_info_t *source)
{
	...
	if (sink->vmin != TCPC_VBUS_SINK_5V || sink->vmax != TCPC_VBUS_SINK_5V ||
	    source->vmin != TCPC_VBUS_SINK_5V || source->vmax != TCPC_VBUS_SINK_5V)
		return false;
...
static bool dpm_is_valid_pdo_pair(struct dpm_pdo_info_t *sink,
				  struct dpm_pdo_info_t *source, u32 policy)
{
	if (sink->vmin > source->vmin)
		return false;
...
bool dpm_find_match_req_info(struct dpm_rdo_info_t *req_info,
			     struct dpm_pdo_info_t *sink,
			     int cnt, u32 *src_pdos, int max_uw, u32 policy)
{
	int i;
