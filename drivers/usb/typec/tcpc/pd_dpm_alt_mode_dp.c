 ...
 */
static bool dp_update_dp_connected_one(struct pd_port *pd_port,
				       u32 dp_connected,
				       u32 dp_local_connected)
{
...
static bool dp_update_dp_connected_both(struct pd_port *pd_port,
					u32 dp_local_connected,
...
void dp_dfp_u_set_state(struct pd_port *pd_port, u8 state)
{
	struct dp_data *dp_data = pd_get_dp_data(pd_port);
...
static bool is_dp_v1_cap_valid(u32 dp_cap)
{
...
#define DP_RECEPTACLE	BIT(6)

/* priority : B -> A -> D/F -> C/E */
static int eval_dp_match_score(u32 local_mode, u32 remote_mode,
			       u32 *local_dp_config, u32 *remote_dp_config)
{
	u32 local_pin_assignment = 0, remote_pin_assignment = 0;
	u32 common_pin_assignment;
...
static u8 dp_dfp_u_select_mode(struct pd_port *pd_port,
			       struct dp_data *dp_data,
			       struct svdm_svid_data *svid_data)
{
	u32 dp_local_mode, dp_remote_mode;
	u32 remote_dp_config = 0, local_dp_config = 0;
				...
				u8 ops, bool ack)
{
	 ...
	 *	From : "USB Mode, USB Configuration"
	 *	To : "DisplayPlay Mode, USB Configuration"
	 *
...
static bool dp_dfp_u_select_pin_mode(struct pd_port *pd_port)
{
	u32 dp_local_connected;
	u32 dp_mode[2], pin_cap[2];
	u32 pin_caps, signal;
	struct dp_data *dp_data = pd_get_dp_data(pd_port);
	struct svdm_svid_data *svid_data =
...
static bool dp_dfp_u_update_dp_connected(struct pd_port *pd_port)
{
	bool valid_connected = false;
	u32 dp_connected, dp_local_connected;
	struct dp_data *dp_data = pd_get_dp_data(pd_port);
	struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;
	...
	case DPSTS_DFP_D_CONNECTED:
	case DPSTS_UFP_D_CONNECTED:
		valid_connected = dp_update_dp_connected_one(pd_port,
							     dp_connected,
							     dp_local_connected);
		if (!valid_connected)
...
static void dp_ufp_u_auto_update(struct pd_port *pd_port)
{
...
bool dp_dfp_u_notify_dp_configuration(struct pd_port *pd_port, bool ack)
{
	...
	tcpci_dp_notify_config_done(tcpc, dp_data->local_config, dp_data->remote_config, ack);
...
static void dp_ufp_u_set_state(struct pd_port *pd_port, u8 state)
{
	struct dp_data *dp_data = pd_get_dp_data(pd_port);
	struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;
...
static bool dp_ufp_u_update_dp_connected(struct pd_port *pd_port)
{
	bool valid_connected = false;
	u32 dp_connected, dp_local_connected;
	struct dp_data *dp_data = pd_get_dp_data(pd_port);
...
static int dp_ufp_u_request_dp_status(struct pd_port *pd_port)
{
	bool ack;
	struct dp_data *dp_data = pd_get_dp_data(pd_port);
	...
	if (ack)
		return pd_reply_svdm_request(pd_port, CMDT_RSP_ACK, 1, &dp_data->local_status);

	return dpm_vdm_reply_svdm_nak(pd_port);
}

bool dp_ufp_u_is_valid_dp_config(struct pd_port *pd_port, u32 dp_config)
{
	/* TODO: Check it later .... */
	u32 sel_config;
	bool retval = false;
	...
	u32 local_mode = svid_data->local_mode.mode_vdo[0];
	u32 remote_pin = PD_DP_CFG_PIN(dp_config);
...
static void dp_ufp_u_auto_attention(struct pd_port *pd_port)
{
...
static int dp_ufp_u_request_dp_config(struct pd_port *pd_port)
{
	bool ack = false;
	u32 dp_config;
	struct dp_data *dp_data = pd_get_dp_data(pd_port);
...
static void dp_ufp_u_send_dp_attention(struct pd_port *pd_port)
{
	struct svdm_svid_data *svid_data;
	struct dp_data *dp_data = pd_get_dp_data(pd_port);
