...
static void pd_postpone_vdm_event(struct tcpc_device *tcpc)
{
	/*
...
static void __pd_free_msg(struct tcpc_device *tcpc,
...
void pd_free_msg(struct tcpc_device *tcpc,
		 struct pd_msg *pd_msg)
{
	mutex_lock(&tcpc->access_lock);
...
void pd_free_event(struct tcpc_device *tcpc,
		   struct pd_event *pd_event)
{
	mutex_lock(&tcpc->access_lock);
...
bool pd_get_event(struct tcpc_device *tcpc,
		  struct pd_event *pd_event)
{
	bool ret;
...
static void pd_discard_pending_vdm_event(struct tcpc_device *tcpc)
{
...
static bool pd_get_attention_event(struct tcpc_device *tcpc,
				   struct pd_event *pd_event)
{
...
static bool pd_check_vdm_state_ready(struct pd_port *pd_port)
{
...
bool pd_get_vdm_event(struct tcpc_device *tcpc,
		      struct pd_event *pd_event)
{
...
static bool pd_reset_pe_vdm_state(struct tcpc_device *tcpc, u32 vdm_hdr)
{
...
static bool pd_is_init_attention_event(struct tcpc_device *tcpc,
				       struct pd_event *pd_event)
{
			       ...
			       const struct tcp_dpm_event *tcp_event)
{
	...
	if (!tcpc->pd_pe_running || READ_ONCE(tcpc->pd_wait_pe_idle)) {
...
static void __tcp_event_buf_reset(struct tcpc_device *tcpc, u8 reason)
{
	struct tcp_dpm_event tcp_event;
...
static void __pd_event_buf_reset(struct tcpc_device *tcpc, u8 reason)
{
	struct pd_event pd_event;
...
static bool __pd_put_hw_event(struct tcpc_device *tcpc, u8 hw_event)
{
	struct pd_event evt = {
...
static bool __pd_put_pe_event(struct tcpc_device *tcpc, u8 pe_event)
{
	struct pd_event evt = {
		.event_type = PD_EVT_PE_MSG,
		.msg = pe_event,
...
bool __pd_put_cc_attached_event(struct tcpc_device *tcpc, u8 type)
{
	struct pd_event evt = {
		.event_type = PD_EVT_HW_MSG,
...
bool pd_put_cc_attached_event(struct tcpc_device *tcpc, u8 type)
{
	bool ret = false;
...
void pd_put_recv_hard_reset_event(struct tcpc_device *tcpc)
{
	...
	if (!tcpc->pd_hard_reset_event_pending &&
	    !READ_ONCE(tcpc->pd_wait_pe_idle) &&
	    tcpc->pd_pe_running) {
...
bool pd_put_pd_msg_event(struct tcpc_device *tcpc,
			 struct pd_msg *pd_msg)
{
	u32 cnt, cmd, extend;
...
void pd_notify_pe_idle(struct pd_port *pd_port)
{
	...
	mutex_lock(&tcpc->access_lock);
	if (READ_ONCE(tcpc->pd_wait_pe_idle)) {
		notify_pe_idle = true;
		tcpc->pd_wait_pe_idle = false;
	}
...
static int tcpc_event_thread_fn(void *data)
{
	...
	while (true) {
		ret = wait_event_timeout(tcpc->event_wait_que,
...
int tcpci_event_init(struct tcpc_device *tcpc)
{
	...
	if (IS_ERR(tcpc->event_task)) {
		ret = PTR_ERR(tcpc->event_task);
		dev_notice(&tcpc->dev, "%s: create event thread fail(%d)\n", __func__, ret);
