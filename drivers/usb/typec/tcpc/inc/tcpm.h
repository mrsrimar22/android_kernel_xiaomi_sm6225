...
#define TCP_NOTIFY_TYPE_MODE	BIT(TCP_NOTIFY_IDX_MODE)
#define TCP_NOTIFY_TYPE_VBUS	BIT(TCP_NOTIFY_IDX_VBUS)
#define TCP_NOTIFY_TYPE_USB	BIT(TCP_NOTIFY_IDX_USB)
#define TCP_NOTIFY_TYPE_MISC	BIT(TCP_NOTIFY_IDX_MISC)
...
#define DPM_CAP_ATTEMPT_ENTER_DC_MODE		BIT(11)
#define DPM_CAP_ATTEMPT_DISCOVER_CABLE_DFP	BIT(12)
#define DPM_CAP_ATTEMPT_ENTER_DP_MODE		BIT(13)
#define DPM_CAP_ATTEMPT_DISCOVER_CABLE		BIT(14)
#define DPM_CAP_ATTEMPT_DISCOVER_ID		BIT(15)
#define DPM_CAP_ATTEMPT_DISCOVER_SVID		BIT(16)
...
#define DPM_CAP_PR_SWAP_REJECT_AS_SRC		BIT(20)
#define DPM_CAP_PR_SWAP_REJECT_AS_SNK		BIT(21)
#define DPM_CAP_PR_SWAP_CHECK_GP_SRC		BIT(22)
#define DPM_CAP_PR_SWAP_CHECK_GP_SNK		BIT(23)
...
typedef int (*tcp_dpm_event_cb)(struct tcpc_device *tcpc,
...
struct tcp_dpm_event {
	...
	union {
		...
		u32 index;
		u32 data_object[PD_DATA_OBJ_SIZE];
	} tcp_dpm_data;
};
...
struct tcpm_svid_list {
	u8 cnt;
	u16 svids[VDO_MAX_SVID_NR];
};

struct tcpm_mode_list {
	u8 cnt;
	u32 modes[VDO_MAX_NR];
};
...
#define PD_UVDM_HDR(vid, custom)	(((u32)(vid) << 16) | ((u32)(custom) & 0x7FFF))
#define PD_SVDM_HDR(vid, custom)	(((u32)(vid) << 16) | (1 << 15) | ((u32)(custom) & 0x1FDF))
...
int register_tcp_dev_notifier(struct tcpc_device *tcp_dev,
			      struct notifier_block *nb, u8 flags);
...
void tcpm_block_init(struct pd_port *pd_port);
void tcpm_block_deinit(struct pd_port *pd_port);
...
u8 tcpm_inquire_typec_attach_state(struct tcpc_device *tcpc);
u8 tcpm_inquire_typec_role(struct tcpc_device *tcpc);
u8 tcpm_inquire_typec_local_rp(struct tcpc_device *tcpc);

int tcpm_typec_set_usb_sink_curr(struct tcpc_device *tcpc, int curr);

int tcpm_typec_set_rp_level(struct tcpc_device *tcpc, u8 level);

int tcpm_typec_set_custom_hv(struct tcpc_device *tcpc, bool en);

int tcpm_typec_role_swap(struct tcpc_device *tcpc);

int tcpm_typec_change_role(struct tcpc_device *tcpc, u8 typec_role);
...
u8 tcpm_inquire_pd_data_role(struct tcpc_device *tcpc);

u8 tcpm_inquire_pd_power_role(struct tcpc_device *tcpc);

u8 tcpm_inquire_pd_state_curr(struct tcpc_device *tcpc_dev);

u8 tcpm_inquire_pd_vconn_role(struct tcpc_device *tcpc);

u8 tcpm_inquire_pd_pe_ready(struct tcpc_device *tcpc);

u32 tcpm_inquire_cable_current(struct tcpc_device *tcpc);

u32 tcpm_inquire_dpm_flags(struct tcpc_device *tcpc);

u32 tcpm_inquire_dpm_caps(struct tcpc_device *tcpc);

void tcpm_set_dpm_caps(struct tcpc_device *tcpc, u32 caps);

/* TCPM DPM PD I/F */

int tcpm_inquire_pd_contract(struct tcpc_device *tcpc, int *mv, int *ma);
int tcpm_inquire_cable_inform(struct tcpc_device *tcpc, u32 *vdos);
int tcpm_inquire_pd_partner_inform(struct tcpc_device *tcpc, u32 *vdos);
...
int tcpm_inquire_pd_partner_modes(struct tcpc_device *tcpc,
				  u16 svid, struct tcpm_mode_list *list);
...
bool tcpm_extract_power_cap_val(u32 pdo, struct tcpm_power_cap_val *cap);

bool tcpm_extract_power_cap_list(struct tcpm_power_cap *cap,
...
int tcpm_dpm_pd_power_swap(struct tcpc_device *tcpc,
			   u8 role, const struct tcp_dpm_event_cb_data *data);
...
int tcpm_dpm_pd_get_country_info(struct tcpc_device *tcpc,
				 u32 ccdo, const struct tcp_dpm_event_cb_data *data,
...
int tcpm_dpm_pd_get_bat_status(struct tcpc_device *tcpc,
			       struct pd_get_battery_status *gbsdb,
			       const struct tcp_dpm_event_cb_data *data, u32 *bsdo);
...
int tcpm_dpm_pd_alert(struct tcpc_device *tcpc,
		      u32 ado, const struct tcp_dpm_event_cb_data *data);
#endif	/* CONFIG_USB_PD_REV30 */
...
int tcpm_dpm_vdm_discover_mode(struct tcpc_device *tcpc,
			       u16 svid, const struct tcp_dpm_event_cb_data *data);
int tcpm_dpm_vdm_enter_mode(struct tcpc_device *tcpc,
			    u16 svid, u8 ops, const struct tcp_dpm_event_cb_data *data);
...
int tcpm_inquire_dp_ufp_u_state(struct tcpc_device *tcpc, u8 *state);

int tcpm_dpm_dp_attention(struct tcpc_device *tcpc,
			  u32 dp_status, u32 mask,
			  const struct tcp_dpm_event_cb_data *data);

#ifdef CONFIG_USB_PD_ALT_MODE_DFP

int tcpm_inquire_dp_dfp_u_state(struct tcpc_device *tcpc, u8 *state);
...
int tcpm_set_pd_charging_policy(struct tcpc_device *tcpc,
				u8 policy, const struct tcp_dpm_event_cb_data *data);

int tcpm_set_pd_charging_policy_default(struct tcpc_device *tcpc, u8 policy);

u8 tcpm_inquire_pd_charging_policy(struct tcpc_device *tcpc);
u8 tcpm_inquire_pd_charging_policy_default(struct tcpc_device *tcpc);
...
int tcpm_dpm_set_vconn_supply_mode(struct tcpc_device *tcpc, u8 mode);
#endif	/* CONFIG_TCPC_VCONN_SUPPLY_MODE */

#ifdef CONFIG_USB_PD_REV30
#ifdef CONFIG_USB_PD_REV30_PPS_SINK
int tcpm_set_apdo_charging_policy(struct tcpc_device *tcpc, u8 policy,
				  int mv, int ma,
				  const struct tcp_dpm_event_cb_data *data);
int tcpm_inquire_pd_source_apdo(struct tcpc_device *tcpc,
				u8 apdo_type, u8 *cap_i,
...
int tcpm_update_bat_last_full(struct tcpc_device *tcpc,
			      enum pd_battery_reference ref, u16 wh);
...
int tcpm_update_pd_status_bat_input(struct tcpc_device *tcpc,
				    u8 bat_input, u8 bat_mask);

int tcpm_update_pd_status_event(struct tcpc_device *tcpc, u8 evt);
...
static inline void tcpc_dev_put(struct tcpc_device *tcpc)
{
}
...
static inline u8 tcpm_inquire_typec_attach_state(struct tcpc_device *tcpc)
{
	return TYPEC_UNATTACHED;
}

static inline u8 tcpm_inquire_typec_role(struct tcpc_device *tcpc)
{
	return TYPEC_ROLE_UNKNOWN;
}

static inline u8 tcpm_inquire_typec_local_rp(struct tcpc_device *tcpc)
{
	return 0;
}
...
static inline int tcpm_typec_set_rp_level(struct tcpc_device *tcpc, u8 level)
{
	return TCPM_ERROR_NO_IMPLEMENT;
}
...
static inline int tcpm_typec_change_role(struct tcpc_device *tcpc, u8 typec_role)
{
	return TCPM_ERROR_NO_IMPLEMENT;
}
...
static inline u8 tcpm_inquire_pd_data_role(struct tcpc_device *tcpc)
{
	return 0;
}

static inline u8 tcpm_inquire_pd_power_role(struct tcpc_device *tcpc)
{
	return 0;
}

static inline u8 tcpm_inquire_pd_state_curr(struct tcpc_device *tcpc_dev)
{
	return 0;
}

static inline u8 tcpm_inquire_pd_vconn_role(struct tcpc_device *tcpc)
{
	return 0;
}

static inline u8 tcpm_inquire_pd_pe_ready(struct tcpc_device *tcpc)
{
	return 0;
}

static inline u32 tcpm_inquire_cable_current(struct tcpc_device *tcpc)
{
	return PD_CABLE_CURR_UNKNOWN;
}

static inline u32 tcpm_inquire_dpm_flags(struct tcpc_device *tcpc)
{
	return 0;
}

static inline u32 tcpm_inquire_dpm_caps(struct tcpc_device *tcpc)
{
	return 0;
}

static inline void tcpm_set_dpm_caps(struct tcpc_device *tcpc, u32 caps)
{
}
...
static inline int tcpm_inquire_cable_inform(struct tcpc_device *tcpc, u32 *vdos)
{
	return TCPM_ERROR_NO_IMPLEMENT;
}

static inline int tcpm_inquire_pd_partner_inform(struct tcpc_device *tcpc, u32 *vdos)
{
	return TCPM_ERROR_NO_IMPLEMENT;
}
...
static inline int tcpm_inquire_pd_partner_modes(struct tcpc_device *tcpc, u16 svid,
...
static inline bool tcpm_extract_power_cap_val(u32 pdo,
					      struct tcpm_power_cap_val *cap)
{
	return false;
}
...
static inline int tcpm_inquire_dp_ufp_u_state(struct tcpc_device *tcpc, u8 *state)
{
	return TCPM_ERROR_NO_IMPLEMENT;
}
...
static inline int tcpm_inquire_dp_dfp_u_state(struct tcpc_device *tcpc, u8 *state)
{
	return TCPM_ERROR_NO_IMPLEMENT;
}
...
static inline int tcpm_set_pd_charging_policy_default(struct tcpc_device *tcpc, u8 policy)
{
	return TCPM_ERROR_NO_IMPLEMENT;
}

static inline u8 tcpm_inquire_pd_charging_policy(struct tcpc_device *tcpc)
{
	return TCPM_ERROR_NO_IMPLEMENT;
}

static inline u8 tcpm_inquire_pd_charging_policy_default(struct tcpc_device *tcpc)
{
	return TCPM_ERROR_NO_IMPLEMENT;
}
#endif	/* USB_POWER_DELIVERY_NA */
...
static inline int tcpm_dpm_set_vconn_supply_mode(struct tcpc_device *tcpc, u8 mode)
{
	return TCPM_ERROR_NO_IMPLEMENT;
}
...
static inline int tcpm_set_apdo_charging_policy(struct tcpc_device *tcpc, u8 policy,
						int mv, int ma,
						const struct tcp_dpm_event_cb_data *data)
{
	return TCPM_ERROR_NO_IMPLEMENT;
...
static inline int tcpm_update_pd_status_input(struct tcpc_device *tcpc, u8 input, u8 mask)
{
	return TCPM_ERROR_NO_IMPLEMENT;
}
...
static inline int tcpm_update_pd_status_event(struct tcpc_device *tcpc, u8 evt)
{
	return TCPM_ERROR_NO_IMPLEMENT;
}
