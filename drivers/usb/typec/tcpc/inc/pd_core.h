...
#define APDO_PPS_CURR_FOLDBACK	BIT(26)
...
#define RDO_EXTEND_MESSAGE	BIT(23)
#define RDO_CURR_FOLDBACK	BIT(22)
...
#define BDO_MODE(obj)		((obj) & GENMASK(31, 28))
...
#define VDO(vid, type, custom) \
		(((u32)(vid) << 16) | ((u32)(type) << 15) | ((u32)(custom) & 0x7FFF))
...
#define VDO_IDH(usbh, usbd, ptype, ptype_dfp, is_modal, vid) \
	(((u32)(usbh) << 31) | ((u32)(usbd) << 30) | (((u32)(ptype) & 0x7) << 27) | \
	 (((u32)(ptype_dfp) & 0x07) << 23) | ((u32)(is_modal) << 26) | ((u32)(vid) & 0xffff))

#define VDO_IDH_PD20(idh)	((idh) & (~(0x1fU << 21)))
 ...
 * <15:0>  : USB bcdDevice
 */
#define VDO_PRODUCT(pid, bcd)	(((u32)(pid) & 0xffff) << 16 | ((u32)(bcd) & 0xffff))
...
#define VDO_CABLE(hw, fw, cbl, gdr, lat, term, tx1d, tx2d, rx1d, rx2d, cur, vps, sopp, usbss) \
	((((hw) & 0x7) << 28) | (((fw) & 0x7) << 24) | (((cbl) & 0x3) << 18) | \
	 ...
	 (((cur) & 0x3) << 5) | ((vps) << 4) | ((sopp) << 3) | ((usbss) & 0x7))
 ...
 * repeated.
 */
#define VDO_SVID(svid0, svid1)	(((u32)(svid0) & 0xffff) << 16 | ((u32)(svid1) & 0xffff))
...
struct pe_data {		/* reset after detached */
	bool pd_connected;
	/* written only by the PD event thread; unlocked readers elsewhere */
	...
	u8 vdm_discard_retry_count;

	u8 msg_id_rx[PD_SOP_NR];
	u8 msg_id_tx[PD_SOP_NR];

	u8 dpm_reaction_retry;
	u8 dpm_svdm_retry_cnt;
	u16 dpm_flags;
	u32 dpm_reaction_id;
	u32 dpm_ready_reactions;
...
#ifdef CONFIG_USB_PD_KEEP_PARTNER_ID
	u32 partner_vdos[VDO_MAX_NR];
#endif	/* CONFIG_USB_PD_KEEP_PARTNER_ID */

	u32 cable_vdos[VDO_MAX_NR];
...
/* DPM */
...
#ifdef CONFIG_USB_PD_REV30_PPS_SINK
	u8 local_snk_cap_nr_pd30;
	u8 local_snk_cap_nr_pd20;
#endif	/* CONFIG_USB_PD_REV30_PPS_SINK */
	...
	u8 dpm_charging_policy;
	u8 dpm_charging_policy_default;

/* ALT Mode */
#ifdef CONFIG_USB_PD_ALT_MODE
	u8 dp_first_connected;
	u8 dp_second_connected;
#endif	/* CONFIG_USB_PD_ALT_MODE */
	...
	u8 uvdm_cnt;
	u16 uvdm_svid;
	u32 uvdm_data[PD_DATA_OBJ_SIZE];
	...
	u16 pd_msg_data_size;
	u8 pd_msg_data_count;
	u8 *pd_msg_data_payload;
	...
	u8 curr_ready_state;
	u8 curr_hreset_state;
	u8 curr_sreset_state;
...
#ifdef CONFIG_USB_PD_REV30_STATUS_LOCAL
	u8 pd_status_bat_in;
	u8 pd_status_present_in;

#ifdef CONFIG_USB_PD_REV30_STATUS_LOCAL_TEMP
	u8 pd_status_temp;
	u8 pd_status_temp_status;
	...
	u8 cap_miss_match; /* For src_cap miss match */
};
...
void pd_core_deinit(struct tcpc_device *tcpc);
...
static inline u32 *pd_get_msg_vdm_data_payload(struct pd_port *pd_port)
{
	u32 *payload = (u32 *)pd_port->pd_msg_data_payload;
...
static inline u8 pd_get_msg_data_count(struct pd_port *pd_port)
{
	return pd_port->pd_msg_data_count;
}

static inline u16 pd_get_msg_data_size(struct pd_port *pd_port)
{
	return pd_port->pd_msg_data_size;
}

static inline u8 pd_get_msg_vdm_data_count(struct pd_port *pd_port)
{
...
static inline u32 pd_get_msg_vdm_hdr(struct pd_port *pd_port)
{
	return pd_port->curr_vdm_hdr;
}

#ifdef CONFIG_USB_PD_REV30
static inline u8 pd_get_msg_hdr_ext(struct pd_port *pd_port)
{
	return PD_HEADER_EXT(pd_port->curr_msg_hdr);
}
#endif	/* CONFIG_USB_PD_REV30 */

static inline u8 pd_get_msg_hdr_rev(struct pd_port *pd_port)
{
	return PD_HEADER_REV(pd_port->curr_msg_hdr);
}

static inline u8 pd_get_msg_hdr_cnt(struct pd_port *pd_port)
{
	return PD_HEADER_CNT(pd_port->curr_msg_hdr);
}

static inline u8 pd_get_msg_hdr_id(struct pd_port *pd_port)
{
	return PD_HEADER_ID(pd_port->curr_msg_hdr);
}

static inline bool pd_check_ctrl_msg_event(struct pd_port *pd_port, u8 msg)
{
	return pd_event_ctrl_msg_match(pd_get_curr_pd_event(pd_port), msg);
}

static inline bool pd_check_data_msg_event(struct pd_port *pd_port, u8 msg)
{
	return pd_event_data_msg_match(pd_get_curr_pd_event(pd_port), msg);
}

static inline bool pd_check_timer_msg_event(struct pd_port *pd_port, u8 msg)
{
...
int pd_set_rx_enable(struct pd_port *pd_port, u8 enable);
...
u32 pd_reset_pdo_power(struct tcpc_device *tcpc, u32 pdo, u32 imax);

void pd_extract_rdo_power(u32 rdo, u32 pdo, u32 *op_curr, u32 *max_curr);

u32 pd_get_cable_curr_lvl(struct pd_port *pd_port);
u32 pd_get_cable_current_limit(struct pd_port *pd_port);

int pd_set_data_role(struct pd_port *pd_port, u8 dr);
int pd_set_power_role(struct pd_port *pd_port, u8 pr);
int pd_init_message_hdr(struct pd_port *pd_port, bool act_as_sink);

int pd_set_cc_res(struct pd_port *pd_port, int pull);
int pd_set_vconn(struct pd_port *pd_port, u8 role);
int pd_reset_local_hw(struct pd_port *pd_port);

int pd_enable_bist_test_mode(struct pd_port *pd_port, bool en);
int pd_schedule_wait_request(struct pd_port *pd_port, u8 type);
...
int pd_update_connect_state(struct pd_port *pd_port, u8 state);
...
void pd_notify_pe_bist_mode(struct pd_port *pd_port, u8 mode);
...
void pd_notify_pe_transmit_msg(struct pd_port *pd_port, u8 type);
...
void pd_notify_tcp_event_buf_reset(struct pd_port *pd_port, u8 reason);
...
void pd_notify_tcp_vdm_event_2nd_result(struct pd_port *pd_port, u8 ret);

/* ---- pd_timer ---- */

static inline void pd_restart_timer(struct pd_port *pd_port, u32 timer_id)
{
	tcpc_restart_timer(pd_port->tcpc, timer_id);
}

static inline void pd_enable_timer(struct pd_port *pd_port, u32 timer_id)
{
	tcpc_enable_timer(pd_port->tcpc, timer_id);
}

static inline void pd_enable_pe_state_timer(struct pd_port *pd_port, u32 timer_id)
{
	pd_port->pe_data.pe_state_timer = timer_id;
	pd_enable_timer(pd_port, timer_id);
}

static inline void pd_disable_timer(struct pd_port *pd_port, u32 timer_id)
{
...
static inline bool pd_put_pe_event(struct pd_port *pd_port, u8 pe_event)
{
	struct pd_event evt = {
...
static inline bool pd_put_dpm_notify_event(struct pd_port *pd_port, u8 notify)
{
	struct pd_event evt = {
...
static inline bool pd_put_dpm_nak_event(struct pd_port *pd_port, u8 notify)
{
	struct pd_event evt = {
		.event_type = PD_EVT_DPM_MSG,
		.msg = PD_DPM_NAK,
...
static inline bool pd_put_dpm_event(struct pd_port *pd_port, u8 msg)
{
	struct pd_event evt = {
		.event_type = PD_EVT_DPM_MSG,
		.msg = msg,
...
static inline bool pd_put_tcp_pd_event(struct pd_port *pd_port, u8 event)
{
	struct pd_event evt = {
...
static inline bool pd_put_tcp_vdm_event(struct pd_port *pd_port, u8 event)
{
	struct pd_event evt = {
		.event_type = PD_EVT_TCP_MSG,
		.msg = event,
...
static inline bool vdm_put_hw_event(struct tcpc_device *tcpc, u8 hw_event)
{
	struct pd_event evt = {
...
static inline bool vdm_put_pe_event(struct tcpc_device *tcpc, u8 pe_event)
{
	struct pd_event evt = {
		.event_type = PD_EVT_PE_MSG,
		.msg = pe_event,
...
static inline bool pd_put_hw_event(struct tcpc_device *tcpc, u8 hw_event)
{
	struct pd_event evt = {
		.event_type = PD_EVT_HW_MSG,
		.msg = hw_event,
...
static inline bool pd_put_sink_tx_event(struct tcpc_device *tcpc, u8 cc_res)
{
#ifdef CONFIG_USB_PD_REV30_COLLISION_AVOID
	struct pd_event evt = {
		.event_type = PD_EVT_HW_MSG,
...
int pd_send_message(struct pd_port *pd_port, u8 sop_type,
		    u8 msg, bool ext, u16 count, const u32 *data);

int pd_send_data_msg(struct pd_port *pd_port,
		     u8 sop_type, u8 msg, u8 cnt, u32 *payload);

int pd_send_sop_ctrl_msg(struct pd_port *pd_port, u8 msg);

int pd_send_sop_prime_ctrl_msg(struct pd_port *pd_port, u8 msg);

int pd_send_sop_data_msg(struct pd_port *pd_port,
			 u8 msg, u8 cnt, const u32 *payload);

int pd_reply_wait_reject_msg(struct pd_port *pd_port);
int pd_reply_wait_reject_msg_no_resp(struct pd_port *pd_port);

int pd_send_swap_msg(struct pd_port *pd_port, u8 msg);

#ifdef CONFIG_USB_PD_REV30
int pd_send_ext_msg(struct pd_port *pd_port,
		    u8 sop_type, u8 msg, bool request,
		    u8 chunk_nr, u8 size, const u8 *data);

static inline int pd_send_sop_ext_msg(struct pd_port *pd_port,
				      u8 msg, u8 size, const void *data)
{
...
static inline int pd_send_vdm_discover_id(struct pd_port *pd_port, u8 sop_type)
{
	return pd_send_svdm_request(pd_port, sop_type, USB_SID_PD,
			CMD_DISCOVER_IDENT, 0, 0, NULL, PD_TIMER_VDM_RESPONSE);
}

static inline int pd_send_vdm_discover_svids(struct pd_port *pd_port, u8 sop_type)
{
	return pd_send_svdm_request(pd_port, sop_type, USB_SID_PD,
			CMD_DISCOVER_SVID, 0, 0, NULL, PD_TIMER_VDM_RESPONSE);
}

static inline int pd_send_vdm_discover_modes(struct pd_port *pd_port, u8 sop_type, u16 svid)
{
...
static inline int pd_send_vdm_enter_mode(struct pd_port *pd_port,
					 u8 sop_type, u16 svid, u8 obj_pos)
{
	return pd_send_svdm_request(pd_port, sop_type, svid,
...
static inline int pd_send_vdm_dp_attention(struct pd_port *pd_port,
					   u8 sop_type, u8 obj_pos, u32 dp_status)
{
...
static inline int pd_reply_svdm_request_simply(struct pd_port *pd_port, u8 reply)
{
...
#ifdef CONFIG_USB_PD_CUSTOM_VDM
int pd_send_custom_vdm(struct pd_port *pd_port, u8 sop_type);
int pd_reply_custom_vdm(struct pd_port *pd_port, u8 sop_type,
			u8 cnt, u32 *payload);
#endif	/* CONFIG_USB_PD_CUSTOM_VDM */
...
void pd_set_sink_tx(struct pd_port *pd_port, u8 cc);
void pd_sync_sop_spec_revision(struct pd_port *pd_port);
void pd_sync_sop_prime_spec_revision(struct pd_port *pd_port, u8 rev);
bool pd_is_multi_chunk_msg(struct pd_port *pd_port);

static inline u8 pd_get_fix_battery_nr(struct pd_port *pd_port)
{
	return PD_SCEDB_FIX_BAT_NR(pd_port->bat_nr);
}

static inline u8 pd_get_swap_battery_nr(struct pd_port *pd_port)
{
