...
int register_tcpc_late_sync_notifier(struct tcpc_device *tcpc, struct notifier_block *nb);
int unregister_tcpc_late_sync_notifier(struct tcpc_device *tcpc, struct notifier_block *nb);
...
void tcpc_dual_role_phy_deinit(struct tcpc_device *tcpc);
#endif	/* CONFIG_DUAL_ROLE_USB_INTF */
...
/*
 * Each subsystem owns the resources it uses: it allocates them in its
 * own *_init() and frees them in the matching *_deinit(), so that
 * tcpc_device_register()/unregister() only orchestrate the calls.
 */
