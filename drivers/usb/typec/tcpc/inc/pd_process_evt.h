...
#ifndef __PD_PROCESS_EVT_H__
#define __PD_PROCESS_EVT_H__
...
static inline u8 pe_get_curr_ready_state(struct pd_port *pd_port)
{
	return pd_port->curr_ready_state;
}

static inline u8 pe_get_curr_hard_reset_state(struct pd_port *pd_port)
{
	return pd_port->curr_hreset_state;
}

static inline u8 pe_get_curr_soft_reset_state(struct pd_port *pd_port)
{
	return pd_port->curr_sreset_state;
}

static inline u8 pe_get_curr_evaluate_pr_swap_state(struct pd_port *pd_port)
{
...
static inline u8 pe_get_curr_send_pr_swap_state(struct pd_port *pd_port)
{
	if (pd_port->power_role == PD_ROLE_SINK)
...
static inline u8 pd_get_curr_hard_reset_recv_state(struct pd_port *pd_port)
{
	if (pd_port->power_role == PD_ROLE_SINK)
...
static inline u8 pd_get_curr_soft_reset_recv_state(struct pd_port *pd_port)
{
	if (pd_port->power_role == PD_ROLE_SINK)
...
#define PE_MAKE_STATE_TRANSIT_SINGLE(reaction, next)	\
	pd_make_pe_state_transit_single(pd_port, \
		pd_port->pe_state_curr, reaction, next)
...
#define PE_MAKE_STATE_TRANSIT(state)	\
	pd_make_pe_state_transit(pd_port, \
		pd_port->pe_state_curr, &state##_reactions)
...
bool pd_make_pe_state_transit(struct pd_port *pd_port, u8 curr_state,
			      const struct pe_state_reaction *state_reaction);

bool pd_process_event(struct pd_port *pd_port, struct pd_event *pd_event);

bool pd_process_event_snk(struct pd_port *pd_port, struct pd_event *evt);
bool pd_process_event_src(struct pd_port *pd_port, struct pd_event *evt);
bool pd_process_event_drs(struct pd_port *pd_port, struct pd_event *evt);
bool pd_process_event_prs(struct pd_port *pd_port, struct pd_event *evt);
bool pd_process_event_vdm(struct pd_port *pd_port, struct pd_event *evt);
bool pd_process_event_vcs(struct pd_port *pd_port, struct pd_event *evt);
bool pd_process_event_com(struct pd_port *pd_port, struct pd_event *evt);
bool pd_process_event_tcp(struct pd_port *pd_port, struct pd_event *evt);

#ifdef CONFIG_USB_PD_CUSTOM_DBGACC
bool pd_process_event_dbg(struct pd_port *pd_port, struct pd_event *evt);
#endif	/* CONFIG_USB_PD_CUSTOM_DBGACC */

#endif	/* __PD_PROCESS_EVT_H__ */
