...
#undef pr_fmt
#define pr_fmt(fmt) "[pd_core]: %s: " fmt, __func__
...
static inline int pd_parse_pdata_bats(struct pd_port *pd_port, struct device_node *np)
{
	...
	for (i = 0; i < pd_port->bat_nr; i++) {
		pr_info("fix_bat_info[%d]: .mfrs_info.vid = 0x%x, .mfrs_info.pid = 0x%x, .mfrs_string = %s, .bat_design_cap = %d\n",
			i,
...
static inline int pd_parse_pdata_countries(struct pd_port *pd_port, struct device_node *np)
{
	...
	for (i = 0; i < pd_port->country_nr; i++) {
		...
		sub = of_find_node_by_name(np, temp_string);
		if (!sub) {
			pr_err("get sub country node fail\n");
			return -ENODEV;
		}
...
{
	pr_info("vid = 0x%x, pid = 0x%x, xid = 0x%x, fw_ver = 0x%x, hw_ver = 0x%0x\n",
		cap->vid, cap->pid, cap->xid,
		cap->fw_ver, cap->hw_ver);

	pr_info("voltage_regulation = %d, hold_time_ms = %d, compliance = 0x%x, touch_current = 0x%x, peak_current = %d %d %d\n",
	...
	pr_info("touch_temp = %d, source_inputs = 0x%x, batteries = 0x%x, source_pdp = 0x%x\n",
		cap->touch_temp,
		cap->source_inputs,
		cap->batteries,
		cap->source_pdp);
...
static inline void pd_parse_pdata_src_cap_ext(struct pd_port *pd_port, struct device_node *np)
{
	...
	if (ret < 0)
		pr_err("get source-cap-ext fail\n");
	else
...
static int pd_parse_pdata(struct pd_port *pd_port)
{
	...
	if (np) {
		...
		if (ret < 0)
			pr_err("get source pdo size fail\n");
		...
		if (ret < 0)
			pr_err("get source pdo data fail\n");

		pr_info("src pdo data =\n");
...
static void pd_core_power_flags_init(struct pd_port *pd_port)
{
	...
	if (!pd_port || !pd_port->tcpc || !pd_port->tcpc->dev.of_node) {
		pr_err("pd_core_power_flags_init: invalid pd_port or of_node\n");
		return;
	}

	np = of_find_node_by_name(pd_port->tcpc->dev.of_node, "dpm_caps");
	if (!np) {
		pr_err("dpm_caps node not found\n");
		return;
	}
...
int pd_core_init(struct tcpc_device *tcpc)
{
	...
	if (ret < 0) {
		pr_err("%s: pd_parse_pdata failed (%d)\n", __func__, ret);
		goto out;
	}
	...
	if (ret < 0) {
		pr_err("pd_dpm_core_init failed (%d)\n", ret);
		goto out;
	}
...
out:
#ifdef CONFIG_USB_PD_BLOCK_TCPM
	mutex_destroy(&pd_port->tcpm_bk_lock);
#endif	/* CONFIG_USB_PD_BLOCK_TCPM */
	mutex_destroy(&pd_port->pd_lock);
	pr_info("fail!\n");
	return ret;
}
...
#ifdef CONFIG_USB_PD_BLOCK_TCPM
	/* Wake up any blocked callers before destroying the lock */
	wake_up_all(&pd_port->tcpm_bk_wait_que);
	mutex_destroy(&pd_port->tcpm_bk_lock);
#endif	/* CONFIG_USB_PD_BLOCK_TCPM */
