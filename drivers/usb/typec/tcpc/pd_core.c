...
static void pd_parse_pdata_bat_info(struct pd_port *pd_port,
				    ...
				    struct pd_battery_info *bat_info)
{
	...
	snprintf(mfrs_info->mfrs_string, sizeof(mfrs_info->mfrs_string), "%s", mstring);
#endif	/* CONFIG_USB_PD_REV30_MFRS_INFO_LOCAL */
	...
	if (ret < 0) {
		...
		bat_cap->bat_design_cap = (u16)PD_BCDB_BAT_CAP_RAW(design_cap);
	}
...
static int pd_parse_pdata_bats(struct pd_port *pd_port, struct device_node *np)
{
	u32 val;
	int ret = 0, i;
	struct device_node *sub;
	...
	pd_port->fix_bat_info = kcalloc(pd_port->bat_nr,
					sizeof(struct pd_battery_info),
					GFP_KERNEL);
	if (!pd_port->fix_bat_info)
		return -ENOMEM;

	for (i = 0; i < pd_port->bat_nr; i++) {
		snprintf(temp_string, sizeof(temp_string), "bat-info%d", i);
		sub = of_find_node_by_name(np, temp_string);
		if (!sub) {
			pr_err("%s: get sub bat node fail\n", __func__);
			kfree(pd_port->fix_bat_info);
			pd_port->fix_bat_info = NULL;
			return -ENODEV;
		}
...
static int pd_parse_pdata_country(struct pd_port *pd_port,
				  struct device_node *sub,
				  struct pd_country_authority *country_info)
{
	...
	country_info->len = (u16)val;
	country_info->data = kcalloc(country_info->len, sizeof(u8), GFP_KERNEL);
	if (!country_info->data)
		return -ENOMEM;

	temp_u32 = kcalloc(country_info->len, sizeof(u32), GFP_KERNEL);
	if (!temp_u32) {
		kfree(country_info->data);
		country_info->data = NULL;
		return -ENOMEM;
	}
	...
	for (j = 0; j < country_info->len; j++)
		country_info->data[j] = (u8)temp_u32[j];
...
static int pd_parse_pdata_countries(struct pd_port *pd_port, struct device_node *np)
{
	int ret = 0, i, j;
	struct device_node *sub;
	char temp_string[26];
	...
	pd_port->country_info = kcalloc(pd_port->country_nr,
					sizeof(struct pd_country_authority),
					GFP_KERNEL);
	if (!pd_port->country_info)
		return -ENOMEM;

	for (i = 0; i < pd_port->country_nr; i++) {
		snprintf(temp_string, sizeof(temp_string), "country%d", i);
		sub = of_find_node_by_name(np, temp_string);
		if (!sub) {
	...
	for (i = 0; i < pd_port->country_nr; i++) {
		...
		for (j = 0; j < pd_port->country_info[i].len; j++)
			pr_info("%s: country_info[%d].data[%d] = 0x%x\n",
				__func__, i, j, pd_port->country_info[i].data[j]);
	}
...
err_free:
	for (j = 0; j < i; j++) {
		kfree(pd_port->country_info[j].data);
		pd_port->country_info[j].data = NULL;
	}
	kfree(pd_port->country_info);
	pd_port->country_info = NULL;
...
static void pd_parse_pdata_src_cap_ext(struct pd_port *pd_port, struct device_node *np)
{
#ifdef CONFIG_USB_PD_REV30_SRC_CAP_EXT_LOCAL
	int ret = 0;
...
static void pd_parse_pdata_mfrs(struct pd_port *pd_port, struct device_node *np)
{
	int ret = 0;
	u32 vid, pid;
#ifdef CONFIG_USB_PD_REV30_MFRS_INFO_LOCAL
...
static void pd_core_power_flags_init(struct pd_port *pd_port)
{
	...
	if (!np) {
		pr_err("%s: dpm_caps DT node not found\n", __func__);
		return;
	}
	...
	pr_info("%s: dpm_caps = 0x%08x\n", __func__, pd_port->dpm_caps);
...
static void fg_bat_absent_work(struct work_struct *work)
{
	...
	if (ret < 0)
		dev_notice(&tcpc->dev, "%s: tcpm shutdown fail\n", __func__);
}
...
static void pd_parse_pdata_deinit(struct pd_port *pd_port)
{
#ifdef CONFIG_USB_PD_REV30_COUNTRY_AUTHORITY
	if (pd_port->country_info) {
		int i;
		...
		for (i = 0; i < pd_port->country_nr; i++)
			kfree(pd_port->country_info[i].data);
		kfree(pd_port->country_info);
		pd_port->country_info = NULL;
	}
#endif
...
int pd_core_init(struct tcpc_device *tcpc)
{
	...
	ret = pd_dpm_core_init(pd_port);
	if (ret < 0)
		goto err_init;

#ifdef CONFIG_RECV_BAT_ABSENT_NOTIFY
	INIT_WORK(&pd_port->fg_bat_work, fg_bat_absent_work);
	pd_port->fg_bat_nb.notifier_call = fg_bat_notifier_call;
	ret = register_battery_notifier(&pd_port->fg_bat_nb);
	if (ret < 0) {
		dev_notice(&tcpc->dev, "%s: register bat notifier fail: %d\n", __func__, ret);
		pd_port->fg_bat_nb.notifier_call = NULL;
	}
...
err_init:
#ifdef CONFIG_USB_PD_BLOCK_TCPM
	tcpm_block_deinit(pd_port);
#endif	/* CONFIG_USB_PD_BLOCK_TCPM */
	pd_parse_pdata_deinit(pd_port);
	return ret;
}
...
#ifdef CONFIG_RECV_BAT_ABSENT_NOTIFY
	if (pd_port->fg_bat_nb.notifier_call) {
		unregister_battery_notifier(&pd_port->fg_bat_nb);
		pd_port->fg_bat_nb.notifier_call = NULL;
	}
	cancel_work_sync(&pd_port->fg_bat_work);
#endif	/* CONFIG_RECV_BAT_ABSENT_NOTIFY */

	pd_dpm_core_deinit(pd_port);
	pd_parse_pdata_deinit(pd_port);
...
int pd_enable_vbus_valid_detection(struct pd_port *pd_port, bool wait_valid)
{
	...
	PE_DBG("WaitVBUS=%d\n", wait_valid);
	pd_notify_pe_wait_vbus_once(pd_port, wait_valid ?
				    PD_WAIT_VBUS_VALID_ONCE :
...
static int pd_update_msg_header(struct pd_port *pd_port)
{
...
int pd_set_vconn(struct pd_port *pd_port, u8 role)
{
	...
	u8 old_role = pd_port->vconn_role;
	struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;
...
static int pd_reset_modal_operation(struct pd_port *pd_port)
{
	u8 i;
	struct svdm_svid_data *svid_data;
		    ...
		    u8 msg, bool ext, u16 count, const u32 *data)
{
	...
	u16 msg_hdr;
	u16 msg_hdr_private;
	u8 pd_rev = pd_get_rev(pd_port, sop_type);
	...
	if (sop_type >= PD_SOP_NR) {
		PE_DBG("[SendMsg] Invalid sop_type %d\n", sop_type);
		return 0;
	}
...
int pd_reply_wait_reject_msg_no_resp(struct pd_port *pd_port)
{
	...
	u8 msg = pd_event->msg_sec == PD_DPM_NAK_REJECT ?
...
{
	u8 cnt;
	u32 payload[PD_DATA_OBJ_SIZE];

	u16 *ext_hdr = (u16 *)payload;
...
int pd_send_hard_reset(struct pd_port *pd_port)
{
	...
	__pm_wakeup_event(tcpc->alert_wake_lock, 6000);
...
{
#ifdef CONFIG_USB_PD_REV30_SYNC_SPEC_REV
	u8 rev = pd_get_msg_hdr_rev(pd_port);
	struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;
