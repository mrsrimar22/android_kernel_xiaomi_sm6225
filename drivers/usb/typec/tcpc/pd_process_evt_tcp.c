...
static int pd_handle_tcp_event_pr_swap(struct pd_port *pd_port, u8 new_role)
{
...
static int pd_handle_tcp_event_dr_swap(struct pd_port *pd_port, u8 new_role)
{
...
static int pd_handle_tcp_event_vconn_swap(struct pd_port *pd_port, u8 new_role)
{
	u8 old_role = pd_port->vconn_role ? 1 : 0;

	if (old_role == new_role)
		return TCP_DPM_RET_DENIED_SAME_ROLE;

	if (!pd_port->vconn_role && !(pd_port->dpm_caps & DPM_CAP_LOCAL_VCONN_SUPPLY))
		return TCP_DPM_RET_DENIED_LOCAL_CAP;
...
static int pd_handle_tcp_event_gotomin(struct pd_port *pd_port)
{
...
static int pd_handle_tcp_event_softreset(struct pd_port *pd_port)
{
	if (!pd_check_pe_state_ready(pd_port))
		return TCP_DPM_RET_DENIED_NOT_READY;
...
static int pd_handle_tcp_event_cable_softreset(struct pd_port *pd_port)
{
	if (!pd_check_pe_state_ready(pd_port))
		return TCP_DPM_RET_DENIED_NOT_READY;
...
static int pd_handle_tcp_event_get_source_cap(struct pd_port *pd_port)
{
...
static int pd_handle_tcp_event_get_sink_cap(struct pd_port *pd_port)
{
	switch (pd_port->pe_state_curr) {
	case PE_SRC_READY:
...
static int pd_handle_tcp_event_request(struct pd_port *pd_port)
{
	int ret = 0;
	...
	case TCP_DPM_EVT_REQUEST_EX:
		ret = pd_dpm_update_tcp_request_ex(pd_port, &tcp_event->tcp_dpm_data.pd_req_ex);
		break;
...
static int pd_handle_tcp_event_bist_cm2(struct pd_port *pd_port)
{
...
static int pd_handle_tcp_event_get_source_cap_ext(struct pd_port *pd_port)
{
	switch (pd_port->pe_state_curr) {
	case PE_SNK_READY:
...
static int pd_handle_tcp_event_get_pps_status(struct pd_port *pd_port)
{
	if (pd_port->pe_state_curr != PE_SNK_READY)
		return TCP_DPM_RET_DENIED_NOT_READY;
...
static int pd_make_tcp_event_transit_ready(struct pd_port *pd_port, u8 state)
{
	if (!pd_check_pe_state_ready(pd_port))
		return TCP_DPM_RET_DENIED_NOT_READY;
...
static int pd_make_tcp_event_transit_ready2(struct pd_port *pd_port,
					    u8 snk_state, u8 src_state)
{
	switch (pd_port->pe_state_curr) {
#ifdef CONFIG_USB_PD_PE_SINK
	case PE_SNK_READY:
...
static int pd_handle_tcp_event_alert(struct pd_port *pd_port)
{
	struct tcp_dpm_event *tcp_event = &pd_port->tcp_event;
...
static int pd_handle_tcp_event_hardreset(struct pd_port *pd_port)
{
	pe_transit_hard_reset_state(pd_port);
	return TCP_DPM_RET_SENT;
}

static int pd_handle_tcp_event_error_recovery(struct pd_port *pd_port)
{
	PE_TRANSIT_STATE(pd_port, PE_ERROR_RECOVERY);
	return TCP_DPM_RET_SENT;
}

static int pd_handle_tcp_dpm_event(struct pd_port *pd_port,
...
bool pd_process_event_tcp(struct pd_port *pd_port,
			  struct pd_event *pd_event)
{
