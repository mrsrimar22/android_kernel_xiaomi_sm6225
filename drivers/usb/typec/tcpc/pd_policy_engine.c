...
/* pd_state_action_fcn_t pe_get_exit_action(u8 pe_state) */
void (*pe_get_exit_action(u8 pe_state))(struct pd_port *)
{
...
#if PE_DBG_ENABLE || PE_STATE_INFO_ENABLE
static void print_state(struct pd_port *pd_port, u8 state)
{
	/*
...
static void pe_reset_vdm_state_variable(struct pd_port *pd_port, struct pe_data *pe_data)
{
...
static void pd_pe_state_change(struct pd_port *pd_port, struct pd_event *pd_event)
{
...
static int pd_handle_event(struct pd_port *pd_port, struct pd_event *pd_event)
{
	...
	if (pd_curr_is_vdm_evt(pd_port)) {
		dpm_imme = pd_port->pe_data.vdm_state_flags & VDM_STATE_FLAG_DPM_ACK_IMMEDIATELY;
...
static bool pd_try_get_vdm_event(struct tcpc_device *tcpc, struct pd_event *pd_event)
{
	bool ret = false;
...
static bool pd_check_sink_tx_ok(struct pd_port *pd_port)
{
#ifdef CONFIG_USB_PD_REV30_COLLISION_AVOID
	if (pd_check_rev30(pd_port) && pd_port->pe_data.pd_traffic_control != PD_SINK_TX_OK)
		return false;
...
static bool pd_check_source_tx_ok(struct pd_port *pd_port)
{
#ifdef CONFIG_USB_PD_REV30_COLLISION_AVOID
...
static bool pd_check_pd30_tx_ready(struct pd_port *pd_port)
{
#ifdef CONFIG_USB_PD_PE_SINK
...
#else
static bool pd_check_pd20_tx_ready(struct pd_port *pd_port)
{
	switch (pd_port->pe_pd_state) {
#ifdef CONFIG_USB_PD_PE_SINK
	case PE_SNK_READY:
...
static bool pd_check_tx_ready(struct pd_port *pd_port)
{
...
static u8 pd_try_get_deferred_tcp_event(struct pd_port *pd_port)
{
...
static u8 pd_try_get_active_event(struct tcpc_device *tcpc, struct pd_event *pd_event)
{
	u8 ret;
	u8 from_pe = PD_TCP_FROM_PE;
	struct pd_port *pd_port = &tcpc->pd_port;
...
static u8 pd_try_get_next_event(struct tcpc_device *tcpc, struct pd_event *pd_event)
{
	u8 ret = 0;
	struct pd_port *pd_port = &tcpc->pd_port;
...
static int pd_handle_dpm_immediately(struct pd_port *pd_port, struct pd_event *pd_event)
{
	bool dpm_immediately;
	struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;

	if (pd_curr_is_vdm_evt(pd_port))
		dpm_immediately = pd_port->pe_data.vdm_state_flags & VDM_STATE_FLAG_DPM_ACK_IMMEDIATELY;
	else
		dpm_immediately = pd_port->pe_data.pe_state_flags2 & PE_STATE_FLAG_DPM_ACK_IMMEDIATELY;
