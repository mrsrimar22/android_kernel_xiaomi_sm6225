...
/*
 * charger dev have 1s delay when SDP, add safe margin.
 * 10ms * 200 = 2000ms = 2s
 */
#define POLLING_INTERVAL	10
#define POLLING_CNT_MAX		200
...
struct rt_pd_manager_data {
	...
	int sink_mv_pd;
	int sink_ma_pd;
	enum pd_connect_result pd_connect_state;
...
static void stop_usb_host(struct rt_pd_manager_data *rpmd)
{
	extcon_set_state_sync(rpmd->extcon, EXTCON_USB_HOST, false);
}

static void start_usb_host(struct rt_pd_manager_data *rpmd)
{
...
static void stop_usb_peripheral(struct rt_pd_manager_data *rpmd)
{
	extcon_set_state_sync(rpmd->extcon, EXTCON_USB, false);
}

static void start_usb_peripheral(struct rt_pd_manager_data *rpmd)
{
#if (KERNEL_VERSION(5, 4, 0) > LINUX_VERSION_CODE)
	int rp = 0;
#endif	/* (LINUX_VERSION_CODE < KERNEL_VERSION(5, 4, 0)) */
...
static bool is_valid_usb_type(int type)
{
	switch (type) {
...
static void usb_dwork_handler(struct work_struct *work)
{
	...
	case DR_MAX:
		...
		break;
	case DR_DEVICE:
		ret = power_supply_get_property(rpmd->usb_psy,
		...
		if (ret < 0 || val.intval == POWER_SUPPLY_TYPE_UNKNOWN) {
			if (rpmd->usb_type_polling_cnt < POLLING_CNT_MAX)
				queue_delayed_work(rpmd->wq, &rpmd->usb_dwork,
						   msecs_to_jiffies(POLLING_INTERVAL));
			break;
		}

		if (!is_valid_usb_type(val.intval))
			break;
				    ...
				    int mv, int ma, u8 type)
{
	...
	if (val.intval < micro_5v)
		val.intval = micro_5v;
	if (test_bit(0, &sel))
		power_supply_set_property(rpmd->usb_psy,
...
static int pd_tcp_notifier_call(struct notifier_block *nb, unsigned long event, void *data)
{
	...
	u8 old_state = TYPEC_UNATTACHED, new_state = TYPEC_UNATTACHED;
	...
	case TCP_NOTIFY_TYPEC_STATE:
		...
		if (old_state == TYPEC_UNATTACHED &&
			...
			cancel_delayed_work_sync(&rpmd->usb_dwork);
			rpmd->usb_dr = DR_IDLE;
			queue_delayed_work(rpmd->wq, &rpmd->usb_dwork, 0);
			...
			dev_info(rpmd->dev, "%s: OTG plug in, polarity = %d\n",
				 __func__, noti->typec_state.polarity);
			/*
			 * fix confusing power_supply CC_ORIENTATION type
			 * set from OTG driver
			 */
...
#endif	/* (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0)) */
	u8 typec_role = TYPEC_ROLE_UNKNOWN;
...
#endif	/* (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0)) */
	int ret = 0;
	u8 data_role = tcpm_inquire_pd_data_role(rpmd->tcpc);
	bool do_swap = false;
...
#endif	/* (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0)) */
	int ret = 0;
	u8 power_role = tcpm_inquire_pd_power_role(rpmd->tcpc);
	bool do_swap = false;
...
#endif	/* (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0)) */
	int ret = 0;
	u8 vconn_role = tcpm_inquire_pd_vconn_role(rpmd->tcpc);
	bool do_swap = false;
...
static void typec_deinit(void *data)
{
	struct rt_pd_manager_data *rpmd = data;
...
static int typec_init(struct rt_pd_manager_data *rpmd)
{
	...
	ret = devm_add_action_or_reset(rpmd->dev, typec_deinit, rpmd);
	if (ret) {
		dev_err(rpmd->dev, "%s: devm_add_action_or_reset fail(%d)\n", __func__, ret);
		return ret;
	}
...
static int rt_pd_manager_probe(struct platform_device *pdev)
{
	...
	if (probe_cnt >= PROBE_CNT_MAX) {
		dev_err(&pdev->dev, "%s: probe count exceeded: %d >= %d\n",
			__func__, probe_cnt, PROBE_CNT_MAX);
		return -ENODEV;
	}

	if (!rt1711h_chip_is_ready()) {
		dev_err(&pdev->dev, "%s: rt1711h_chip not ready\n", __func__);
		return -EPROBE_DEFER;
	}
	...
	rpmd->wq = alloc_ordered_workqueue("%s", WQ_HIGHPRI | WQ_MEM_RECLAIM,
					   dev_name(&pdev->dev));
	if (!rpmd->wq) {
		dev_err(&pdev->dev, "%s: create workqueue fail\n", __func__);
		ret = -ENOMEM;
		goto err_free;
	}
	...
	rpmd->pd_nb.priority = TCP_NOTIFY_PRIO_RT_PD_MANAGER;
	...
	ret = tcpci_late_sync_init(rpmd->tcpc);
	if (ret < 0) {
		dev_err(&pdev->dev, "%s: late sync init fail (%d)\n", __func__, ret);
		ret = -EPROBE_DEFER;
		goto err_unreg_tcpc_notifier;
	}

	dev_info(&pdev->dev, "%s: OK!!\n", __func__);
	return 0;

err_unreg_tcpc_notifier:
	unregister_tcp_dev_notifier(rpmd->tcpc, &rpmd->pd_nb, TCP_NOTIFY_TYPE_ALL);
err_destroy_wq:
	if (rpmd->wq)
		destroy_workqueue(rpmd->wq);
err_free:
