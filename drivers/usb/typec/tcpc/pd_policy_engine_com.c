...
static void *pd_get_tcp_event_data_object(struct pd_port *pd_port)
{
...
{
	u8 state = PD_CONNECT_TYPEC_ONLY_SNK_DFT;
...
static u8 pe20_power_ready_entry(struct pd_port *pd_port)
{
...
static u8 pe30_power_ready_entry(struct pd_port *pd_port)
{
...
{
	u32 *ccdo = pd_get_tcp_event_data_object(pd_port);
