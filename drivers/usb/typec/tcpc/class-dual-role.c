...
static char *kstrdupcase(const char *str, gfp_t gfp, bool to_upper)
{
	...
	/* allocate zeroed buffer to ensure NUL termination */
	ret = kzalloc(strlen(str) + 1, gfp);
	if (!ret)
		return NULL;
	...
	while (*str)
		*ustr++ = to_upper ? toupper((unsigned char)*str++) :
...
int dual_role_uevent(struct device *dev, struct kobj_uevent_env *env)
{
	...
	/* defend against a NULL or empty name */
