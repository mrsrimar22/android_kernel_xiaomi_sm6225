...
static char *kstrdupcase(const char *str, gfp_t gfp, bool to_upper)
{
	...
	for (p = ret; *p; p++)
		*p = to_upper ? toupper(*p) : tolower(*p);
				       ...
				       struct device_attribute *attr, char *buf)
{
	...
	if (off == DUAL_ROLE_PROP_SUPPORTED_MODES) {
		...
		if (ret < 0) {
			if (ret == -ENODATA)
				dev_dbg(dev, "%s: driver has no data for `%s' property\n",
					__func__, attr->attr.name);
			else if (ret != -ENODEV)
				dev_err(dev, "%s: driver failed to report `%s' property: %zd\n",
					__func__, attr->attr.name, ret);
			return ret;
		}
	}

	if (off == DUAL_ROLE_PROP_SUPPORTED_MODES) {
		BUILD_BUG_ON(ARRAY_SIZE(supported_modes_text) !=
		...
		BUILD_BUG_ON(ARRAY_SIZE(mode_text) !=
		...
		BUILD_BUG_ON(ARRAY_SIZE(pr_text) !=
		...
		BUILD_BUG_ON(ARRAY_SIZE(dr_text) !=
		...
		BUILD_BUG_ON(ARRAY_SIZE(vconn_supply_text) !=
...
static void dual_role_init_attrs(struct device_type *dev_type)
{
	int i;
...
int dual_role_uevent(struct device *dev, struct kobj_uevent_env *env)
{
	...
	for (j = 0; j < dual_role->desc->num_properties; j++) {
		...
		name = attr->attr.name;
		if (strlen(name) >= sizeof(attrname)) {
			ret = -ENAMETOOLONG;
			goto out;
		}

		for (k = 0; name[k]; k++)
			attrname[k] = toupper(name[k]);
		attrname[k] = 0;
