...
int tcpc_dual_role_phy_init(struct tcpc_device *tcpc)
{
	...
	name_len = strlen(tcpc->desc.name) + 11;
	str_name = devm_kzalloc(&tcpc->dev, name_len, GFP_KERNEL);
	if (!str_name)
		return -ENOMEM;

	snprintf(str_name, name_len, "dual-role-%s", tcpc->desc.name);
