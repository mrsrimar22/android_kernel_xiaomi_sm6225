				      ...
				      unsigned int val)
{
	...
	dev_info(&tcpc->dev, "%s: data role swap (%d->%d): %d\n",
					 ...
					 unsigned int val)
{
	...
	dev_info(&tcpc->dev, "%s: vconn swap (%d->%d): %d\n",
...
int tcpc_dual_role_phy_init(struct tcpc_device *tcpc)
{
	...
	dual_desc = kzalloc(sizeof(*dual_desc), GFP_KERNEL);
	if (!dual_desc)
		return -ENOMEM;

	tcpc_get_dual_desc(tcpc);

	str_name = kasprintf(GFP_KERNEL, "dual-role-%s", tcpc->desc.name);
	if (!str_name) {
		ret = -ENOMEM;
		goto err_name;
	}
	...
	dr_usb = devm_dual_role_instance_register(&tcpc->dev, dual_desc);
	if (IS_ERR(dr_usb)) {
...
void tcpc_dual_role_phy_deinit(struct tcpc_device *tcpc)
{
	...
	kfree(tcpc->dual_role_desc->name);
	kfree(tcpc->dual_role_desc);
