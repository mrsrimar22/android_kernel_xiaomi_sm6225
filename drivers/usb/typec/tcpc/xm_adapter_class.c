...
struct class *adapter_class;
EXPORT_SYMBOL_GPL(adapter_class);

#define CLASS_PREFIX "[xm_adapter_class]: "
#define class_err(fmt, ...) pr_err(CLASS_PREFIX fmt, ##__VA_ARGS__)
#define class_info(fmt, ...) pr_info(CLASS_PREFIX fmt, ##__VA_ARGS__)
#define class_dbg(fmt, ...) pr_debug(CLASS_PREFIX fmt, ##__VA_ARGS__)
			 ...
			 char *buf)
{
	...
	return snprintf(buf, 20, "%s\n",
			adapter_dev->alias_name ?
			adapter_dev->alias_name : "anonymous");
}
...
int adapter_dev_get_pd_verified(void)
{
	...
	if (adapter_dev) {
		verified = adapter_dev->verifed;
		adapter_put(adapter_dev);
		return verified;
	}
...
{
	if (adapter_dev && adapter_dev->ops && adapter_dev->ops->get_svid)
				...
				unsigned int data_len)
{
	if (adapter_dev && adapter_dev->ops && adapter_dev->ops->request_vdm_cmd)
		return adapter_dev->ops->request_vdm_cmd(adapter_dev, cmd,
							 data, data_len);
			       ...
			       char *buf)
{
	...
	if (adapter_dev->ops && adapter_dev->ops->get_svid)
...
{
	if (adapter_dev && adapter_dev->ops && adapter_dev->ops->get_cap)
				     ...
				     const char *buf, size_t size)
{
	...
	sscanf_ret = sscanf(buf, "%d,%63s", &cmd, raw_buf);
	if (sscanf_ret <= 0) {
	...
	hexstr = strim(raw_buf);
	if (strstarts(hexstr, "0x") || strstarts(hexstr, "0X"))
		hexstr += 2;

	len = strlen(hexstr);
	if (len == 0 || !strncasecmp(hexstr, "none", 4) ||
	    !strncasecmp(hexstr, "null", 4) || !strncasecmp(hexstr, "no", 2) ||
	    (len == 1 && hexstr[0] == '-')) {
		len = 0;
	} else {
		if (len % 2 != 0) {
			snprintf(hex_buf, sizeof(hex_buf), "0%s", hexstr);
			hexstr = hex_buf;
			len++;
		}
		if (len / 2 > alloc_size || hex2bin(data, hexstr, len / 2) < 0) {
			class_err("%s: invalid hex payload '%s'\n", __func__, hexstr);
			mutex_unlock(&adapter_dev->ops_lock);
			return -EINVAL;
		}
	}

	class_info("%s: cmd: %d, final count = %zu\n", __func__, cmd, len / 2);
	if (adapter_dev->ops && adapter_dev->ops->request_vdm_cmd)
		adapter_dev->ops->request_vdm_cmd(adapter_dev, cmd, data, len / 2);
				    ...
				    char *buf)
{
	...
	case USBPD_UVDM_REVERSE_AUTHEN:
		return snprintf(buf, PAGE_SIZE, "%d,%d\n", cmd, adapter_dev->vdm_data.reauth);
				    ...
				    const char *buf, size_t size)
{
	...
	if (kstrtobool(buf, &val)) {
		adapter_dev->verify_process = false;
		return -EINVAL;
	}
	...
	if (adapter_dev->ops && adapter_dev->ops->set_pd_verify_process)
		adapter_dev->ops->set_pd_verify_process(adapter_dev, val);
				   ...
				   const char *buf, size_t size)
{
	...
	if (adapter_dev->verifed && adapter_dev->ops && adapter_dev->ops->get_cap)
			       ...
			       char *buf)
{
	...
	if (adapter_dev->ops && adapter_dev->ops->get_power_role)
				  ...
				  char *buf)
{
	...
	if (adapter_dev->ops && adapter_dev->ops->get_current_state)
		adapter_dev->ops->get_current_state(adapter_dev);

	class_info("%s: current_state is %d\n", __func__, adapter_dev->current_state);

	if (adapter_dev->current_state >= ARRAY_SIZE(usbpd_state_strings))
			  ...
			  char *buf)
{
	...
	if (adapter_dev->ops && adapter_dev->ops->get_pdos)
...
{
	struct adapter_device *adapter_dev = to_adapter_device(dev);
	srcu_cleanup_notifier_head(&adapter_dev->evt_nh);
	mutex_destroy(&adapter_dev->ops_lock);
	kfree(adapter_dev);
}
					       ...
					       const char *alias_name)
{
	...
	/* Copy alias_name */
	if (alias_name)
		adapter_dev->alias_name = alias_name;

	rc = device_register(&adapter_dev->dev);
	if (rc) {
		put_device(&adapter_dev->dev);
		return ERR_PTR(rc);
	}
	adapter_dev->ops = ops;
	return adapter_dev;
...
subsys_initcall_sync(adapter_class_init);
