		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (!data) {
		class_info("%s: allocation failed\n", __func__);
		return -ENOMEM;
	}
	class_info("%s: kzalloc %s ok.\n", __func__, in_interrupt() ? "atomic" : "kernel");

	/* bound the %s scanned token to avoid overflow of local buffer */
	sscanf_ret = sscanf(buf, "%d,%63s", &cmd, buffer);
	if (sscanf_ret <= 0) {
		/* nothing parsable */
		class_info("%s: sscanf failed or no input\n", __func__);
		kfree(data);
		return -EINVAL;
	}

	/* If only cmd parsed (no token), ensure buffer is empty string */
	if (sscanf_ret == 1)
		buffer[0] = '\0';
	...
	/* Convert hex string into data (StringToHex unchanged) */
	StringToHex(buffer, data, &count);

	/* Defensive cap: ensure we don't trust StringToHex to not exceed our allocation */
	if (count > alloc_size)
		count = alloc_size;

	class_info("%s: count = %u\n", __func__, count);
