...
int dpm_check_supported_modes(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(svdm_svid_ops); i++) {
		...
		pr_info("%s: SVDM supported mode [%d]: name = %s, svid = 0x%x\n",
			__func__, i, svdm_svid_ops[i].name, svdm_svid_ops[i].svid);
	}
...
static void pd_dpm_update_pdos_flags(struct pd_port *pd_port, u32 pdo)
{
	u16 dpm_flags = pd_port->pe_data.dpm_flags & ~DPM_FLAGS_RESET_PARTNER_MASK;
...
void pd_dpm_inform_cable_id(struct pd_port *pd_port, bool src_startup)
{
	...
	u32 *payload = pd_get_msg_vdm_data_payload(pd_port);
	struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;

	if (payload) {
		memcpy(pd_port->pe_data.cable_vdos,
		       payload,
...
static void dpm_build_sink_pdo_info(struct dpm_pdo_info_t *sink,
				    u8 type, int request_v, int request_i)
{
	...
	sink->vmax = request_v;
	sink->vmin = request_v;
	sink->uw = (u64)request_v * request_i;
	sink->ma = request_i;
}
...
static bool dpm_build_request_info_with_new_src_cap(struct pd_port *pd_port,
						    ...
						    u8 charging_policy)
{
	u8 sel = pd_port->pe_data.selected_cap;
	struct dpm_pdo_info_t sink, source;

	if (sel == 0 || sel > src_cap->nr)
		return false;
	...
	return dpm_find_match_req_info(req_info, &sink,
			src_cap->nr, src_cap->pdos, -1, charging_policy);
}
...
static int pps_request_thread_fn(void *data)
{
	...
	while (true) {
		...
		if (kthread_should_stop()) {
			dev_notice(&tcpc->dev, "%s exits\n", __func__);
			break;
		}

		if (!ret) /* timed out, re-check the wait condition */
			continue;
...
void pd_dpm_start_pps_request_thread(struct pd_port *pd_port, bool en)
{
	...
	atomic_set(&pd_port->pps_request, en);
				       ...
				       u8 charging_policy)
{
	...
	for (i = 0; i < snk_cap->nr; i++) {
		...
		find_cap = dpm_find_match_req_info(req_info, &sink,
...
{
	int i;
	u8 charging_policy = pd_port->dpm_charging_policy;
	...
	if (pd_event_data_msg_match(pd_event, PD_DATA_SOURCE_CAP) &&
		...
		if (dpm_build_request_info_with_new_src_cap(pd_port, req_info,
							    src_cap, charging_policy))
			return true;
	}
...
static void dpm_update_request_i_new(struct pd_port *pd_port,
				     struct dpm_rdo_info_t *req_info)
{
...
static void dpm_update_request_bat(struct pd_port *pd_port,
				   struct dpm_rdo_info_t *req_info, u32 flags)
{
	u32 mw_max, mw_op;
	u32 vmin_safe = req_info->vmin ? req_info->vmin : 100; /* clamp 0 to 5V (100 * 50mV) */

	pd_port->request_i_max = req_info->max_uw / vmin_safe;
	pd_port->request_i_op = req_info->oper_uw / vmin_safe;
...
static void dpm_update_request_not_bat(struct pd_port *pd_port,
				       struct dpm_rdo_info_t *req_info, u32 flags)
{
...
static void dpm_update_request(struct pd_port *pd_port,
			       struct dpm_rdo_info_t *req_info)
{
	u32 flags = 0;
	struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;
			      ...
			      struct tcp_dpm_pd_request *pd_req)
{
	...
	if (pd_port->request_apdo &&
	    (sink.vmin == pd_port->request_v && sink.ma == pd_port->request_i))
...
static u8 pd_dpm_build_rdo(struct pd_port *pd_port)
{
	bool find_cap = false;
	struct dpm_rdo_info_t req_info;
	struct pd_port_power_caps *snk_cap = &pd_port->local_snk_cap;
...
void pd_dpm_snk_evaluate_caps(struct pd_port *pd_port)
{
	...
	PD_WARN_ON(!pd_get_msg_data_payload(pd_port));
...
static bool dpm_evaluate_request(struct pd_port *pd_port,
				 u32 rdo, u8 rdo_pos)
{
	u32 pdo;
	u32 sink_v;
...
void pd_dpm_src_evaluate_request(struct pd_port *pd_port)
{
	...
	u32 *payload = pd_get_msg_data_payload(pd_port);
	struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;
...
{
	tcpci_source_vbus(pd_port->tcpc, TCP_VBUS_CTRL_HRESET,
...
static bool dpm_ufp_update_svid_data_enter_mode(struct pd_port *pd_port,
						u16 svid, u8 ops)
{
...
static bool dpm_ufp_update_svid_data_exit_mode(struct pd_port *pd_port,
					       u16 svid, u8 ops)
{
	u8 i;
...
{
	struct svdm_svid_data *svid_data;
	u16 svid = dpm_vdm_get_svid(pd_port);

	svid_data = dpm_get_svdm_svid_data(pd_port, svid);

	PD_WARN_ON(!svid_data);
...
{
	u16 svid = dpm_vdm_get_svid(pd_port);
	bool ack = dpm_get_svdm_svid_data(pd_port, svid);
...
static void dpm_dfp_update_partner_id(struct pd_port *pd_port,
				      u32 *payload)
{
#ifdef CONFIG_USB_PD_KEEP_PARTNER_ID
	u8 cnt = pd_get_msg_vdm_data_count(pd_port);
	u32 size = sizeof(u32) * (cnt);
...
static void dpm_dfp_update_svid_data_exist(struct pd_port *pd_port,
					   u16 svid)
{
	u8 k;
	struct svdm_svid_data *svid_data;
...
static void dpm_dfp_update_svid_data_modes(struct pd_port *pd_port,
					   u16 svid, u32 *mode_list, u8 count)
{
	u8 i;
	struct svdm_svid_data *svid_data;
	struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;
	...
	if (count != 0)
		memcpy(svid_data->remote_mode.mode_vdo, mode_list, sizeof(u32) * count);
}

static void dpm_dfp_update_svid_enter_mode(struct pd_port *pd_port,
					   u16 svid, u8 ops)
{
	struct svdm_svid_data *svid_data;
	struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;
	...
	tcpci_enter_mode(tcpc, svid_data->svid, ops, svid_data->remote_mode.mode_vdo[ops]);
}

static void dpm_dfp_update_svid_data_exit_mode(struct pd_port *pd_port,
					       u16 svid, u8 ops)
{
	u8 i;
	bool modal_operation;
...
static int dpm_dfp_consume_svids(struct pd_port *pd_port,
				 u32 *svid_list, u8 count)
{
...
{
	u8 ops = dpm_vdm_get_ops(pd_port);
	u16 svid = dpm_vdm_get_svid(pd_port);
	u8 expected_ops = pd_port->mode_obj_pos;
	u16 expected_svid = pd_port->mode_svid;
	struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;
...
{
	u8 ops __maybe_unused = dpm_vdm_get_ops(pd_port);
	u16 svid = dpm_vdm_get_svid(pd_port);
	struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;
...
void pd_dpm_ufp_recv_uvdm(struct pd_port *pd_port)
{
	...
	if (!pd_get_msg_data_payload(pd_port)) {
...
{
	u16 svid;
	u16 expected_svid = pd_port->uvdm_svid;
...
{
	const u32 reaction_clear = DPM_REACTION_GET_SINK_CAP | DPM_REACTION_ATTEMPT_GET_FLAG;
	struct pd_event *pd_event = pd_get_curr_pd_event(pd_port);
...
{
	u32 reaction_clear = DPM_REACTION_GET_SOURCE_CAP | DPM_REACTION_ATTEMPT_GET_FLAG;
	struct pd_event *pd_event = pd_get_curr_pd_event(pd_port);
	struct pd_port_power_caps *src_cap = &pd_port->pe_data.remote_src_cap;
...
void pd_dpm_prs_evaluate_swap(struct pd_port *pd_port, u8 role)
{
	...
	bool check_src = (pd_port->dpm_caps & DPM_CAP_PR_SWAP_CHECK_GP_SRC) ? true : false;
	bool check_snk = (pd_port->dpm_caps & DPM_CAP_PR_SWAP_CHECK_GP_SNK) ? true : false;
...
{
	/* iSnkSwapStdby : 2.5mA */
	tcpci_sink_vbus(pd_port->tcpc, TCP_VBUS_CTRL_PR_SWAP,
			TCPC_VBUS_SINK_0V, 0);
}
...
void pd_dpm_prs_enable_power_source(struct pd_port *pd_port, bool en)
{
	...
	tcpci_source_vbus(pd_port->tcpc, TCP_VBUS_CTRL_PR_SWAP,
			  vbus_level, -1);
...
static const u32 c_invalid_bsdo = BSDO(0xffff, BSDO_BAT_INFO_INVALID_REF);
...
int pd_dpm_send_battery_status(struct pd_port *pd_port)
{
	...
	return pd_send_sop_data_msg(pd_port, PD_DATA_BAT_STATUS,
			PD_BSDO_SIZE, bsdo);
}
...
int pd_dpm_send_country_info(struct pd_port *pd_port)
{
	...
	struct pd_country_info cidb;
	struct pd_country_authority *ca = pd_port->country_info;
	u32 *pccdo = pd_get_msg_data_payload(pd_port);
	u16 cc = CCDO_COUNTRY_CODE(*pccdo);
...
{
	u32 *data = pd_get_msg_data_payload(pd_port);
	struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;
...
{
	u32 ado = pd_port->pe_data.local_alert;
	struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;
...
{
	u32 reactions = DPM_REACTION_CAP_ALWAYS;
...
#ifdef CONFIG_USB_PD_ATTEMPT_ENTER_MODE
	dpm_reaction_set(pd_port, DPM_REACTION_DISCOVER_ID | DPM_REACTION_DISCOVER_SVID);
#endif	/* CONFIG_USB_PD_ATTEMPT_ENTER_MODE */
...
static bool dpm_register_svdm_ops(struct pd_port *pd_port,
				  struct svdm_svid_data *svid_data,
...
int pd_dpm_core_init(struct pd_port *pd_port)
{
	int i, j, err;
	bool ret;
	u8 svid_ops_nr = ARRAY_SIZE(svdm_svid_ops);
	struct tcpc_device *tcpc = pd_port->tcpc;

	pd_port->svid_data = kcalloc(svid_ops_nr,
				     sizeof(struct svdm_svid_data),
				     GFP_KERNEL);
	...
	if (IS_ERR_OR_NULL(pd_port->pps_request_wake_lock)) {
		dev_err(&tcpc->dev, "%s: register pps_request wake lock fail\n",
			__func__);
		err = IS_ERR(pd_port->pps_request_wake_lock) ?
		      PTR_ERR(pd_port->pps_request_wake_lock) : -ENOMEM;
		goto err_free_svid_data;
	}
	...
	if (IS_ERR(pd_port->pps_request_task)) {
		err = PTR_ERR(pd_port->pps_request_task);
		dev_err(&tcpc->dev, "%s: create pps_request thread fail(%d)\n",
			__func__, err);
		goto err_unregister_wake_lock;
	}
#endif	/* CONFIG_USB_PD_REV30 */
