...
#define VDM_CMD_FLAG_CABLE_CMD				BIT(0)
#define VDM_CMD_FLAG_SEND_BY_UFP			BIT(1)
#define VDM_CMD_FLAG_SEND_BY_DFP			BIT(2)
#define VDM_CMD_FLAG_RECV_BY_UFP			BIT(3)
#define VDM_CMD_FLAG_RECV_BY_DFP			BIT(4)
#define VDM_CMD_FLAG_PD30_DUPLEX			BIT(5)

struct vdm_state_transition {
	u8	vdm_cmd;
	u8	vdm_init_state;
	u8	vdm_request_state;
	u8	vdm_cmd_flags;
};
...
#define VDM_DFP_CMD_REACTION(cmd, init, req)	\
	VDM_CMD_REACTION(cmd, init, req,	\
		VDM_CMD_FLAG_SEND_BY_DFP |	\
		VDM_CMD_FLAG_RECV_BY_UFP)

#define VDM_UFP_CMD_REACTION(cmd, init, req)	\
	VDM_CMD_REACTION(cmd, init, req,	\
		VDM_CMD_FLAG_SEND_BY_UFP |	\
		VDM_CMD_FLAG_RECV_BY_DFP)
...
static bool pd_vdm_state_transit_rx(struct pd_port *pd_port,
...
static bool pd_vdm_state_transit_tx(struct pd_port *pd_port,
				    u8 vdm_cmdt,
				    const struct vdm_state_transition *state_transition)
{
	int ret_code = TCP_DPM_RET_VDM_NAK;
	u8 curr_state = state_transition->vdm_request_state;
	u8 next_state = curr_state + 1;
...
{
	u8 shift = 1;
	u8 vdm_cmd_flags = state_transition->vdm_cmd_flags;
...
static bool pd_vdm_state_transit(struct pd_port *pd_port,
				 u8 vdm_cmdt,
				 const struct vdm_state_transition *state_transition)
{
	u8 curr_state;
...
static bool pe_check_vdm_state_transit_valid(struct pd_port *pd_port,
					     u8 transit_type, u8 *vdm_cmdt,
					     const struct vdm_state_transition *state_transition)
{
	u8 curr_state;
	u8 vdm_cmd;
...
static bool pd_make_vdm_state_transit(struct pd_port *pd_port, u8 transit_type)
{
	int i;
	bool check_tx;
	u8 vdm_cmdt;
	...
	for (i = 0; i < ARRAY_SIZE(pe_vdm_state_reactions); i++) {
		...
		if (!pe_check_vdm_state_transit_valid(pd_port, transit_type,
						      &vdm_cmdt, state_transition))
			continue;

		if (check_tx)
			return vdm_is_state_transition_available(pd_port,
					false, state_transition);
		else
			return pd_vdm_state_transit(pd_port,
					vdm_cmdt, state_transition);
	}
...
static bool pd_make_vdm_state_transit_sop(struct pd_port *pd_port)
{
	return pd_make_vdm_state_transit(pd_port, VDM_STATE_TRANSIT_SOP_CMD);
}

static bool pd_make_vdm_state_transit_cable(struct pd_port *pd_port)
{
	return pd_make_vdm_state_transit(pd_port, VDM_STATE_TRANSIT_SOP_PRIME_CMD);
}

static bool pd_make_vdm_state_transit_nak(struct pd_port *pd_port)
{
...
static bool pd_ufp_u_auto_send_attention(struct pd_port *pd_port)
{
...
static bool pd_process_ctrl_msg(struct pd_port *pd_port,
...
static bool pd_process_uvdm(struct pd_port *pd_port,
			    struct pd_event *pd_event)
{
	struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;
...
static const char *assign_vdm_cmd_name(u8 cmd)
{
...
static const char *assign_vdm_dp_cmd_name(u8 cmd)
{
...
static void print_vdm_msg(struct pd_port *pd_port,
			  struct pd_event *pd_event)
{
#if (PE_EVT_INFO_VDM_DIS == 0)
	u8 cmd;
	...
	u32 vdm_hdr = pd_port->curr_vdm_hdr;
	struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;
...
static bool pd_process_sop_vdm(struct pd_port *pd_port,
			       struct pd_event *pd_event)
{
	struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;
...
static bool pd_process_sop_prime_vdm(struct pd_port *pd_port,
				     struct pd_event *pd_event)
{
	return pd_make_vdm_state_transit_cable(pd_port);
}

static bool pd_check_svid_valid(struct pd_port *pd_port, u16 svid)
{
	switch (svid) {
	case USB_SID_PD:
	case USB_VID_MQP:
...
static bool pd_process_data_msg(struct pd_port *pd_port,
				struct pd_event *pd_event)
{
	bool ret = false;
	u32 vdm_hdr;
...
static bool pd_process_dpm_msg(struct pd_port *pd_port,
			       struct pd_event *pd_event)
{
...
static bool pd_process_hw_msg(struct pd_port *pd_port,
			      struct pd_event *pd_event)
{
	struct pe_data *pe_data = &pd_port->pe_data;
	struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;
...
static bool pd_process_pe_msg(struct pd_port *pd_port,
			      struct pd_event *pd_event)
{
	switch (pd_event->msg) {
...
static bool pd_process_timer_msg(struct pd_port *pd_port,
				 struct pd_event *pd_event)
{
	switch (pd_event->msg) {
...
static const u8 tcp_vdm_evt_init_state[] = {
...
static bool pd_process_tcp_cable_event(struct pd_port *pd_port,
				       struct pd_event *pd_event)
{
	bool ret;
	int tcp_ret;
...
static u32 tcpc_update_bits(u32 var, u32 update, u32 mask)
{
	return (var & (~mask)) | (update & mask);
}

static void pd_parse_tcp_dpm_evt_svdm(struct pd_port *pd_port)
{
	struct tcp_dpm_svdm_data *svdm_data = &pd_port->tcp_event.tcp_dpm_data.svdm_data;

	pd_port->mode_svid = svdm_data->svid;
	pd_port->mode_obj_pos = svdm_data->ops;
}

static void pd_parse_tcp_dpm_evt_dp_status(struct pd_port *pd_port)
{
	struct tcp_dpm_dp_data *dp_data_tcp = &pd_port->tcp_event.tcp_dpm_data.dp_data;
	struct dp_data *dp_data = pd_get_dp_data(pd_port);
...
static void pd_parse_tcp_dpm_evt_dp_config(struct pd_port *pd_port)
{
	struct tcp_dpm_dp_data *dp_data_tcp = &pd_port->tcp_event.tcp_dpm_data.dp_data;
	struct dp_data *dp_data = pd_get_dp_data(pd_port);
...
#ifdef CONFIG_USB_PD_CUSTOM_VDM
static void pd_parse_tcp_dpm_evt_uvdm(struct pd_port *pd_port)
{
	struct tcp_dpm_custom_vdm_data *vdm_data = &pd_port->tcp_event.tcp_dpm_data.vdm_data;

	pd_port->uvdm_cnt = vdm_data->cnt;
	pd_port->uvdm_wait_resp = vdm_data->wait_resp;
	memcpy(pd_port->uvdm_data, vdm_data->vdos, sizeof(u32) * vdm_data->cnt);
...
static void pd_parse_tcp_dpm_evt_from_tcpm(struct pd_port *pd_port,
					   struct pd_event *pd_event)
{
	switch (pd_event->msg) {
...
static bool pd_check_tcp_msg_valid(struct pd_port *pd_port, u8 new_state)
{
	return pd_make_vdm_state_transit(pd_port, new_state);
}

static bool pd_process_tcp_msg(struct pd_port *pd_port,
			       struct pd_event *pd_event)
{
	u8 new_state;
	struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;
