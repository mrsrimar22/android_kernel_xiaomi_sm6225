				  ...
				  char *buf)
{
	...
	case TCPC_DESC_VENDOR_ID:
		dev_info(dev, "%s: cc_vendor_id = %x\n", __func__, tcpc->desc.pid);
		if (tcpc->desc.pid == 0x1711)
...
static void tcpc_update_battery_status(struct work_struct *work)
{
	struct tcpc_device *tcpc = container_of(work, struct tcpc_device, bat_update_work);
	union power_supply_propval value;
	int ret;

	if (!tcpc || !tcpc->bat_psy)
		return;
...
static int bat_nb_call_func(struct notifier_block *nb,
			    unsigned long val, void *v)
{
	...
	if (!tcpc) {
		TCPC_ERR("tcpc is null\n");
		return NOTIFY_OK;
	}

	if (val == PSY_EVENT_PROP_CHANGED && psy == tcpc->bat_psy)
		schedule_work(&tcpc->bat_update_work);

	return NOTIFY_OK;
}
#endif	/* CONFIG_USB_PD_REV30 */

static int tcpc_event_resources_init(struct tcpc_device *tcpc)
{
#ifdef CONFIG_USB_POWER_DELIVERY
	int ret = 0;
	...
	if (!test_and_set_bit(TCPC_RES_EVENT_INIT, &tcpc->res_owner_flags)) {
		ret = tcpci_event_init(tcpc);
		if (ret < 0) {
			tcpci_unlock_typec(tcpc);
			dev_err(&tcpc->dev, "%s: event_init fail: %d\n", __func__, ret);
			return ret;
		}
	}
	...
	if (!tcpc->usb_psy) {
		...
		if (!tcpc->usb_psy) {
			tcpci_unlock_typec(tcpc);
			dev_err(&tcpc->dev, "%s: get usb psy fail\n", __func__);
			return -ENODEV;
		}
	}
	...
	dev_info(&tcpc->dev, "%s: typec attach new = %d\n",
	...
	if (!tcpc->bat_psy) {
		dev_err(&tcpc->dev, "%s: get battery psy fail\n", __func__);
		goto err_get_bat_psy;
	}

	tcpc->charging_status = BSDO_BAT_INFO_IDLE;
	tcpc->bat_soc = 0;
	INIT_WORK(&tcpc->bat_update_work, tcpc_update_battery_status);
	tcpc->bat_nb.notifier_call = bat_nb_call_func;
	tcpc->bat_nb.priority = 0;
	ret = power_supply_reg_notifier(&tcpc->bat_nb);
	if (ret < 0) {
		dev_err(&tcpc->dev, "%s: register power supply notifier fail(%d)\n",
			__func__, ret);
		goto err_reg_bat_nb;
	}

	power_supply_changed(tcpc->bat_psy);
	return 0;
...
static void tcpc_event_resources_deinit(struct tcpc_device *tcpc)
{
#ifdef CONFIG_USB_PD_REV30
	if (tcpc->bat_nb.notifier_call)
		power_supply_unreg_notifier(&tcpc->bat_nb);

	cancel_work_sync(&tcpc->bat_update_work);
...
{
	struct tcp_notifier_block_wrapper *nb_wrapper = container_of(nb,
		struct tcp_notifier_block_wrapper, stub_nb);
...
static int __register_tcp_dev_notifier(struct tcpc_device *tcp_dev,
				       struct notifier_block *nb, u8 idx)
{
	struct tcp_notifier_block_wrapper *nb_wrapper;
	int ret = 0;

	nb_wrapper = kzalloc(sizeof(*nb_wrapper), GFP_KERNEL);
	if (!nb_wrapper)
		return -ENOMEM;
	...
	nb_wrapper->stub_nb.priority = nb->priority;
	ret = srcu_notifier_chain_register(tcp_dev->evt_nh + idx,
					   &nb_wrapper->stub_nb);
	if (ret < 0) {
		kfree(nb_wrapper);
		return ret;
	...
	list_add_tail(&nb_wrapper->list, &tcp_dev->mr_list);
...
static struct tcp_notifier_block_wrapper *
__find_and_remove_wrapper(struct tcpc_device *tcp_dev,
			  struct notifier_block *nb, u8 idx)
{
	struct tcp_notifier_block_wrapper *wrapper, *found = NULL;

	mutex_lock(&tcp_dev->mr_lock);
	list_for_each_entry(wrapper, &tcp_dev->mr_list, list) {
		if (wrapper->action_nb == nb && wrapper->idx == idx) {
					 ...
					 struct notifier_block *nb, u8 idx)
{
	...
	nb_wrapper = __find_and_remove_wrapper(tcp_dev, nb, idx);
	if (nb_wrapper) {
		ret = srcu_notifier_chain_unregister(tcp_dev->evt_nh + idx,
						     &nb_wrapper->stub_nb);
		kfree(nb_wrapper);
		return ret;
	}
...
static int tcpc_device_irq_enable(struct tcpc_device *tcpc)
{
	...
	if (!tcpc->ops->init) {
		dev_notice(&tcpc->dev,
			   "%s: Please implment tcpc ops init function\n", __func__);
		return -EINVAL;
	}
	...
	if (ret < 0) {
		tcpci_unlock_typec(tcpc);
		dev_err(&tcpc->dev, "%s: tcpc init fail\n", __func__);
		return ret;
	}
	...
	tcpci_unlock_typec(tcpc);
	if (ret < 0) {
		dev_err(&tcpc->dev, "%s: tcpc typec init fail\n", __func__);
		return ret;
	}

	ret = tcpc_event_resources_init(tcpc);
	if (ret < 0) {
		dev_err(&tcpc->dev, "%s: tcpc event resources init fail(%d)\n",
			__func__, ret);
		return ret;
	}

	dev_info(&tcpc->dev, "%s: tcpc irq enable OK!\n", __func__);
	return 0;
}

int register_tcpc_late_sync_notifier(struct tcpc_device *tcpc,
				     struct notifier_block *nb)
{
	if (!tcpc)
		return -EINVAL;

	return srcu_notifier_chain_register(&tcpc->late_sync_notifier, nb);
}
EXPORT_SYMBOL(register_tcpc_late_sync_notifier);

int unregister_tcpc_late_sync_notifier(struct tcpc_device *tcpc,
				       struct notifier_block *nb)
{
	if (!tcpc)
		return -EINVAL;

	return srcu_notifier_chain_unregister(&tcpc->late_sync_notifier, nb);
}
EXPORT_SYMBOL(unregister_tcpc_late_sync_notifier);

int tcpci_late_sync_init(struct tcpc_device *tcpc)
{
	int ret;
	...
	srcu_notifier_call_chain(&tcpc->late_sync_notifier, 0, tcpc);
...
EXPORT_SYMBOL(tcpci_late_sync_init);

/*
 * Lock lifecycle is centralised here, in the tcpc core.  All locks that live
 * in struct tcpc_device / struct pd_port are allocated by
 * tcpc_lock_resources_init() before any subsystem (timer, PD core) comes up,
 * and released once by tcpc_lock_resources_deinit() from the device .release
 * path.  Subsystem init/deinit functions no longer init/destroy these locks,
 * so a subsystem cannot leave a lock half-owned or double-destroy it.
 */
static void tcpc_lock_resources_init(struct tcpc_device *tcpc)
{
	...
	spin_lock_init(&tcpc->fifo_lock);
#endif /* CONFIG_TCPC_NOTIFICATION_NON_BLOCKING */
#ifdef CONFIG_USB_POWER_DELIVERY
	mutex_init(&tcpc->pd_port.pd_lock);
#ifdef CONFIG_USB_PD_BLOCK_TCPM
	mutex_init(&tcpc->pd_port.tcpm_bk_lock);
#endif /* CONFIG_USB_PD_BLOCK_TCPM */
#endif /* CONFIG_USB_POWER_DELIVERY */
}

static void tcpc_lock_resources_deinit(struct tcpc_device *tcpc)
{
#ifdef CONFIG_USB_POWER_DELIVERY
#ifdef CONFIG_USB_PD_BLOCK_TCPM
	mutex_destroy(&tcpc->pd_port.tcpm_bk_lock);
#endif /* CONFIG_USB_PD_BLOCK_TCPM */
	mutex_destroy(&tcpc->pd_port.pd_lock);
#endif /* CONFIG_USB_POWER_DELIVERY */
...
{
	struct tcpc_device *tcpc = to_tcpc_device(dev);
	struct tcp_notifier_block_wrapper *wrapper, *tmp;
	int i;

	if (!tcpc) {
		dev_warn(dev, "%s: called with NULL tcpc\n", __func__);
		return;
	}
...
#ifdef CONFIG_USB_POWER_DELIVERY
	if (test_and_clear_bit(TCPC_RES_PD_CORE_INIT, &tcpc->res_owner_flags))
		pd_core_deinit(tcpc);
#endif	/* CONFIG_USB_POWER_DELIVERY */

#ifdef CONFIG_USB_POWER_DELIVERY
	if (test_and_clear_bit(TCPC_RES_EVENT_INIT, &tcpc->res_owner_flags)) {
		tcpc_event_resources_deinit(tcpc);
		tcpci_event_deinit(tcpc);
	}
#endif	/* CONFIG_USB_POWER_DELIVERY */

	if (test_and_clear_bit(TCPC_RES_TIMER_INIT, &tcpc->res_owner_flags))
		tcpci_timer_deinit(tcpc);

	tcpci_alert_deinit(tcpc);
	tcpci_notify_deinit(tcpc);

	mutex_lock(&tcpc->mr_lock);
	list_for_each_entry_safe(wrapper, tmp, &tcpc->mr_list, list) {
		dev_warn(dev, "%s: notifier idx=%u leaked, cleaning up\n",
			 __func__, wrapper->idx);
		list_del(&wrapper->list);
		srcu_notifier_chain_unregister(tcpc->evt_nh + wrapper->idx,
					       &wrapper->stub_nb);
		kfree(wrapper);
	}
	mutex_unlock(&tcpc->mr_lock);

	srcu_cleanup_notifier_head(&tcpc->late_sync_notifier);
	for (i = 0; i < TCP_NOTIFY_IDX_NR; i++)
		srcu_cleanup_notifier_head(&tcpc->evt_nh[i]);

	device_init_wakeup(dev, false);
	tcpc_lock_resources_deinit(tcpc);
}
					 ...
					 void *drv_data)
{
	...
	dev_info(parent, "%s: register tcpc device (%s)\n",
	...
	tcpc_lock_resources_init(tcpc);
	INIT_LIST_HEAD(&tcpc->mr_list);
	for (i = 0; i < TCP_NOTIFY_IDX_NR; i++)
		srcu_init_notifier_head(&tcpc->evt_nh[i]);
	srcu_init_notifier_head(&tcpc->late_sync_notifier);

	device_set_of_node_from_dev(&tcpc->dev, parent);

	ret = tcpci_notify_init(tcpc);
	if (ret < 0) {
		dev_err(parent, "%s: notify init fail(%d)\n", __func__, ret);
		goto err_notify_init;
	}

	ret = device_register(&tcpc->dev);
	if (ret) {
		dev_err(parent, "%s: device register fail(%d)\n", __func__, ret);
		goto err_device_register;
	}

	device_init_wakeup(&tcpc->dev, true);

	ret = tcpci_alert_init(tcpc);
	if (ret < 0) {
		dev_err(&tcpc->dev, "%s: alert init fail(%d)\n", __func__, ret);
		goto err_alert_init;
	}

	if (!test_and_set_bit(TCPC_RES_TIMER_INIT, &tcpc->res_owner_flags)) {
		ret = tcpci_timer_init(tcpc);
		if (ret < 0) {
			dev_err(&tcpc->dev, "%s: timer init fail(%d)\n", __func__, ret);
			clear_bit(TCPC_RES_TIMER_INIT, &tcpc->res_owner_flags);
			goto err_timer_init;
		}
	}

#ifdef CONFIG_USB_POWER_DELIVERY
	if (!test_and_set_bit(TCPC_RES_PD_CORE_INIT, &tcpc->res_owner_flags)) {
		ret = pd_core_init(tcpc);
		if (ret < 0) {
			dev_err(&tcpc->dev, "%s: pd core init fail(%d)\n", __func__, ret);
			clear_bit(TCPC_RES_PD_CORE_INIT, &tcpc->res_owner_flags);
			goto err_pd_core_init;
		}
	}
#endif	/* CONFIG_USB_POWER_DELIVERY */
	...
	if (ret < 0)
		dev_err(&tcpc->dev, "%s: dual role usb init fail\n", __func__);
...
err_alert_init:
	/*
	 * device_register() succeeded: device_unregister() will call
	 * .release (tcpc_device_release) which cleans up notify, SRCU
	 * notifier heads, and mutexes.  Do NOT manually call
	 * tcpci_notify_deinit / srcu_cleanup / mutex_destroy here
	 * .release already handles them.
	 */
...
err_device_register:
	/*
	 * device_register() failed: put_device() triggers .release
	 * which cleans up notify, SRCU notifier heads, and mutexes.
	 */
	put_device(&tcpc->dev);
	return ERR_PTR(ret);
...
err_notify_init:
	/*
	 * device_register() was never called, .release will not run:
...
#ifdef CONFIG_DUAL_ROLE_USB_INTF
	tcpc_dual_role_phy_deinit(tcpc);
#endif	/* CONFIG_DUAL_ROLE_USB_INTF */
...
#ifdef CONFIG_USB_POWER_DELIVERY
	if (test_and_clear_bit(TCPC_RES_EVENT_INIT, &tcpc->res_owner_flags)) {
		/* Release psy refs and bat notifier before stopping the kthread. */
		tcpc_event_resources_deinit(tcpc);
		tcpci_event_deinit(tcpc);
	}
#endif	/* CONFIG_USB_POWER_DELIVERY */
...
static int __init tcpc_class_init(void)
{
	...
	if (IS_ERR(tcpc_class)) {
		pr_err("%s: Unable to create tcpc class, errno = %ld\n",
