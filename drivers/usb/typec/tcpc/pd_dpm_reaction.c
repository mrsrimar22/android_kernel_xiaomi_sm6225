...
#define DPM_REACTION_COND_ALWAYS	BIT(0)
#define DPM_REACTION_COND_UFP_ONLY	BIT(1)
#define DPM_REACTION_COND_DFP_ONLY	BIT(2)
#define DPM_REACTION_COND_PD30		BIT(3)
...
#define DPM_REACTION_COND_CHECK_ONCE			BIT(5)
#define DPM_REACTION_COND_ONE_SHOT			BIT(6)
#define DPM_REACTION_COND_LIMITED_RETRIES		BIT(7)
...
static u8 dpm_reaction_ufp_flow_delay(struct pd_port *pd_port)
{
...
static u8 dpm_reaction_dfp_flow_delay(struct pd_port *pd_port)
{
	struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;
...
static u8 dpm_reaction_vconn_stable_delay(struct pd_port *pd_port)
{
	struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;
...
static u8 dpm_reaction_get_source_cap_ext(struct pd_port *pd_port)
{
...
static u8 dpm_reaction_get_sink_cap(struct pd_port *pd_port)
{
	return TCP_DPM_EVT_GET_SINK_CAP;
}

static u8 dpm_reaction_get_source_cap(struct pd_port *pd_port)
{
	return TCP_DPM_EVT_GET_SOURCE_CAP;
}

static u8 dpm_reaction_attempt_get_flag(struct pd_port *pd_port)
{
	return TCP_DPM_EVT_GET_SINK_CAP;
}
...
static u8 dpm_reaction_request_pr_swap(struct pd_port *pd_port)
{
	u32 prefer_role = DPM_CAP_EXTRACT_PR_CHECK(pd_port->dpm_caps);
...
static u8 dpm_reaction_request_dr_swap(struct pd_port *pd_port)
{
	u32 prefer_role = DPM_CAP_EXTRACT_DR_CHECK(pd_port->dpm_caps);
...
static u8 dpm_reaction_dynamic_vconn(struct pd_port *pd_port)
{
...
static u8 dpm_reaction_request_vconn_source(struct pd_port *pd_port)
{
...
static u8 pd_dpm_reaction_discover_cable(struct pd_port *pd_port)
{
...
static u8 dpm_reaction_return_vconn_source(struct pd_port *pd_port)
{
	struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;
...
static u8 dpm_reaction_discover_id(struct pd_port *pd_port)
{
...
static u8 dpm_reaction_discover_svid(struct pd_port *pd_port)
{
...
static u8 dpm_reaction_mode_operation(struct pd_port *pd_port)
{
...
static u8 dpm_reaction_send_alert(struct pd_port *pd_port)
{
	u32 alert_urgent;
...
const u32 c_get_status_alert_type =
	ADO_ALERT_OCP | ADO_ALERT_OTP | ADO_ALERT_OVP |
	ADO_ALERT_OPER_CHANGED | ADO_ALERT_SRC_IN_CHANGED;

static u8 dpm_reaction_alert_status_changed(struct pd_port *pd_port)
{
...
static u8 dpm_reaction_alert_battry_changed(struct pd_port *pd_port)
{
	u8 i;
	u8 mask;
	u8 bat_change_i = 255;
	u8 bat_change_mask1, bat_change_mask2;
	...
	if (bat_change_mask1) {
		...
		for (i = 0; i < 4; i++) {
			...
			if (bat_change_mask2 & mask) {
				...
				pd_port->pe_data.remote_alert &= ~ADO_HOT_SWAP_BAT_SET(bat_change_mask2);
				break;
			}
		}
	}
...
static u8 dpm_reaction_handle_alert(struct pd_port *pd_port)
{
	u32 alert_type = ADO_ALERT_TYPE(pd_port->pe_data.remote_alert);
...
static u8 dpm_get_pd_connect_state(struct pd_port *pd_port)
{
...
static void dpm_check_vconn_highv_prot(struct pd_port *pd_port)
{
...
static u8 dpm_reaction_update_pe_ready(struct pd_port *pd_port)
{
	u8 state;
	struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;
...
typedef u8 (*dpm_reaction_fun)(struct pd_port *pd_port);
...
static u8 dpm_get_reaction_env(struct pd_port *pd_port)
{
	u8 conditions;
...
static bool dpm_check_reaction_busy(struct pd_port *pd_port,
...
static u8 dpm_check_reaction_available(struct pd_port *pd_port,
				       u8 env, const struct dpm_ready_reaction *reaction)
{
	int ret;
...
static bool dpm_check_clear_reaction(struct pd_port *pd_port,
				     const struct dpm_ready_reaction *reaction)
{
...
u8 pd_dpm_get_ready_reaction(struct pd_port *pd_port)
{
	u8 evt;
	u8 env;
	u32 clear_reaction = DPM_REACTION_CAP_READY_ONCE;
