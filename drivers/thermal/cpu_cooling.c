				 ...
				 unsigned long state)
{
	 ...
	 */
#ifdef CONFIG_ARCH_QCOM
	if (USE_LMH_DEV == 1 && cpufreq_cdev->plat_ops) {
...
void cpu_limits_set_level(unsigned int cpu, unsigned int max_freq)
{
	...
	mutex_lock(&cooling_list_lock);
	list_for_each_entry(cpufreq_cdev, &cpufreq_cdev_list, node) {
		if (sscanf(cpufreq_cdev->cdev->type,
			   "thermal-cpufreq-%u", &cdev_cpu) != 1)
			continue;
		...
		/*
		 * Map a requested max frequency to the lowest state whose
		 * frequency does not exceed it. get_state_freq() already
		 * handles both the EM table and the freq_table fallback.
		 */
		state = cpufreq_cdev->max_level;
		...
		cpufreq_cdev->cdev->ops->set_cur_state(cpufreq_cdev->cdev,
						       state);
		break;
	}
	mutex_unlock(&cooling_list_lock);
