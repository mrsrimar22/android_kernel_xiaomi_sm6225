 ...
 */
#define pr_fmt(fmt) "[st21nfc]: %s: " fmt, __func__
...
static irqreturn_t st21nfc_dev_irq_handler(int irq, void *dev_id)
{
	...
	if (device_may_wakeup(&st21nfc_dev->client->dev))
		pm_stay_awake(&st21nfc_dev->client->dev);
...
static int st21nfc_loc_set_polaritymode(struct st21nfc_device *st21nfc_dev, int mode)
{
	...
	/* irq_set_irq_type is sufficient — no free/re-request needed */
	ret = irq_set_irq_type(st21nfc_dev->client->irq, irq_type);
	if (ret) {
				       ...
				       bool is_ntf)
{
	...
	if (new_state == old_state) {
		if (st21nfc_dev->pw_states[ST21NFC_IDLE].last_entry != 0 ||
			...
			pr_err("Error: Switched from %s to %s!: %llx, ntf=%d\n",
	...
	pr_debug("Switching from %s to %s: %llx, ntf=%d\n",
		 st21nfc_power_state_name[old_state],
		 st21nfc_power_state_name[new_state],
		 current_time_ms, is_ntf);
...
{
	s64 current_time_ms = ktime_to_ms(ktime_get_boottime());
				       ...
				       char *buf, size_t count)
{
	...
	/* Use IS_ERR_OR_NULL to match probe's gpiod_pidle guard */
				...
				size_t count, loff_t *offset)
{
	...
	if (gpiod_get_value(st21nfc_dev->gpiod_irq) == 0) {
		pr_info("read called but no IRQ\n");
...
#ifdef ST54J_PWRSTATS
	if (st21nfc_dev->r_state_current == ST21NFC_HEADER) {
		/* Strip leading idle characters */
		while (idle < ret && st21nfc_dev->buffer[idle] == IDLE_CHARACTER)
			idle++;
		...
		/* Apply power stats filter if enough data */
		if (idle < HEADER_LENGTH) {
			st21nfc_power_stats_filter(st21nfc_dev, st21nfc_dev->buffer, ret);
			st21nfc_dev->r_state_current =
				(st21nfc_dev->r_state_current == ST21NFC_HEADER &&
				 ret > 2 && st21nfc_dev->buffer[2] > 0) ?
				ST21NFC_PAYLOAD : ST21NFC_HEADER;
		}
	}
#endif /* ST54J_PWRSTATS */
	...
	/* pm_relax is a no-op if pm_stay_awake wasn't called (e.g. no-IRQ path) */
	if (device_may_wakeup(&st21nfc_dev->client->dev))
		pm_relax(&st21nfc_dev->client->dev);
	return done;
}
			      ...
			      unsigned long arg)
{
	...
	case ST21NFC_LEGACY_SET_POLARITY_RISING:
		...
		ret = st21nfc_loc_set_polaritymode(st21nfc_dev, IRQF_TRIGGER_RISING);
		break;
	case ST21NFC_SET_POLARITY_HIGH:
	case ST21NFC_LEGACY_SET_POLARITY_HIGH:
		pr_info("### ST21NFC_SET_POLARITY_HIGH ###\n");
		ret = st21nfc_loc_set_polaritymode(st21nfc_dev, IRQF_TRIGGER_HIGH);
		break;
	case ST21NFC_PULSE_RESET:
	case ST21NFC_LEGACY_PULSE_RESET:
		pr_info("Double Pulse Request entry\n");
		if (!IS_ERR_OR_NULL(st21nfc_dev->gpiod_reset)) {
			...
			usleep_range(20000, 20100);
			gpiod_set_value(st21nfc_dev->gpiod_reset, 1);
			pr_info("Double Pulse Request done\n");
			if (st21nfc_st54spi_cb != NULL)
	...
	case ST21NFC_LEGACY_RECOVERY:
		...
		pr_info("Recovery Request entry\n");
		if (!IS_ERR_OR_NULL(st21nfc_dev->gpiod_reset)) {
			/*
			 * Need to free IRQ before using the pin as GPIO output.
			 * Non-devm free/request because lifecycle is manual here.
			 */
			if (st21nfc_dev->irq_is_attached) {
				free_irq(st21nfc_dev->client->irq, st21nfc_dev);
			...
			if (ret) {
				pr_err("gpiod_direction_output failed\n");
				ret = -ENODEV;
				break;
			}
			...
			usleep_range(10000, 10100);
			gpiod_set_value(st21nfc_dev->gpiod_reset, 1);
			pr_info("Recovery Pulse Request done\n");
		}
		usleep_range(20000, 20100);
		...
		/* Re-arm IRQ handler after recovery */
		st21nfc_dev->irq_enabled = true;
		ret = request_irq(st21nfc_dev->client->irq,
				  st21nfc_dev_irq_handler,
				  st21nfc_dev->polarity_mode,
				  st21nfc_dev->client->name,
				  st21nfc_dev);
		if (ret) {
			pr_err("request_irq after recovery failed\n");
			ret = -ENODEV;
			break;
		}
...
static u64 st21nfc_power_duration(struct st21nfc_device *data,
				  enum st21nfc_power_state pstate,
				  s64 current_time_ms)
{
	u64 duration = data->c_pw_states[pstate].duration;

	if (data->c_pw_current == pstate)
		duration += current_time_ms - data->c_pw_states[pstate].last_entry;
				...
				char *buf)
{
	...
	s64 current_time_ms;
	u64 idle_duration;
	u64 active_ce_duration;
	u64 active_rw_duration;
			 ...
			 const struct i2c_device_id *id)
{
	...
	if (IS_ERR_OR_NULL(st21nfc_dev->gpiod_reset)) {
		pr_err("Unable to request reset-gpios\n");
		ret = -ENODEV;
		goto err_acpi_gpios;
	}
	...
	if (IS_ERR_OR_NULL(st21nfc_dev->gpiod_pidle)) {
		pr_warn("[OPTIONAL] Unable to request pidle-gpios\n");
		ret = 0;
	} else {
		...
		if (ret) {
			pr_err("devm_request_irq for power stats idle failed\n");
			goto err_acpi_gpios;
		}

		ret = sysfs_create_file(&dev->kobj, &dev_attr_power_stats.attr);
		if (ret) {
			pr_err("sysfs_create_file for power stats failed\n");
			goto err_acpi_gpios;
		}

		mutex_init(&st21nfc_dev->pidle_mutex);
		/* IRQ WQ should not use UNBOUND */
		st21nfc_dev->st_p_wq = alloc_workqueue("st_pstate_work",
						       WQ_MEM_RECLAIM, 1);
		if (!st21nfc_dev->st_p_wq) {
			pr_err("failed to create workqueue\n");
			ret = -ENOMEM;
			goto err_sysfs_pidle;
		}
	...
	if (IS_ERR_OR_NULL(st21nfc_dev->gpiod_clkreq)) {
		pr_warn("[OPTIONAL] Unable to request clkreq-gpios\n");
		ret = 0;
	} else {
		if (!device_property_read_bool(dev, "st,clk_pinctrl")) {
			pr_info("[OPTIONAL] clk_pinctrl not set\n");
			st21nfc_dev->pinctrl_en = 0;
		} else {
			pr_info("[OPTIONAL] clk_pinctrl set\n");
		...
		if (ret < 0) {
			pr_err("st21nfc_clock_select failed\n");
			goto err_wq_pidle;
		}
	}
	...
	/* Non-devm request_irq: lifecycle managed manually across probe/remove/recovery */
	st21nfc_dev->irq_enabled = true;
	...
	pr_debug("debug irq_gpio=%d, client-irq=%d, pidle_gpio=%d\n",
...
err_wq_pidle:
	...
	/* fall through - also clean up sysfs and mutex */
err_sysfs_pidle:
	if (!IS_ERR_OR_NULL(st21nfc_dev->gpiod_pidle)) {
