...
static irqreturn_t max77729_irq_thread(int irq, void *data)
{
	...
	if ((irq_src & MAX77729_IRQSRC_USBC) && max77729->cc_booting_complete) {
		...
		pr_info("[%s]: %s: irq_reg, complete [%x], %x, %x, %x, %x, %x\n",
		...
		pr_info("[%s]: %s: dump_reg, %x, %x, %x, %x, %x, %x, %x, %x\n",
				MFD_DEV_NAME, __func__,
...
int max77729_irq_init(struct max77729_dev *max77729)
{
	...
	if (!max77729->irq_gpio) {
		dev_warn(max77729->dev, "%s: No interrupt specified.\n", __func__);
		max77729->irq_base = 0;
		return 0;
	}

	if (!max77729->irq_base) {
		dev_err(max77729->dev, "%s: No interrupt base specified.\n", __func__);
		return 0;
	}
