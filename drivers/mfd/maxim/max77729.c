...
static int of_max77729_dt(struct device *dev, struct max77729_platform_data *pdata)
{
	...
	if (!np_battery) {
		...
		/* release the reference returned by of_find_node_by_name */
		...
		const u8 *fw_bin, int fw_bin_len, int enforce_do)
{
	...
	pr_info("[%s]: %s: magic/%x/ major/%x/ minor/%x/ product_id/%x/ rev/%x/ id/%x/\n",
			MFD_DEV_NAME, __func__, fw_header->magic, fw_header->major, fw_header->minor, fw_header->product_id, fw_header->rev, fw_header->id);
...
retry:
	...
	if ((max77729->FW_Revision != fw_header->major) || (max77729->FW_Minor_Revision != fw_header->minor)) {
		...
		max77729->FW_Minor_Revision &= MINOR_VERSION_MASK;
		pr_info("[%s]: %s: Start FW updating (%02X.%02X)\n", MFD_DEV_NAME, __func__, max77729->FW_Revision, max77729->FW_Minor_Revision);
			      ...
			      const struct i2c_device_id *dev_id)
{
	...
	if (IS_ERR(max_pinctrl)) {
		...
		dev_err(max77729->dev, "No pinctrl config specified (NULL)\n");
		ret = -ENODEV;
		goto err_unregister_debug;
	}
	...
	if (IS_ERR(max_gpio_default)) {
		...
		ret = PTR_ERR(max_gpio_default);
		goto err_unregister_debug;
	} else if (!max_gpio_default) {
		dev_err(max77729->dev, "No active config specified (NULL)\n");
		ret = -ENODEV;
		goto err_unregister_debug;
	}
