...
static int qpnp_vibrator_play_on(struct vib_pwm_chip *chip)
{
	...
	if (chip->en_gpiod)
		gpiod_set_value_cansleep(chip->en_gpiod, 1);
...
static int qpnp_vibrator_play_off(struct vib_pwm_chip *chip)
{
	...
	if (chip->en_gpiod)
		gpiod_set_value_cansleep(chip->en_gpiod, 0);
...
static struct vib_pwm_chip *chip_from_dev(struct device *dev)
{
	struct led_classdev *cdev = dev_get_drvdata(dev);
	return container_of(cdev, struct vib_pwm_chip, cdev);
}

static ssize_t state_show(struct device *dev,
			  struct device_attribute *attr, char *buf)
{
	return sysfs_emit(buf, "%d\n", chip_from_dev(dev)->vib_enabled);
}
...
static ssize_t effect_store(struct device *dev,
			    ...
			    const char *buf, size_t count)
{
	struct vib_pwm_chip *chip = chip_from_dev(dev);
	u32 val;
	int ret;
			     ...
			     struct device_attribute *attr, char *buf)
{
	...
	if (hrtimer_active(&chip->stop_timer))
		time_ms = ktime_to_ms(hrtimer_get_remaining(&chip->stop_timer));
			      ...
			      const char *buf, size_t count)
{
	...
	if (chip->effect_idx == 0) {
		val = clamp(val, (u32)QPNP_VIB_MIN_PLAY_MS,
			         (u32)QPNP_VIB_MAX_PLAY_MS);
	}
...
static int qpnp_vib_parse_dt(struct vib_pwm_chip *chip)
{
	...
	child_node = of_get_next_available_child(node, NULL);
	if (!child_node) {
	...
	if (IS_ERR(pwm)) {
		...
		if (rc == -EPROBE_DEFER)
			return rc;
		pr_err("get pwm device failed: %d\n", rc);
		return rc;
	}
	chip->pwm_dev = pwm;

	chip->en_gpiod = devm_gpiod_get_optional(chip->dev, "vib,en",
						  GPIOD_OUT_LOW);
	if (IS_ERR(chip->en_gpiod)) {
		rc = PTR_ERR(chip->en_gpiod);
		pr_err("devm_gpiod_get_optional(vib,en) failed: %d\n", rc);
		return rc;
	}

	if (chip->en_gpiod)
		pr_info("vib,en-gpio acquired via descriptor\n");
	else
		pr_info("vib,en-gpio not present, skipping\n");
