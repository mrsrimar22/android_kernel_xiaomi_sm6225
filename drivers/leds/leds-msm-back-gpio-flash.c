...
static int led_gpio_flash_probe(struct platform_device *pdev)
{
	...
	flash_led = devm_kzalloc(&pdev->dev, sizeof(struct led_gpio_flash_data), GFP_KERNEL);
	pr_err("***************************Brave************************\n");
	if (flash_led == NULL) {
	...
	pr_info("Brave flash_led->flash_on = %d \n",flash_led->flash_en);
	...
	for (i = 0; i < 2; i++) {
		...
		pr_info("*****************Brave**********%s seq_name[%d] = %s\n", __func__, i, seq_name);
		if (rc < 0)
			dev_err(&pdev->dev, "%s failed %d\n", __func__, __LINE__);
		if (!strcmp(seq_name, "flash_en")) {
			flash_led->ctrl_seq[FLASH_EN].seq_type = FLASH_EN;
			pr_info("**********************Brave********%s:%d seq_type[%d] %d\n", __func__, __LINE__,
			...
			flash_led->ctrl_seq[FLASH_NOW].seq_type = FLASH_NOW;
			pr_info("*****************Brave********%s:%d seq_type[%d] %d\n", __func__, __LINE__,
