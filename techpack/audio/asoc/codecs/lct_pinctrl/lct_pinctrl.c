...
static int lct_pinctrl_ctrl_pin(lct_pinctrl_dev_t *lct_pinctrl_dev, int on)
{
	...
	if (lct_pinctrl_dev->reset_gpio_p) {
		if (on) {
			ret = msm_cdc_pinctrl_select_active_state(lct_pinctrl_dev->reset_gpio_p);
			if (ret < 0)
				pr_err("%s: set active failed (%d)\n", __func__, ret);
		} else {
			ret = msm_cdc_pinctrl_select_sleep_state(lct_pinctrl_dev->reset_gpio_p);
			if (ret < 0)
				pr_err("%s: set sleep failed (%d)\n", __func__, ret);
		}
	}
