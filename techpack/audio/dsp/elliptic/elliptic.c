...
static int device_open(struct inode *inode, struct file *filp)
{
	...
	if (major != elliptic_major || minor < 0 || minor >= ELLIPTIC_NUM_DEVICES) {
...
int elliptic_data_initialize(struct elliptic_data *elliptic_data, size_t queue_size,
	...
	size_t buffer_size, elliptic_data_push_t data_source)
{
	...
	struct elliptic_data *elliptic_data;
	unsigned int fifo_result;
	uint8_t zero_pad_buffer[ELLIPTIC_MSG_BUF_SIZE];
	...
	for (; i < i_max; ++i) {
		...
		if (data_source == ELLIPTIC_DATA_PUSH_FROM_KERNEL) {
			...
			copy_from_user_result = kfifo_from_user(&elliptic_data->fifo_isr, buffer,
		...
		if (zeros_to_pad > 0) {
			fifo_result = kfifo_in(&elliptic_data->fifo_isr, zero_pad_buffer,
				zeros_to_pad);

			if (fifo_result == 0) {
				elliptic_data_isr_fifo_pop(elliptic_data, buffer_size);
	...
	unsigned long param)
{
	...
	case IOCTL_ELLIPTIC_ACTIVATE_ENGINE: {
		...
		{
			int32_t msg[4] = {enable ? 1 : 0, 0, 0, 0};
	...
	case IOCTL_ELLIPTIC_DATA_IO_MIRROR: {
		...
		if ((mirror_tag == MIRROR_TAG) &&
			...
			unsigned char *payload = kzalloc(mirror_payload_size, GFP_KERNEL);

			if (!payload) {
				EL_PRINT_E("failed to alloc mirror payload buffer");
				return -ENOMEM;
			}
...
static void elliptic_driver_cleanup(int devices_to_destroy)
{
	...
	if (elliptic_devices) {
		...
		for (i = 0; i < devices_to_destroy; ++i) {
			...
			elliptic_device_cleanup(&elliptic_devices[i], i, elliptic_class);
		}
...
static size_t load_calibration_data(char *filename)
{
	...
	if (fd >= 0) {
		size_t bytes_read = ksys_read(fd, calibration_data,
