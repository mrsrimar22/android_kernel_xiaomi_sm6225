...
atomic_t elliptic_rx_active = ATOMIC_INIT(0);
EXPORT_SYMBOL(elliptic_rx_active);

atomic_t elliptic_tx_active = ATOMIC_INIT(0);
EXPORT_SYMBOL(elliptic_tx_active);
...
static struct elliptic_engine_calibration_data elliptic_engine_calibration_data_cache = {
	.reserved = {
...
static struct elliptic_engine_diagnostics_data elliptic_engine_diagnostics_data_cache = {
	.reserved = { 0 }
};
...
static struct elliptic_engine_tag_info elliptic_engine_tag_cache = {
	{ 0 }
};
...
static struct elliptic_shared_data_block shared_data_blocks[] = {
	...
	{ ELLIPTIC_OBJ_ID_CALIBRATION_V2_DATA, ELLIPTIC_CALIBRATION_V2_DATA_SIZE,
...
void elliptic_set_calibration_data(uint8_t *calib_data, size_t size)
{
	...
	if (size == ELLIPTIC_CALIBRATION_DATA_SIZE) {
		...
		memcpy(&elliptic_engine_calibration_data_cache, calib_data, size);
	}
	if (size == ELLIPTIC_CALIBRATION_V2_DATA_SIZE) {
		calibration_obj = elliptic_get_shared_obj(ELLIPTIC_OBJ_ID_CALIBRATION_V2_DATA);
		memcpy(&elliptic_engine_calibration_v2_data_cache, calib_data, size);
	}

	if (!calibration_obj) {
		EL_PRINT_E("ell..set_calibration_data() calib=NULL (%zu)", size);
		return;
	}

	memcpy(calibration_obj->buffer, calib_data, size);
}

static __maybe_unused void elliptic_set_diagnostics_data(uint8_t *diag_data, size_t size)
{
	...
	if (size <= ELLIPTIC_DIAGNOSTICS_DATA_SIZE) {
		...
		memcpy(&elliptic_engine_diagnostics_data_cache, diag_data, size);
		memcpy(diagnostics_obj->buffer, diag_data, size);
	}
}

static const size_t NUM_SHARED_RW_OBJS = ARRAY_SIZE(shared_data_blocks);
...
static __maybe_unused int get_elliptic_calibration_data(uint8_t *caldata, uint32_t max_size)
{
	...
	memcpy(caldata, &elliptic_engine_calibration_data_cache, max_size);
	return copied;
}

static __maybe_unused int get_elliptic_calibration_v2_data(uint8_t *caldata, uint32_t max_size)
{
	uint32_t copied = ELLIPTIC_CALIBRATION_V2_DATA_SIZE;
	...
	memcpy(caldata, &elliptic_engine_calibration_v2_data_cache, max_size);
	return copied;
}

static __maybe_unused int get_elliptic_diagnostics_data(uint8_t *diagdata, uint32_t max_size)
{
	uint32_t copied = ELLIPTIC_DIAGNOSTICS_DATA_SIZE;
	...
	memcpy(diagdata, &elliptic_engine_diagnostics_data_cache, max_size);
	return copied;
}

static uint32_t ultrasound_enable_cache;

static int elliptic_ultrasound_enable_get(struct snd_kcontrol *kcontrol,
...
static int elliptic_ultrasound_enable_set(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
...
static int elliptic_ultrasound_tx_port_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
...
static int elliptic_ultrasound_tx_port_set(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	int ret;
...
static int elliptic_ultrasound_rx_port_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
...
static int elliptic_ultrasound_rx_port_set(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	int ret;
...
static int elliptic_ultrasound_rampdown_get(struct snd_kcontrol *kcontrol,
				     struct snd_ctl_elem_value *ucontrol)
{
...
static int elliptic_ultrasound_rampdown_set(struct snd_kcontrol *kcontrol,
				     struct snd_ctl_elem_value *ucontrol)
{
...
static int elliptic_ultrasound_diagnostics_get(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = 0;
	return 0;
...
static int elliptic_ultrasound_request_diagnostics(struct snd_kcontrol *kcontrol,
					    struct snd_ctl_elem_value *ucontrol)
{
...
static int elliptic_system_configuration_get(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
...
static int elliptic_system_configuration_put(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
...
static int elliptic_calibration_data_get(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
...
static int elliptic_calibration_data_put(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
...
static int elliptic_calibration_v2_data_get(struct snd_kcontrol *kcontrol,
				     struct snd_ctl_elem_value *ucontrol)
{
...
static int elliptic_calibration_v2_data_put(struct snd_kcontrol *kcontrol,
				     struct snd_ctl_elem_value *ucontrol)
{
...
static int elliptic_diagnostics_data_get(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	memcpy(ucontrol->value.bytes.data,
	       &elliptic_engine_diagnostics_data_cache,
...
static int elliptic_diagnostics_data_put(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static int elliptic_ml_data_get(struct snd_kcontrol *kcontrol,
			 struct snd_ctl_elem_value *ucontrol)
{
	...
	memcpy(ucontrol->value.bytes.data, ml_obj->buffer, ELLIPTIC_ML_DATA_SIZE);
	return 0;
}

static int elliptic_ml_data_put(struct snd_kcontrol *kcontrol,
			 struct snd_ctl_elem_value *ucontrol)
{
	return elliptic_data_write(ELLIPTIC_ULTRASOUND_SET_PARAMS,
...
static int elliptic_sensor_data_get(struct snd_kcontrol *kcontrol,
			     struct snd_ctl_elem_value *ucontrol)
{
	memcpy(ucontrol->value.bytes.data,
...
static int elliptic_sensor_data_put(struct snd_kcontrol *kcontrol,
			     struct snd_ctl_elem_value *ucontrol)
{
...
static int elliptic_version_data_get(struct snd_kcontrol *kcontrol,
			      struct snd_ctl_elem_value *ucontrol)
{
...
static int elliptic_version_data_put(struct snd_kcontrol *kcontrol,
			      struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static int elliptic_branch_data_get(struct snd_kcontrol *kcontrol,
			     struct snd_ctl_elem_value *ucontrol)
{
...
static int elliptic_branch_data_put(struct snd_kcontrol *kcontrol,
			     struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static int elliptic_tag_data_get(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
...
static int elliptic_tag_data_put(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static int elliptic_calibration_param_get(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
...
static int elliptic_calibration_param_put(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct soc_mixer_control *mc = (struct soc_mixer_control *)kcontrol->private_value;
...
static int elliptic_system_configuration_param_get(struct snd_kcontrol *kcontrol,
					    struct snd_ctl_elem_value *ucontrol)
{
	struct soc_mixer_control *mc = (struct soc_mixer_control *)kcontrol->private_value;
...
static int elliptic_system_configuration_param_put(struct snd_kcontrol *kcontrol,
					    struct snd_ctl_elem_value *ucontrol)
{
	struct soc_mixer_control *mc = (struct soc_mixer_control *)kcontrol->private_value;
	struct elliptic_system_configuration_parameter param;
	...
	if (mc->shift >= ELLIPTIC_SYSTEM_CONFIGURATION_CUSTOM_SETTING_0 &&
	    mc->shift <= ELLIPTIC_SYSTEM_CONFIGURATION_CUSTOM_SETTING_15) {
		csi = mc->shift - ELLIPTIC_SYSTEM_CONFIGURATION_CUSTOM_SETTING_0;
		EL_PRINT_E("ELLIPTIC_SYSTEM_CONFIGURATION_CUSTOM_SETTING_XX csi:%zu", csi);

		if (csi >= ARRAY_SIZE(elliptic_system_configuration_cache.custom_settings))
			return -EINVAL;

		EL_PRINT_E("ucontrol->value.integer.value[0]:%ld", ucontrol->value.integer.value[0]);

		elliptic_system_configuration_cache.custom_settings[csi] = ucontrol->value.integer.value[0];
	...
	switch (mc->shift) {
	case ELLIPTIC_SYSTEM_CONFIGURATION_LATENCY:
		elliptic_system_configuration_cache.latency = ucontrol->value.integer.value[0];
		param.type = ESCPT_LATENCY;
		param.latency = elliptic_system_configuration_cache.latency;
		break;
	case ELLIPTIC_SYSTEM_CONFIGURATION_SENSITIVITY:
		elliptic_system_configuration_cache.sensitivity = ucontrol->value.integer.value[0];
		param.type = ESCPT_CHANNEL_SENSITIVITY;
		param.sensitivity = elliptic_system_configuration_cache.sensitivity;
		break;
	case ELLIPTIC_SYSTEM_CONFIGURATION_SPEAKER_SCALING:
		elliptic_system_configuration_cache.speaker_scaling[0] = ucontrol->value.integer.value[0];
		elliptic_system_configuration_cache.speaker_scaling[1] = ucontrol->value.integer.value[1];
		param.type = ESCPT_SPEAKER_SCALING;
		param.speaker_scaling[0] = elliptic_system_configuration_cache.speaker_scaling[0];
		param.speaker_scaling[1] = elliptic_system_configuration_cache.speaker_scaling[1];
		break;
	case ELLIPTIC_SYSTEM_CONFIGURATION_MICROPHONE_INDEX:
		elliptic_system_configuration_cache.microphone_index = ucontrol->value.integer.value[0];
		param.type = ESCPT_MICROPHONE_INDEX;
		param.microphone_index = elliptic_system_configuration_cache.microphone_index;
		break;
	case ELLIPTIC_SYSTEM_CONFIGURATION_OPERATION_MODE:
		elliptic_system_configuration_cache.operation_mode = ucontrol->value.integer.value[0];
		param.type = ESCPT_OPERATION_MODE;
		param.operation_mode = elliptic_system_configuration_cache.operation_mode;
		break;
	case ELLIPTIC_SYSTEM_CONFIGURATION_OPERATION_MODE_FLAGS:
		elliptic_system_configuration_cache.operation_mode_flags = ucontrol->value.integer.value[0];
		param.type = ESCPT_OPERATION_MODE_FLAGS;
		param.operation_mode_flags = elliptic_system_configuration_cache.operation_mode_flags;
		break;
	case ELLIPTIC_SYSTEM_CONFIGURATION_LOG_LEVEL:
		elliptic_system_configuration_cache.log_level = ucontrol->value.integer.value[0];
		param.type = ESCPT_LOG_LEVEL;
		param.log_level = elliptic_system_configuration_cache.log_level;
		break;
	case ELLIPTIC_SYSTEM_CONFIGURATION_INPUT_ENABLED:
		elliptic_system_configuration_cache.input_enabled = ucontrol->value.integer.value[0];
		param.type = ESCPT_INPUT_ENABLED;
		param.input_enabled = elliptic_system_configuration_cache.input_enabled;
		break;
	case ELLIPTIC_SYSTEM_CONFIGURATION_OUTPUT_ENABLED:
		elliptic_system_configuration_cache.output_enabled = ucontrol->value.integer.value[0];
		param.type = ESCPT_OUTPUT_ENABLED;
		param.output_enabled = elliptic_system_configuration_cache.output_enabled;
		break;
	case ELLIPTIC_SYSTEM_CONFIGURATION_SUSPEND:
		elliptic_system_configuration_cache.engine_suspend = ucontrol->value.integer.value[0];
		param.type = ESCPT_SUSPEND;
		param.engine_suspend = elliptic_system_configuration_cache.engine_suspend;
		break;
	case ELLIPTIC_SYSTEM_CONFIGURATION_EXTERNAL_EVENT:
		elliptic_system_configuration_cache.external_event = ucontrol->value.integer.value[0];
		param.type = ESCPT_EXTERNAL_EVENT;
		param.external_event = elliptic_system_configuration_cache.external_event;
		break;
	case ELLIPTIC_SYSTEM_CONFIGURATION_CALIBRATION_METHOD:
		elliptic_system_configuration_cache.calibration_method = ucontrol->value.integer.value[0];
		param.type = ESCPT_CALIBRATION_METHOD;
		param.calibration_method = elliptic_system_configuration_cache.calibration_method;
		ktime_get_real_ts64(&ts);
		param.calibration_timestamp = (int32_t)ts.tv_sec;
		break;
	case ELLIPTIC_SYSTEM_CONFIGURATION_DEBUG_MODE:
		elliptic_system_configuration_cache.debug_mode = ucontrol->value.integer.value[0];
		param.type = ESCPT_DEBUG_MODE;
		param.debug_mode = elliptic_system_configuration_cache.debug_mode;
		break;
	case ELLIPTIC_SYSTEM_CONFIGURATION_CONTEXT:
		elliptic_system_configuration_cache.context = ucontrol->value.integer.value[0];
		param.type = ESCPT_CONTEXT;
		param.context = elliptic_system_configuration_cache.context;
		break;
	case ELLIPTIC_SYSTEM_CONFIGURATION_CAPTURE:
		elliptic_system_configuration_cache.capture = ucontrol->value.integer.value[0];
		param.type = ESCPT_CAPTURE;
		param.capture = elliptic_system_configuration_cache.capture;
		break;
	case ELLIPTIC_SYSTEM_CONFIGURATION_INPUT_CHANNELS:
		elliptic_system_configuration_cache.input_channels = ucontrol->value.integer.value[0];
		param.type = ESCPT_INPUT_CHANNELS;
		param.input_channels = elliptic_system_configuration_cache.input_channels;
		break;
	case ELLIPTIC_SYSTEM_CONFIGURATION_RE_SEND:
		elliptic_system_configuration_cache.re_send = ucontrol->value.integer.value[0];
		param.type = ESCPT_RE_SEND;
		param.re_send = elliptic_system_configuration_cache.re_send;
		break;
	default:
		return -EINVAL;
	}
...
static const struct snd_kcontrol_new ultrasound_filter_mixer_controls[] = {
	...
	SOC_SINGLE_EXT("Ultrasound Custom Setting 0",
		       ...
		       ELLIPTIC_SYSTEM_CONFIGURATION_CUSTOM_SETTING_MAX_VALUE, 0,
		       elliptic_system_configuration_param_get,
		       elliptic_system_configuration_param_put),
