		...
		struct sde_crtc *sde_crtc, bool enable)
{
	...
	crtc = &sde_crtc->base;
	/* Async worker: do not access crtc->state; use cached_encoder_mask */
	...
	if (enable) {
		...
		/* Do not enable vblank if CRTC is already disabled */
		if (!sde_crtc->enabled)
			return -EINVAL;
		...
		/*
		 * Invalidate cached encoder mask once CRTC is disabled and
		 * all encoder vblank callbacks have been unregistered.
		 */
		if (!sde_crtc->enabled)
...
static void sde_crtc_disable(struct drm_crtc *crtc)
{
	...
	/*
	 * Do not zero cached_encoder_mask here: the asynchronous
	 * vblank disable worker still needs it to unregister encoder
	 * callbacks. It is cleared once the worker completes.
	 */
		...
		struct drm_crtc_state *old_crtc_state)
{
	...
	/*
	 * Refresh cached_encoder_mask on every enable for vblank work.
	 * Avoid drm_crtc_vblank_on during seamless DMS case when CRTC
