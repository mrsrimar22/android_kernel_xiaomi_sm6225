...
// this pin conflicting with dsi_mdp_transfer
...
/*static void enable_i2c_pullup()
{
	...
	if (!gpio_is_valid(iov_gpio)) {
		...
		ret = devm_gpio_request_one(&wl2866d_data.i2c_client->dev, iov_gpio, GPIOF_OUT_INIT_HIGH, "wl2866d_iov");
		if (ret < 0) {
...
int wl2866d_camera_power_control(unsigned int out_iotype, int is_power_on)
{
	...
	if (ret < 0)
		pr_err("set discharge mode fail\n");
	...
	if (is_power_on) {
		...
		if (ret < 0 || reg_read != wl2866d_on_config[out_iotype].value) {
			...
			if (ret < 0 ) {
				pr_err("set voltage %d fail\n", out_iotype);
				goto fail_return;
			}
		} else {
			pr_debug("voltage is right, skip write reg\n");
		}
		...
		if ((1050000 == is_power_on) && (OUT_DVDD1 == out_iotype)) {
			...
			if (ret < 0)
				pr_err("set DVDD1 voltage failed\n");
			else
				pr_info("set DVDD1 to 1.05V\n");
		}
	} else {
		reg_val &= ~(1 << out_iotype);
			     ...
			     size_t count, loff_t *offset)
{
	...
	if (count > WL2866D_IO_BUFFER_LIMIT) {
		pr_err("out of range, write size %d > %d\n", count, WL2866D_IO_BUFFER_LIMIT);
		return -ERANGE;
	}
			 ...
			 const struct i2c_device_id *id)
{
	...
	if (ret < 0) {
		pr_err("register misc_device failed (%d)\n", ret);
		return ret;
	}
...
static int wl2866d_remove(struct i2c_client *client)
{
	...
	if (ret < 0) {
		pr_err("deregister misc_device failed (%d)\n", ret);
		return ret;
	}
...
static int __init cam_wl2866_init_module(void)
{
	...
	do {
		...
		pr_info("init retry: %d\n", num_retry);
		num_retry--;
	} while ((!wl2866d_data.on) && (num_retry > 0));

	pr_info("init successed\n");
	return ret;
}
