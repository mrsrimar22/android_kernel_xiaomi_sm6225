...
struct wl2866d_device {
	...
	bool			on;
	struct mutex		lock;
	char			io_buf[WL2866D_IO_BUFFER_LIMIT];
	char			misc_name[32];
};
					  ...
					  int is_power_on)
{
	...
	if (is_power_on) {
		u8 vset = (out_iotype == OUT_DVDD2 && is_power_on == 1200000) ?
			  0x64 : wl2866d_rail_cfg[out_iotype].default_val;
		...
		return wl2866d_update_bits(wdev, WL2866D_REG_OUT_EN,
	...
	return wl2866d_update_bits(wdev, WL2866D_REG_DISCHARGE,
				   BIT(out_iotype), BIT(out_iotype));
}
...
static int wl2866d_match_id(struct wl2866d_device *wdev)
{
	...
	u8 id_reg, id_val, id_val1, chip_id;
	int ret;

	ret = of_property_read_u8(np, "id_reg", &id_reg);
	if (ret) {
		pr_err("id_reg missing or invalid\n");
		return ret;
	}

	ret = of_property_read_u8(np, "id_val", &id_val);
	if (ret) {
		pr_err("id_val missing or invalid\n");
		return ret;
	}

	ret = of_property_read_u8(np, "id_val1", &id_val1);
	if (ret) {
		pr_err("id_val1 missing or invalid\n");
		return ret;
	}

	ret = wl2866d_read_reg(wdev, id_reg, &chip_id);
	if (ret) {
		pr_err("id read failed: %d\n", ret);
		return ret;
	}

	if (chip_id != id_val && chip_id != id_val1) {
		pr_err("id mismatch: chip_id=0x%02x expected=0x%02x/0x%02x\n",
		       chip_id, id_val, id_val1);
		return -ENODEV;
	}
...
static int wl2866d_init_module_dev(struct wl2866d_device *wdev)
{
	...
	for (i = 0; i < (int)num; i++) {
		ret = wl2866d_write_reg(wdev, (u8)inits[i * 2],
					(u8)inits[i * 2 + 1]);
		if (ret < 0) {
